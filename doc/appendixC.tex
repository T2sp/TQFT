...
\begin{proof}
    \begin{description}
        ...
        　以上の考察から，自然同型 $\theta$ は，図式 $D$ の勝手な錐 $(C,\, \tau) \in \Obj{\CONE{D}}$ が与えられると，対応する $\theta_C^{-1}(\tau) \in \Hom{\Cat{C}}(C,\, X)$ を一意的に定めるが，これは\hyperref[def:lim]{極限の普遍性}に他ならない．
...
\begin{myexample}[label=def:Kan]{Kan複体の圏}
    ...
    　さらに，$\Kan$ は\exref{def:Fun-infty}と同様の方法で\hyperref[def:SimpCat]{単体的豊穣圏}と見做すこともできる．
...
\begin{mydef}[label=def:infinity-presheaf]{{$(\infty,\, 1)$}-前層}
    \begin{itemize}
        ...
        \end{align}
        のこと．
        \item $K$ 上の $\bm{(\infty,\, 1)}$\textbf{-前層が成す} $\bm{(\infty,\, 1)}$\textbf{-圏}とは，$(\infty,\, 1)$-前層全体の集合 $\Hom{\sSet} (\OP{K},\, \SPACES)$ を\exref{def:Fun-infty}の構成と全く同じ方法で\hyperref[def:SimpSet]{単体的集合}と見做した
        \begin{align}
