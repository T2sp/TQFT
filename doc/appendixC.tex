...
\begin{enumerate}
    ...
    \begin{center}
        \begin{tikzpicture}
            \pgfmathsetmacro{\factor}{sqrt(3)/2};
            \pgfmathsetmacro{\factorh}{sqrt(2/3)};
            \node[bullet={below:$x$}] (x) at (0,0,0)  {};
            \node[bullet={below:$y$}] (y) at (2,0,0) {};
            \node[bullet={below:$z$}] (z) at (1,2*\factor,0)  {};
            \node[bullet={below:$w$}] (w) at (1,2/3*\factor,2*\factorh)  {};
            % \node (s) at ($(1,0)!0.33!(z)$) {$\sigma$};
        \end{tikzpicture}
    \end{center}
...
\begin{myprop}[label=prop:boundary-coeq]{コイコライザとしての境界}
    \hyperref[def:horn]{境界} $\partial \Delta^n$ は圏\hyperref[def:SimpSet]{$\SimpSet$}における\hyperref[def:colim]{コイコライザ}である：
    \begin{center}
        \begin{tikzcd}[row sep=large, column sep=large]
...
\begin{myprop}[label=prop:horn-coeq]{コイコライザとしての角}
    \hyperref[def:horn]{角} $\Lambda^n_i$ は圏\hyperref[def:SimpSet]{$\SimpSet$}における\hyperref[def:colim]{コイコライザ}である：
    \begin{center}
        \begin{tikzcd}[row sep=large, column sep=large]
             &\coprod_{\substack{j,\, k \in [n] \setminus \{i\} \\ j < k}} \Delta^{n-2} \ar[r, shift left, "u"]\ar[r, shift right, "v"'] &\coprod_{l \in [n] \setminus \{i\}} \Delta^{n-1} \ar[r, "w"] &\partial \Delta^n
        \end{tikzcd}
    \end{center}
\end{myprop}
