...
\begin{mydef}[label=def:nerve]{脈体}
    \begin{itemize}
        ...
        のこと．
        \begin{itemize}
            \item $\forall \Cat{C} \in \Obj{\Cat{C}}$ に対して $\mathcal{N}(\Cat{C}) \in \Obj{\SimpSet}$ を対応づける．
...
\begin{proof}
    ...
    \begin{description}
        ...
        \end{align}
        が対応付く．その上 $f$ が自然変換であることから\hyperref[def:simplex-cat]{面写像}と $f$ は交換するので
        \begin{align}
            \partial_0^1 \circ f_{[1]}(u) = f_{[0]} \circ \partial_0^1 (u) = f_{[0]} (X), \\
            \partial_1^1 \circ f_{[1]}(u) = f_{[0]} \circ \partial_1^1 (u) = f_{[0]} (Y)
        \end{align}
        ...
        \end{itemize}
        ものとして定義する．もし $F_f$ が関手ならば明らかに $\theta(F_f) = f$ であるから，$F_f$ が関手であることを示せば良い．実際\hyperref[def:simplex-cat]{縮退写像}により
        \begin{align}
            F_f(\Id_X) = \Id_{F(X)}
        \end{align}
        がわかる．合成則は
