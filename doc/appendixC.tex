...
\begin{mylem}[label=lem:n-fold-Map]{{$n$}-fold mapping spaceの非退化な単体}
    ...
    \begin{align}
        ...
        &\cong \Biggl\{\, (\sigma_w)_{w \in W_{m,\, n}} \in C_K^{m,\, n} \Biggm| \substack{0 \le \forall j \le n, \\ \partial^{m+1}_0 \cdots \partial^{m+j}_{j-1} \circ \partial^{m+j+1}_{m+j+1} \cdots \partial^{m+n}_{m+n}(\sigma_{w_{j}}) = x_j} \,\Biggr\}
    \end{align}
...
\end{align}
と定義する．$m$-射の対応は，補題\ref{lem:n-fold-Map}と同様に系\ref{col:product-coeq}の自然同型を用いて
\begin{align}
    C^{m,\, n}_K \xrightarrow{r} \Hom{\sSet} (\Delta^m \times \Delta^n,\, K) \xrightarrow{\theta^n_{ij}{}_{[m]}} \Hom{\sSet} (\Delta^m \times \Delta^1,\, K) \xrightarrow{\bar{q}^*} C^{m,\, 1}_K
\end{align}
の順に計算すると
\begin{align}
    ...
    &= \Bigl( K(\gamma_{\omega_w})(\sigma_{w_{\omega_w}}) \Bigr)_{w \in W_{m,\, 1}}
\end{align}
となる．ただし，$\omega_w \coloneqq (\Id_{[m]} \times q^n_{ij}) \circ \nu_w \in \Hom{\Delta} \bigl( [m+1],\, [m] \times [n] \bigr)$ とおいた\footnote{$\omega_w = (\Id_{[m]} \times q^n_{ij}) \circ (\nu_w{}_*)_{[m+1]} (\Id_{[m+1]})$ である．}．
