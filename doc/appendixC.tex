...
\begin{align}
    W_{m,\, n} \coloneqq \Bigl\{\, (w_1,\, \dots,\, w_{m+n}) \in \{0,\, 1\}^{\times (m+n)} \Bigm| \#\bigl\{\, i \bigm| w_i=0 \,\bigr\} = m,\; \#\bigl\{\, j \bigm| w_j=1 \,\bigr\} = n \,\Bigr\} 
\end{align}
さらに，$\forall w \in W_{m,\, n}$ に対して順序を保つ写像 $\sigma_w \in \Hom{\Delta} \bigl( [m+n],\, [m] \times [n] \bigr)$ を次のように定義する：
\begin{align}
    \sigma_w (k) \coloneqq \bigl( \# \{i\mid i \le k,\; w_i = 0\},\, \# \{j\mid j \le k,\; w_j = 1\} \bigr) 
\end{align}
これは単射 $\sigma_w{}_* \in \Hom{\sSet}(\Delta^{m+n},\, \Delta^{m} \times \Delta^{n})$ を次のようにして定める：
\begin{align}
   (\sigma_w{}_*)_{[p]} \colon \Delta^{m+n}_p &\lto \Delta^{m}_p \times \Delta^{n}_p \cong \Hom{\Delta} \bigl( [p],\, [m] \times [n] \bigr), \\
    \alpha &\lmto \sigma_w \circ \alpha
\end{align}
