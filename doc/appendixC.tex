...
\begin{mydef}[label=def:SimpSet,breakable]{単体的集合}
    ...
    \begin{align}
        \partial^{n-1}_i \circ \partial^{n}_j &= \partial^{n-1}_{j-1} \circ \partial^{n}_i&& (i<j), \\
        \partial^{n+1}_i \circ \sigma^{n}_j &= \sigma^{n-1}_{j-1} \circ \partial^{n}_i && (i<j), \\
        \partial^{n+1}_i \circ \sigma^{n}_i &= \mathrm{id} &&(0\le i \le n), \\
        \partial^{n+1}_{i+1} \circ \sigma^{n}_i &= \mathrm{id} &&(0\le i \le n), \\
        \partial^{n+1}_i \circ \sigma^{n}_j &= \sigma^{n-1}_j \circ \partial^{n}_{i-1} &&(i > j+1), \\
        \sigma^{n+1}_i \circ \sigma^{n}_j &= \sigma^{n+1}_{j+1} \circ \sigma^{n}_i &&(i \le j)
    \end{align}
\end{mydef}
...
\begin{mydef}[label=def:SimpSet-homotopic,breakable]{単体的ホモトピー}
    \begin{itemize}
        ...
        \begin{center}
            \begin{tikzcd}[row sep=large, column sep=large]
                &X \cong X \times \Delta^0 \ar[r, "\Id \times \sigma_1"]\ar[dr, "f"] &X \times \Delta^1 \ar[d, "\eta"] &X \times \Delta^1 \ar[l, "\Id \times \sigma_0"] \ar[dl, "g"] \\
                & &Y &
            \end{tikzcd}
        \end{center}
