...
\begin{marker}
    命題\ref{prop:SimpSet-basic}-(1) によって，\hyperref[def:SimpSet]{$n$-単体} $\sigma \in K_n$ を\hyperref[def:nat]{自然変換} $\sigma \in \Hom{\SimpSet}(\Delta^n,\, K)$ と同一視できる！
\end{marker}
...
\begin{proof}
    \begin{enumerate}
        ...
        \begin{align}
            ...
            \cong K([n]) = K_n
        \end{align}
        \item \hyperref[def:diagram]{図式} $D \colon I \lto \SimpSet$ を与える．
...
\begin{proof}
    ...
    よって\hyperref[def:eq-coeq]{コイコライザの普遍性}から圏 $\SimpSet$ の可換図式
    \begin{center}
        \begin{tikzcd}[row sep=large, column sep=large]
...
\begin{proof}
    ...
    \begin{description}
        ...
        $\forall f \in \Hom{\SimpSet}\bigl(\Lambda^n_j,\, \mathrm{N}(\Cat{C})\bigr)$ を1つ与える．このとき $0 < \forall j < \forall n$ に対して $f$ が $u \in \Hom{\SimpSet} \bigl( \Delta^n,\, \mathrm{N}(\Cat{C}) \bigr)$ へ一意的に拡張できることを示せば良い．
        ...
        \end{align}
        が定まる．ここから\eqref{eq:nerve-spine}の方法で $U \in \mathrm{N}(\Cat{C})_n$ が一意的に定まり，\hyperref[lem:Yoneda]{米田の補題}により対応する $u \in \Hom{\SimpSet} \bigl(\Delta^n,\, \mathrm{N}(\Cat{C})\bigr)$ が一意的に定まるが，構成からこれが所望の $u$ である~\cite[Lemma1.3.4.2]{Kerodon}．
        ...
        \begin{itemize}
            ...
            \begin{description}
                ...
                $(g,\, f) \in K_1 \times K_1$ かつ $\partial^1_1(g) = \partial^1_{0}(f)$ が成り立つので系\ref{col:horn-coeq}が使えて，$(g,\, f) \in \Hom{\SimpSet}(\Lambda^2_1,\, K)$ と見做せる．
                \item[\textbf{(STEP2)}]　 
                
                仮定より，$(g,\, f) \in \Hom{\SimpSet}(\Lambda^2_1,\, K)$ に対する弱Kan条件の\underline{一意解} $u \in \Hom{\SimpSet}(\Delta^2,\, K) \cong K_2$ が存在する．
                \item[\textbf{(STEP3)}]　 
        ...
        \begin{description}
            ...
            \end{align}
            を充たすため系\ref{col:horn-coeq}が使えて，$(u_0,\, u_2,\, u_3) \in \Hom{\SimpSet}(\Lambda^3_1,\, K)$ と見做せる．すると，仮定より\hyperref[def:KanCplx]{弱Kan条件}の一意解
            \begin{center}
                \begin{tikzcd}[row sep=large, column sep=large]
                ...
                \end{tikzcd}
            \end{center}
            が存在する．i.e. ある $\tau \in K_3$ が\footnote{\hyperref[lem:Yoneda]{米田の補題}より $\tau \in \Hom{\SimpSet}(\Delta^3,\, K) \cong K_3$ である．}一意的に存在して，
            \begin{align}
                u_0 &= \partial^3_0 (\tau), &
                u_2 &= \partial^3_2 (\tau), &
                u_3 &= \partial^3_3 (\tau)
            ...
            \begin{center}
                \begin{tikzcd}[row sep=large, column sep=large]
                    &  & C \arrow[rd, "f"] \arrow[ddd, "(h\circ g) \circ f"] \arrow[lldd, "g \circ f"'] &                                                     \\
                    &  &                                                                                & D \arrow[ldd, "h\circ g"] \arrow[llld, "g", dashed] \\
                 E\arrow[rrd, "h"'] &  &                                                                                &                                                     \\
                    &  & F                                                                              &                                                    
                \end{tikzcd}
            \end{center}
        ...
        \end{align}
        を構成しよう．$\theta$ は\hyperref[def:nat]{自然変換}であるから，$\forall [n] \in $ に対して写像 $\theta_{[n]} \colon K_n \lto \mathrm{N}(\Cat{C})_n$ を定めれば良い．命題\ref{prop:SimpSet-basic}-(1) より $\forall \sigma \in K_n$ は自然に $\Hom{\SimpSet} (\Delta^n,\, K)$ の元と見做せるため，
        \begin{align}
        ...
        \begin{align}
            \theta_* \colon \Hom{\SimpSet} (\Delta^n,\, K) &\lto \Hom{\SimpSet} \bigl(\Delta^n,\, \mathrm{N}(\Cat{C})\bigr), \\
            \sigma &\lmto \theta \circ \sigma
        \end{align}
        が全単射であることである．$0 < j < n$ を1つとる．$u \in \Hom{\SimpSet} (\Lambda^n_j,\, K)$ に対する\hyperref[def:KanCplx]{弱Kan条件}の解を $\overline{u} \in \Hom{\SimpSet} (\Delta^n,\, K)$ と書くと，解の一意性の仮定より写像
        \begin{align}
            \phi_K \colon \Hom{\SimpSet} (\Delta^n,\, K) &\lto \Hom{\SimpSet} (\Lambda^n_j,\, K), \\
            \overline{u} &\lmto u
        \end{align}
        は全単射である．$\mathrm{N}(\Cat{C})$ にも同様の構成ができるため，可換図式
        \begin{center}
            \begin{tikzcd}
                {\Hom{\SimpSet}(\Delta^n,\, K)} \arrow[d, "\phi_K"'] \arrow[rr, "\theta_*"] &  & {\Hom{\SimpSet}\bigl(\Delta^n,\, \mathrm{N}(\Cat{C})\bigr)} \arrow[d, "\phi_{\mathrm{N}(\Cat{C})}"] \\
                {\Hom{\SimpSet}(\Lambda_j^n,\, K)} \arrow[rr, "\theta_*|_{\Lambda^n_j}"]    &  & {\Hom{\SimpSet}\bigl(\Lambda_j^n,\, \mathrm{N}(\Cat{C})\bigr)}                                     
            \end{tikzcd}
        \end{center}
...
\begin{proof}
    ...
    \begin{description}
        ...
        \begin{align}
            \theta^n_j \colon \Hom{\SimpSet}\bigl(\Delta^n,\,\mathrm{N}(\Cat{C})\bigr) &\lto \Hom{\SimpSet}\bigl(\Lambda_j^n,\,\mathrm{N}(\Cat{C})\bigr)
        \end{align}
        が存在する．
        ...
        \end{align}
        を充たすため系\ref{col:horn-coeq}が使えて，$(f,\, \Id_Y) \in \Hom{\SimpSet}\bigl(\Lambda^2_2,\, \mathrm{N}(\Cat{C})\bigr)$ と見做せる．
        このとき $\theta^2_2$ の全射性により，$\sigma \in \mathrm{N}(\Cat{C})_2 \cong \Hom{\SimpSet} \bigl( \Delta^2,\, \mathrm{N}(\Cat{C}) \bigr)$ が存在して $(f,\, \Id_Y) = \theta^2_2(\sigma)$ を充たす．
        ...
        \end{align}
        が成り立つことが分かった．
        同様に，$(\Id_X,\, f) \in \Hom{\SimpSet}\bigl(\Lambda^2_0,\, \mathrm{N}(\Cat{C})\bigr)$ と見做せること，および $\theta^2_0$ の全射性から $h \in \mathrm{N}(\Cat{C})_1 = \Hom{\Cat{C}} (Y,\, X)$ であって $h \circ f = \Id_X$ を充たすものが存在する．
        ...
        $\forall \sigma \in \Hom{\SimpSet}\bigl( \Lambda^n_0,\, \mathrm{N}(\Cat{C}) \bigr)$ は $\Obj{\Cat{C}}$ の元である．よって，$\bar{\sigma} \in \Hom{\SimpSet}\bigl(\Delta^1,\, \mathrm{N}(\Cat{C})\bigr)$ として $\bar{\sigma} \in \Hom{\Cat{C}}(X,\, \sigma)$ をとれば $\partial^1_0(\bar{\sigma}) = \sigma$ を充たす．
        このことは系\ref{col:horn-coeq}より $\bar{\sigma}|_{\Lambda^n_0} = \sigma$ を意味する．

        　次に，$n=2,\, j=0$ とする．このとき $\forall \sigma \in \Hom{\SimpSet}\bigl( \Lambda^n_0,\, \mathrm{N}(\Cat{C}) \bigr)$ は，系\ref{col:horn-coeq}より $\bigl(\sigma_1 \coloneqq \partial^2_1(\sigma),\, \sigma_2 \coloneqq \sigma^2_2(\sigma)\bigr) \in \mathrm{N}(\Cat{C})_{1}^{\times 2}$ であって以下の図式として書けるものと同一視できる：
        \begin{center}
            \begin{tikzcd}
                & Z &   \\
            ...
            \end{tikzcd}
        \end{center}
        $\Cat{C}$ は\hyperref[def:groupoid]{groupoid}なので，この図式に $\sigma_0 \coloneqq \sigma_1 \circ \sigma_2 \in \mathrm{N}(\Cat{C})_1$ を付け足すことで所望の $\bar{\sigma} \in \mathrm{N}(\Cat{C})_2 \cong \Hom{\SimpSet} \bigl(\Delta^2,\, \mathrm{N}(\Cat{C})\bigr)$ を得る．
        ...
        % $\forall \sigma \in \Hom{\SimpSet} \bigl( \Lambda^n_0,\, \mathrm{N}(\Cat{C}) \bigr)$ から次のようにして $\mathrm{N}(\Cat{C})_n$ の元を構成する：
...
\begin{mydef}[label=def:enriched-functor]{豊穣関手}
