...
\begin{myexample}[label=ex:Map2]{射の空間の2-射}
    ...
    \end{align}
    の存在が分かる．後に示す系\ref{col:product-coeq}および補題\ref{lem:n-fold-Map}によると，一般の $n$ の場合は次のようになる：
    \begin{align}
        \label{eq:Mapn}
        \inftyMap{K}(x,\, y)_n
        &\cong \Bigl\{\, (\sigma_0,\, \dots,\, \sigma_n) \in K_{n+1}^{\times (n+1)} \Bigm| \substack{1 \le \forall j \le n,\; \partial^{n+1}_j(\sigma_{j-1}) = \partial^{n+1}_j(\sigma_j) \\ \partial^{n+1}_0(\sigma_0) = \sigma^n_0 \circ \cdots \circ \sigma^0_0(y),\; \partial^{n+1}_{n+1} (\sigma_n) = \sigma^{n}_{0} \circ \cdots \circ \sigma^0_0(x)} \,\Bigr\} 
    \end{align}
\end{myexample}
...
\textbf{Shuffle}と呼ばれる集合を次のように定義する：
\begin{align}
    W_{m,\, n} \coloneqq \bigl\{\, (w_1,\, \dots,\, w_{m+n}) \in \{0,\, 1\}^{\times (m+n)} \bigm| \# \{\, i \mid w_i = 0 \,\} = m,\; \# \{\, j \mid w_j = 1 \,\} = n \,\bigr\} 
\end{align}
...
さらに，
\begin{align}
    S_{m,\, n} \coloneqq \bigl\{\, (w,\, k) \in W_{m,\, n} \times \{1,\, \dots,\, m+n-1\} \bigm| w_k = 0,\; w_{k+1} = 1 \,\bigr\}
\end{align}
とおき，写像
\begin{align}
    \tau_k \colon W_{m,\, n} &\lto W_{m,\, n}, \\
    (w_1,\, \dots,\, w_{m+n}) &\lmto \bigl( w_1,\, \dots,\, w_{k+1},\, w_k,\, \dots,\, w_{m+n} \bigr)
\end{align}
を考える．$(w,\, k) \in S_{m,\, n}$ を図示すると
\begin{align}
    (w,\, k) = \quad
    \begin{tikzpicture}[baseline={([yshift=-.5ex]current bounding box.center)}]
        \path coordinate (O)
...
\begin{mylem}[label=lem:n-fold-Map]{{$n$}-fold mapping spaceの非退化な単体}
    ...
    \begin{align}
        ...
        &\cong \Biggl\{\, (\sigma_w)_{w \in W_{m,\, n}} \in C_K^{m,\, n} \Biggm| \substack{0 \le \forall j \le n, \\ \partial^{m+1}_0 \cdots \partial^{m+j}_{j-1} \circ \partial^{m+j+1}_{m+j+1} \cdots \partial^{m+n}_{m+n}(\sigma_{w_{j}}) = x_j} \,\Biggr\} \eqqcolon M^{m,\, n}_K
    \end{align}
...
\begin{mylem}[label=lem:restrict,breakable]{制限写像の具体形}
    ...
    および
    \begin{align}
        \partial^k_{ij} &\coloneqq \underbrace{\partial^{m+2}_0 \cdots \partial^{m+i+1}_{i-1}}_{i} \circ \underbrace{\partial_{i+k}^{m+i+2} \cdots \partial_{k+j-2}^{m+j}}_{j-i-1} \circ \underbrace{\partial_{j+m+1}^{m+j+1} \cdots \partial_{m+n}^{m+n}}_{n-j} \colon K_{m+n} \lto K_{m+1}
    \end{align}
    とおいた．
\end{mylem}
...
\begin{proof}
    ...
    であるから，
    \begin{align}
        \gamma_{\omega_v} = d_{m+n}^{m+n} \circ \cdots \circ d_{j+m+1}^{m+j+1} \circ d_{k+j-2}^{m+j} \circ \cdots \circ d_{i+k}^{m+i+2} \circ d_{i-1}^{m+i+1} \circ \cdots \circ d_0^{m+2}
    \end{align}
    が言えた．
\end{proof}
