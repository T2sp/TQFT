...
\begin{myexample}[label=def:infty-Fun]{{$(\infty,\, 1)$}-圏の関手が成す {$\infty,\, 1$}-圏}
    \hyperref[def:infty-1]{$(\infty,\, 1)$-圏} $K,\, L$ を与える．
    このとき，
    % 自然変換の集合 $\Hom{\sSet} (K,\, L)$ を\hyperref[def:infty-1]{$(\infty,\, 1)$-圏}と見做すことができる．
    ...
    \end{align}
    は $(\infty,\, 1)$-圏であることが知られている~\cite[Theorem 1.5.3.7.]{Kerodon}．
\end{myexample}
...
\begin{mydef}[label=def:infty-homotopy-morphism,breakable]{1-射の間のホモトピー}
    $K$ を\hyperref[def:infty-1]{$(\infty,\, 1)$-圏}，$f,\, g \in K_1$ を\hyperref[def:infty-1]{始点と終点}が同一であるような $K$ の1-射とする．
...
\begin{mylem}[label=def:infty-homotopy-equivalence]{ホモトピックは同値関係}
    $K$ を\hyperref[def:infty-1]{$(\infty,\, 1)$-圏}，$\Hom{K}(X,\, Y) \subset K_1$ を，対象 $X,\, Y \in K_0$ をそれぞれ始点，終点に持つような1-射全体が成す集合とする．
    このとき，2項関係
    \begin{align}
...
$K$ を\hyperref[def:infty-1]{$(\infty,\, 1)$-圏}とする．$\forall X,\, Y,\, Z \in K_0$ に対して
\begin{align}
...
\end{align}
と定義する\footnote{角の図式 $(f,\, \bullet,\, g) \in K_1^{\times 2}$ を\hyperref[def:KanCplx]{\textsf{\textbf{(Kan)}}}によって埋める2-射 $\sigma \in K_2$ に対して，$g \circ f \coloneqq \partial^2_1 (\sigma)$ とおいた．この写像はwell-definedである．}．
    

\begin{mydef}[label=def:hcat-infty,breakable]{{$(\infty,\, 1)$}-圏のホモトピー圏}
    \hyperref[def:infty-1]{$(\infty,\, 1)$-圏} $K$ の\textbf{ホモトピー圏} (homotopy category) $\htpy{K}$ とは，次のように定義される $(1,\, 1)$-圏のこと：
    \begin{itemize}
...
\begin{mydef}[label=def:isom-infty]{{$(\infty,\, 1)$}-圏における対象の同型}
    \hyperref[def:infty-1]{$(\infty,\, 1)$-圏} $K$ を与える．
    \begin{itemize}
...
\begin{mydef}[label=def:hqCat]{{$\qCat$}のホモトピー圏}
    ...
    \begin{itemize}
        \item \hyperref[def:infty-1]{$(\infty,\, 1)$-圏}を対象とする
...
\begin{mydef}[label=def:equiv-infty]{{$(\infty,\, 1)$}-圏の同値}
    \hyperref[def:infty-1]{$(\infty,\, 1)$-圏の関手} $F \colon K \lto L$ が\textbf{$\bm{(\infty,\, 1)}$-圏同値}であるとは，$[F] \in \Hom{\htpy{(\qCat)}}(K,\, L)$ が同型射であることを言う．
\end{mydef}
...
\begin{myexample}[label=def:Kan]{Kan複体の圏}
    ...
    　さらに，$\Kan$ は\hyperref[def:SimpCat]{単体的豊穣圏}と見做すこともできる．
    実際，任意のKan複体 $X,\, Y \in \Obj{\Kan}$ に対して
    \begin{align}
        \Hom{\KAN}(X,\, Y) \colon \OP{\Delta} &\lto \SETS, \\
    ...
    \begin{itemize}
        \item 集合 $\Obj{\Kan}$
        \item Hom対象 $\Hom{\KAN}(X,\, Y) \in \Obj{\sSet}$
    \end{itemize}
    を持つ単体的豊穣圏 $\KAN$ が構成できた．
    特に，
    \begin{align}
...
\begin{myexample}[label=def:Spaces]{空間の成す {$(\infty,\, 1)$}-圏}
    ...
    \end{align}
    と書く．実際，\hyperref[def:SimpSet]{単体的集合} $\SPACES \in \Obj{\sSet}$ は\hyperref[def:infinity-1]{$(\infty,\, 1)$-圏}である~\cite[Proposition 5.5.1.2.]{Kerodon}
\end{myexample}
...
\begin{mydef}[label=def:op-infty]{{$(\infty,\, 1)$}-圏の逆}
    ...
    \end{align}
    のことを言う．$\OP{K}$ もまた $(\infty,\, 1)$-圏である~\cite[Proposition 1.4.2.6.]{Kerodon}．
\end{mydef}
...
\begin{mydef}[label=def:infinity-presheaf]{{$(\infty,\, 1)$}-前層}
    \begin{itemize}
        ...
        \end{align}
        のこと．
        \item $K$ 上の $\bm{(\infty,\, 1)}$\textbf{-前層が成す} $\bm{(\infty,\, 1)}$\textbf{-圏}とは，$(\infty,\, 1)$-前層全体の集合 $\Hom{\sSet} (\OP{K},\, \SPACES)$ を\exref{def:Kan}のHom対象の構成と全く同じ方法で\hyperref[def:SimpSet]{単体的集合}と見做した
        \begin{align}
            \inftyPsh{K} \coloneqq \Hom{\sSet}(\OP{K},\, \SPACES) \in \Obj{\sSet}
        \end{align}
        のこと．$\inftyPsh{K}$ は $(\infty,\, 1)$-圏である~\cite[Theorem 1.5.3.7.]{Kerodon}
