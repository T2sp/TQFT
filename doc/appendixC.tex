...
\begin{center}
    ...
    \begin{tikzcd}
    {\inftyMap{K}(x,\, y)_n} \arrow[rr] \arrow[dd] \arrow[rd] &                                                  & \{y\} \arrow[d, hook,"\sigma^n_0 \circ \cdots \sigma^0_0(y)"]     \\
    ...
    \{x\} \arrow[r, hook, "\sigma^n_0 \circ \cdots \sigma^0_0(x)"]                                     & {\Hom{\sSet}(\Delta^n \times \{0\},\, K)} \cong K_n                    &                          
    \end{tikzcd}
\end{center}
として計算される．i.e. 
\begin{align}
...
\begin{myexample}[label=ex:Map0]{射の空間の対象（0-射）}
    ...
    \begin{center}
        ...
        {\inftyMap{K}(x,\, y)_0} \arrow[rr] \arrow[dd] \arrow[rd] &                                                  & \{y\} \arrow[d, hook]     \\
                                                                  & {\Hom{\sSet}(\Delta^0 \times \Delta^1,\, K)} \cong K_1 \arrow[d, "{(\Id_{\Delta^1} \times d^1_1{}_*)^*}"] \arrow[r, "{(\Id_{\Delta^1} \times d^1_0{}_*)^*}"] & {\Hom{\sSet}(\Delta^0 \times \{1\},\, K)} \cong K_0 \\
...
\begin{myexample}[label=ex:Map2]{射の空間の2-射}
    ...
    \end{align}
    の存在が分かる．後に示す系\ref{col:product-coeq}によると，一般の $n$ の場合は次のようになる：
    \begin{align}
        \label{eq:Mapn}
        \inftyMap{K}(x,\, y)_n
        &\cong \Bigl\{\, (\sigma_1,\, \dots,\, \sigma_n) \in K_{n+1}^{\times (n+1)} \Bigm| \substack{1 \le \forall j \le n,\; \partial^{n+1}_j(\sigma_{j-1}) = \partial^{n+1}_j(\sigma_j) \\ \partial^{n+1}_0(\sigma_0) = \sigma^n_0 \circ \cdots \circ \sigma^0_0(y),\; \partial^{n+1}_{n+1} (\sigma_n) = \sigma^{n}_{0} \circ \cdots \circ \sigma^0_0(x)} \,\Bigr\} 
    \end{align}
\end{myexample}
...
\begin{proof}
    ...
    \begin{center}
        ...
        \end{tikzcd}
    \end{center}
    が成り立つ．後は $\bar{q} \colon \mathrm{Coeq}(u,\, v) \lto \Delta^m \times \Delta^n$ が\hyperref[def:nat]{自然同型}であることを示せば良い．
...
\begin{mycol}[label=col:product-coeq]{直積の公式}
    $\forall K \in \Obj{\sSet}$ に対して，写像
    \begin{align}
        \Hom{\sSet}(\Delta^m \times \Delta^n,\, K) &\lto \Bigl\{\, (\sigma_w)_{w \in W_{m,\, n}} \in \prod_{W_{m,\,n}} K_{m+n} \Bigm| \substack{\forall (w,\, k) \in S_{m,\, n},\\ \partial^{m+n}_k(\sigma_w) = \partial^{m+n}_k(\sigma_{\tau_k(w)})} \,\Bigr\}, \\
...
\begin{proof}
    命題\ref{prop:product-coeq}に\hyperref[prop:lim-colim-basic]{余極限とHomの交換}と\hyperref[lem:Yoneda]{米田の補題}を適用することで示された．
\end{proof}
