...
\begin{center}
    ...
    \end{tikzcd}
\end{center}
が成り立つから，$[g] \in G/H$ の代表元として
\begin{align}
    \exp_G\bigl(\iunit \pi_A(x)X^A\bigr) \in G
\end{align}
を選ぶことができる．この $\dim G - \dim H$ 成分場 $\pi \colon M \lto \mathrm{Lie} (G/H)$ のことを\textbf{NGボゾン場}と呼ぶ．
このとき，大域的な $\forall g \in G$ の作用に応じた場 $\pi$ の変換 $\pi \lmto \pi'$ は
\begin{align}
    [\exp_G\bigl(\iunit \pi_A(x)X^A\bigr)] \lmto [g \exp_G\bigl(\iunit \pi_A(x)X^A\bigr)] \eqqcolon [\exp_G \bigl( \iunit \pi'_A(x) X^A \bigr) ]
\end{align}
...
\begin{align}
    [\exp_G \bigl( \iunit \pi'_A(x) X^A \bigr) ] = [g \exp_G\bigl(\iunit \pi_A(x)X^A\bigr)] = [\exp_G\bigl(\iunit \pi_A(x)X^A\bigr)]
\end{align}
...
\begin{align}
    \xi(x)^{-1} \partial_\mu \xi(x) \lmto h^{-1}(g,\, \pi) \xi(x)^{-1} \partial_\mu \xi(x) h(g,\, \pi) +  \underbrace{h^{-1}(g,\, \pi) \partial_\mu h(g,\, \pi)}_{\in \mathfrak{h}}
\end{align}
...
定理\ref{thm:NG-relativistic}のLorentz共変性の条件 (1) を外すと話はややこしくなる．
...
\subsection{$0$ 次対称性のアノマリー}


% ゲージ変換 $B_p \lmto B_p + \dd{\lambda_{p-1}}$ 
\textbf{アノマリー}\footnote{訳語だと\textbf{対称性の量子的破れ}と呼ぶこともある~\cite{Fujikawa2001path}．}とは，繰り込み不変量であって，UVとIRの対称性を整合させるものである．この解釈は\textbf{ABJアノマリー} (Adler Bell Jackiw anomaly) と言うよりかは\textbf{'t Hooftアノマリー}の一般化だと考えるべきである．
% 歴史的には摂動的アノマリーが最初に見つかったが，非摂動的アノマリーを定義するには

現代的には，考えている時空 $\mfdcal{M}{D+1}$ を境界に持つ $\mfdcal{N}{D+2} \WHERE \partial \mfdcal{N}{D+2} = \mfdcal{M}{D+1}$ 上のTQFT\footnote{特に，\textbf{可逆な場の理論} (invertible field theory; IFT) になっているようなもの．}を使ってアノマリーを記述することで，RG不変性を示す．
まずLie群 $G$ で特徴づけられる $0$-form symmetryの場合を考えよう．on shellな保存カレントを $1$-form背景ゲージ場 $A_1$ に結合させる\footnote{固定されたゲージ場を運動項を持たないダイナミカルなゲージ場だと思ってカレントを線形に結合させると言うこと．}ことができる．このとき，ゲージ場はダイナミカルでない\footnote{経路積分で $A_1$ についての和を取らないと言うこと．}ので，分配関数は
\begin{align}
    Z[A_1] = \int [\dd{\Psi}]\, e^{-S[\psi,\, A_1]}
\end{align}
の形になっている．この $0$-form symmetryは，ゲージ変換 $A_1 \lmto A_1 + \dd{\lambda_0}$ の下で分配関数が不変にならないとき，アノマリーを持つと言う：
\begin{align}
    Z[A_1, + \dd{\lambda_0}] = e^{\iunit \int_{\mfdcal{M}{D+1}} \mathcal{A}[A_1,\, \lambda_0]} Z[A_1]
    % \WHERE  \mathcal{A}[A_1,\, 0]  = 0
\end{align}
この位相 $\mathcal{A}[A_1,\, \lambda_0]$ は $D+2$ 次元時空のTQFTの境界の寄与だと理解できる：
\begin{align}
...
\end{align}
この $\dd{\mathcal{A}[A_1,\, \lambda_0]}$ を $\widehat{\mathcal{A}}[A_1 + \dd{\lambda_0}] \in \Omega^{D+2}(\mfdcal{N}{D+2})$ と見做すのである．従って，元の理論の分配関数を
\begin{align}
    \widehat{Z}[A_1] \coloneqq Z[A_1] e^{\iunit \int_{\mfdcal{N}{D+2}} \widehat{\mathcal{A}}[A_1]}
\end{align}
と正則化すれば，アノマリーを相殺できる．位相 $e^{\iunit \int_{\mfdcal{N}{D+2}} \widehat{\mathcal{A}}[A_1]}$ は\textbf{SPT}と呼ばれる．$\widehat{\mathcal{A}}[A_1]$ がTQFTなので，繰り込み群の影響を受けないのである．

例えば，カイラルアノマリーをFujikawa法で計算した結果は，
\begin{align}
    \widehat{\mathcal{A}}[A_1,\, \lambda_0] = \kappa \lambda_0 \frac{F_2 \wedge F_2}{8\pi^2}
\end{align}
とすると
\begin{align}
    \mathcal{A}[A_1] = \kappa A_1 \wedge \frac{F_2 \wedge F_2}{8\pi^2}
\end{align}
になるので再現されている．
...
\begin{myexample}[label=ex:anomaly-p-formQED]{$p$-form $\LU(1)$ ゲージ理論のアノマリー}
    ...
    \end{align}
    がある．対応するトポロジカル演算子の期待値は
    \begin{align}
        \expval{\uirm{U}{(e)}_{e^{\iunit \alpha}}(\mfdcal{M}{D-p})} 
        &= \int [\dd{A_p}]\, e^{\frac{\iunit}{2g^2} \int_{\mfdcal{M}{D+1}} F_{p+1} \wedge \star F_{p+1}} e^{\iunit \alpha \int_{\mfdcal{M}{D-p}} \star \uirm{J}{(e)}} \\
        &= \int [\dd{A_p}]\, e^{\frac{\iunit}{2g^2} \int_{\mfdcal{M}{D+1}} F_{p+1} \wedge \star F_{p+1} + \frac{\iunit}{g^2} \alpha \int_{\mfdcal{M}{D+1}} \star \uirm{J}{(e)} \wedge \delta(\mfdcal{M}{D-p})} \\
        &=
    \end{align}
