...
$\Cat{C}$ の\hyperref[def:semisimple-cat]{単純対象}が成す有限集合を $\mathrm{Simp}(\Cat{C})$ と書く．
...
\end{align}
のような略記をすることがある．
さらに，補題\ref{lem:nondegen-pairing}により，有限次元 $\mathbb{K}$-ベクトル空間 $\expval{x_1,\, \dots,\, x_n}$ の基底を $\{\varphi_\alpha\}$ と書くと，その双対基底 $\{\varphi^\alpha\}$ は $\expval{x_n^*,\, \dots,\, x_1^*}$ の基底をなす．
