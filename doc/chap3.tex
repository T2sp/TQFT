...
を用いて構成される射
\begin{align}
    \sum_{\alpha \in \Lambda} p_\alpha \otimes i_\alpha \in \Hom{\Cat{C}} (x,\, a) \otimes \Hom{\Cat{C}} (a,\, x)
\end{align}
を次のような図式で表現する：
\begin{align}
    \begin{tikzpicture}[baseline={([yshift=-.5ex]current bounding box.center)}]
        ...
        +(0,2) coordinate (e)
        +(1,2) coordinate (f);
        \draw[->-=.5] (a) -- node[midway,left] {$x$} (c) node[trinode,fill=red] {};
        \draw[->-=.5] (c) -- node[midway,left] {$a$} (e);
        \draw[->-=.5] (b) -- node[midway,right] {$a$} (d) node[trinvnode,fill=red] {};
...
\begin{mylem}[label=lem:triangle-rule]{技術的な補題1}
    ...
    \begin{enumerate}
        \item \begin{align}
            \begin{tikzpicture}[baseline={([yshift=-.5ex]current bounding box.center)}]
                ...
                +(0,2) coordinate (c)
                +(0,3) coordinate (d);
                \draw[->-=.5] (a) -- node[midway,left] {$a$} (b) node[trinvnode,fill=red] {};
                \draw[->-=.5] (b) -- node[midway,left] {$x$} (c) node[trinode,fill=red] {};
        ...
        \item \begin{align}
            ...
            \begin{tikzpicture}[baseline={([yshift=-.5ex]current bounding box.center)}]
                ...
                +(0,2) coordinate (c);
                +(0,3) coordinate (d);
                \draw[->-=.5] (a) -- node[midway,left] {$x$} (b) node[trinode,fill=red] {};
                \draw[->-=.5] (b) -- node[midway,left] {$a$} (c) node[trinvnode,fill=red] {};
