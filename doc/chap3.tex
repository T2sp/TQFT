...
\begin{proof}
    ...
    \begin{itemize}
        \item $\mathsf{E}(P^{(1)})$ に新しく1つの無向辺 $e$ が，
        \item $\mathrm{Reg}(P)$ に新しく1つの領域 $R_5$ が
    \end{itemize}
    追加される．彩色
    \begin{align}
    ...
    図中の頂点から構成される\hyperref[def:linkedG]{絡みグラフ}のうち，新しく追加された辺 $e$ と関連する部分のみであり，
    \begin{align}
    ...
    \begin{align}
        ...
        &= \frac{\dim \bigl( \Gamma(\textcolor{blue}{R_3}) \bigr)}{\dim \bigl( \Gamma(R_1) \bigr)^2 \dim \bigl( \Gamma(\textcolor{red}{R_2}) \bigr) } \prod_{S \in \mathrm{Reg}(P)} \dim \bigl( \Gamma(S) \bigr)^{\chi(S)}, \\
