...
標準的射影 $p \colon G \lto G/H$ について $T_{1_G} p \colon \mathfrak{g} \lto \mathrm{Lie}(G/H)$  は全射であるが，$X \in \mathfrak{h} \IFF T_{1_G}p (X) = T_{0}(p \circ \exp_G) \left( \eval{\dv{}{t}}_{t=0} \right)=0$ なので $\mathfrak{h} \subset \Ker T_{1_G} p$ がわかり，次元を考えることで $\Ker T_{1_G} p = \mathfrak{h}$ が言える．よって準同型定理から $\mathfrak{g}/\mathfrak{h} \cong \mathrm{Lie}(G/H)$ である．
...
\end{align}
を選ぶことができる．この $\dim G - \dim H$ 成分場 $\pi \colon M \lto \mathbb{R}^{\dim G - \dim H}$ のことを\textbf{NGボゾン場}と呼ぶ．
...
\begin{align}
    \xi(x)^{-1} \partial_\mu \xi(x) = \frac{1-e^{-\iunit \ad(\pi)}}{\iunit \ad(\pi)} (\partial_\mu \pi) = \partial_\mu \pi - \frac{\iunit}{2!} \comm{\pi}{\partial_\mu \pi} + \order{\pi^2 \partial_\mu \pi}
\end{align}
なので
\begin{align}
...
最後に，これまで大域的対称性だと思っていた群 $G$ を局所的対称性にすることでゲージ場を導入する（\textbf{ゲージ化}）．そのためにはMaurer-Cartan形式の $\partial_\mu$ を共変微分に置き換えれば良く，NGBとゲージ場との結合はそれで決まる．
