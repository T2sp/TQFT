...
系がLie群 $G$ で特徴付けられる $0$-form symmetry を持つ場合を考えよう．このとき，無限小な対称性変換はLie代数 $\mathfrak{g}$ の自然基底 $\partial_A \in \mathfrak{g}$ を用いて\footnote{苦肉の策だが，Lie代数の添字を $A,\, B,\, C,\, \dots$ とする．さらに，$\mathfrak{g} = T_{1_G}G$ なので厳密には $\eval{\partial_A}_{1_G}$ と書くべきだがこれを $\partial_A$ と略記する．} 
$\mathcal{T}_{\varepsilon^A \partial_A}$ と書けるので\footnote{場がベクトル束の切断として定式化されている場合，これは厳密には $G$ の（ファイバーに作用する）表現の微分表現を考えていることになる．今の場合，物理の慣習に従って $G$ は線型Lie群で，かつファイバーへの作用は基本表現だと思っているのでさほど問題にはならない．}，Noetherの定理より on shell な保存則\eqref{eq:conservation-Noether-df}を充たす $\dim G$ 個のNoetherカレント\eqref{eq:Noether-current} $j_A\quad (A = 1,\, \dots,\, \dim G)$ が存在し，
...
そこで，代わりにある局所演算子（i.e. compactly supportedな演算子）$\hat{\phi}(x)$ が存在して
\begin{align}
    \label{def:0form-order-param}
    \mel{0}{\comm{\iunit \hat{Q}_A}{\hat{\phi}(x)}}{0} \neq 0
\end{align}
...
\begin{mytheo}[label=thm:NG-relativistic]{Nambu-Goldstone（相対論的）}
    ...
    \begin{enumerate}
        ...
        \item あるNoetherカレント $j_A$ に対応する $0$-form symmetry が自発的に破れている
    \end{enumerate}
    とする．このとき，系は (2) を充たす $j_A$ とちょうど同じ数だけ線形分散を持つ零質量の独立な励起（\textbf{NGモード}）を持つ．
\end{mytheo}

\begin{proof}
    系は場 $\varphi \in \Gamma(E)$ からなるとする．
    この系の作用を $S[\varphi]$ と書く．Green関数の生成汎函数を，$J \in \Gamma(E)$ に関する汎函数として
    \begin{align}
        Z[J] \coloneqq e^{\iunit W[J]} \coloneqq 
    \end{align}
