...
この章で登場する多様体は特に断らない限り常に $C^\infty$ 多様体である．
また，体 $\mathbb{K}$ と言ったら $\mathbb{K} = \mathbb{R},\, \mathbb{C},\, \mathbb{H}$ のいずれかを指すことにしよう．

また，グラフ $G$ の頂点集合を $\mathsf{V}(G)$，辺集合を $\mathsf{E}(G)$ と書く．
    ...
    \begin{description}
        ...
        \begin{center}
            \begin{tikzcd}[row sep=large, column sep=large]
                ...
                &(Y \otimes Z) \otimes X &Y \otimes (Z \otimes X) \ar[l, "a_{Y,\, Z,\, X}^{-1}"] &Y \otimes (X \otimes Z) \ar[l, "1_Y \otimes b_{X,\, Z}"]
            \end{tikzcd}
        \end{center}
...
\begin{mydef}[label=def:closed-monoidal,breakable]{閉圏}
    ...
    \begin{itemize}
        ...
        \begin{align}
            \multimap \colon \mathcal{C}^{\mathrm{op}} \times \mathcal{C} \lto \Cat{C}
        \end{align}
        と，\textbf{currying}と呼ばれる自然同型
        \begin{align}
            \Familyset[\big]{c_{X,\, Y,\, Z} \colon \Hom{\Cat{C}}(X \otimes Y,\, Z) \xrightarrow{\cong} \Hom{\Cat{C}}(X,\, Y \multimap Z)}{X,\, Y,\, Z \in \Obj{\mathcal{C}}}
        \end{align}
        の2つが存在することを言う．
        \item $\mathcal{C}$ が\textbf{右に閉じている} (right closed) とは，\textbf{internal hom functor}と呼ばれる関手と，\textbf{currying}と呼ばれる自然同型
        \begin{align}
            \Familyset[\big]{c_{X,\, Y,\, Z} \colon \Hom{\Cat{C}}(X \otimes Y,\, Z) \xrightarrow{\cong} \Hom{\Cat{C}}(Y,\, X \multimap Z)}{X,\, Y,\, Z \in \Obj{\mathcal{C}}}
        \end{align}
        の2つが存在することを言う．
    \end{itemize}
\end{mydef}
...
\begin{myaxiom}[label=ax:Atiyah-TQFT,breakable]{Atiyahの公理系（若干簡略版）}
    ...
    % \hyperref[ex:Bord]{コボルディズム圏}からある\hyperref[def:braided-monoidal]{対称モノイダル圏} $\mathcal{D}$ への\hyperref[def:monidal-functor]{strict monoidal functor}\footnote{strong monoidal functorとする場合もある（例えば\url{https://ncatlab.org/nlab/show/cobordism}）ようだが，原論文~\cite{Atiyah1988tqft}ではstrict monoidal functorになっていた．}
    % \begin{align}
...
このとき，大域的な $g \in G$ の作用に応じた場 $\pi$ の変換 $\pi \lmto \pi'$ は
\begin{align}
...
NGボゾン場の項を含むラグランジアンは，$G$-不変でなくてはいけない．そのような項を構成するために\hyperref[def:Maurer-Cartan]{Maurer-Cartan形式} $\theta \in \Omega^1(G;\, \mathfrak{g})$ を使う．これを $C^\infty$ 写像 $\xi \coloneqq \exp_G \bigl( \iunit \pi(\mhyphen) \bigr) \colon \mathcal{M} \lto G$ で引き戻すことで左不変な $\xi^* \theta \in \Omega^1(\mathcal{M};\, \mathfrak{g})$ を得る．
...
次に物質場との結合を記述する方法を考える．物質場 $\psi$ の値域は破れていない対称性 $H$ の既約表現 $\rho_0 \colon H \lto \LGL(\mathcal{H})$ の表現空間 $\mathcal{H}$ だと考えて良い．$\forall g \in G$ による変換則は\eqref{eq:NGB-transform}の $h(g,\, \pi) \in H$ を使って
\begin{align}
    \label{eq:NGB-matter}
...
最後に，これまで大域的対称性だと思っていた群 $G$ の作用を局所的対称性にするために背景ゲージ場を導入する．そのためにはMaurer-Cartan形式の $\partial_\mu$ を共変微分に置き換えれば良く，NGBと背景ゲージ場との結合はそれで決まる．
あからさまには
\begin{align}
...
\begin{mytheoph}[label=thm:pform-CMW]{高次対称性についてのColeman-Mermin-Wagnerの定理}
...
ものとして定義する．
\begin{itemize}
    \item $\forall X \in \Obj{\htpy{\MFLD}}$ に対して集合
    \begin{align}
        \mathrm{Prin}_G(X) &\coloneqq \bigl\{\, X\; \text{上の\hyperref[def.PFD]{主束}の\hyperref[def.bundlemap]{束同型}類} \,\bigr\}
    \end{align}
    を，
    \item $\forall X,\, Y \in \Obj{\htpy{\MFLD}}$ の $\forall [f] \in \Hom{\htpy{\MFLD}} (X,\, Y)$ に対して写像
    \begin{align}
...
対応づける対応
\begin{align}
    \mathrm{Prin}_G \colon \htpy{\MFLD} \lto \SETS
\end{align}
は反変関手である~\cite[p.53, 10.1 Theorem]{Husemoller1994}．

\begin{mydef}[label=def:universal-bundle]{普遍束}
    主束 $G \hookrightarrow P \xrightarrow{\pi} B$ が $G$ の\textbf{普遍束} (universal bundle) であるとは，$\forall X \in \Obj{\htpy{\MFLD}}$ について定まる写像
    \begin{align}
...
\begin{myprop}[label=prop:universal-basic]{普遍束の特徴付け}
    ...
    \begin{description}
        \item[\textbf{(univ-1)}]  $\forall \textcolor{red}{X} \in \Obj{\htpy{\MFLD}}$ および $X$ を底空間にもつ任意の主束 $G \hookrightarrow \textcolor{red}{P} \xrightarrow{\pi} \textcolor{red}{X}$ に対して，ある連続写像 $f \colon \textcolor{red}{X} \lto BG$ が存在して
        $\textcolor{red}{P}$ と\hyperref[def:pullback-bundle]{引き戻し束} $f^*(EG)$ が束同型になる．
        \item[\textbf{(univ-2)}]  $\forall \textcolor{red}{X} \in \Obj{\htpy{\MFLD}}$ および任意の連続写像 $f,\, g \colon \textcolor{red}{X} \lto BG$ に対して，
...
\begin{proof}
    ...
    \begin{description}
        ...
        　最後に $\irm{EG}{M}$ が第2可算なHausdorff空間であることを示さないといけないが，技術的なのでここでは省略する．

        \item[\textbf{普遍束であること}]　
        ~\cite[p.55, 11.2 Theorem]{Husemoller1994}
...
もしくは，有限群の場合
\footnote{
    位相群の場合であっても，$G$ の\hyperref[def:simplicial-top]{特異単体}は\hyperref[def:KanCplx]{Kan複体}なので，この方法で対象が1点の $\Kan$ 豊穣圏が得られる．これの\hyperref[def:nerve-hc]{homotopy coherent nerve}をとったものの幾何学的実現により位相空間としての $BG$ が (up to ホモトピーで) 正しく構成される．
}は $G$ を対象が1点でHom集合が $G$ であるような圏
\begin{align}
    \deloop{G} \coloneqq
    % https://tikzcd.yichuanshen.de/#N4Igdg9gJgpgziAXAbVABwnAlgFyxMJZABgBpiBdUkANwEMAbAVxiRAB12AjJhhmHCAC+VEDCgBzeEVAAzAE4QAtkjIgcEVdQYQIaIgEYAHGVmM4MUQzpcYDAAqZc+QohDysEgBaDq9ZqxuAOLCFEJAA
\begin{tikzcd}
\bullet \arrow["G"', loop, distance=2em, in=125, out=55]
\end{tikzcd}
\end{align}
と見做したとき，それの\hyperref[def:nerve]{脈体}を取って得られる\hyperref[def:infinity-1]{$(\infty,\, 1)$-圏} $\Ner(\deloop{G})$ の\hyperref[def:geometric-realization]{幾何学的実現}は $K(G,\, 1)$ になる~\cite[\href{https://kerodon.net/tag/0038}{Tag 0038}]{kerodon}．
...
\begin{mytheo}[label=thm:group-cohomology-top]{群コホモロジー（位相空間論的）}
    任意の有限群 $G$ および\hyperref[ax:G-mod]{$G$ 加群} $M$ に対して
    \begin{align}
...
\end{itemize}
によって定義する\footnote{正確には，圏 $\PATH{\mfdcal{M}{d}}$ とはこれのHom集合を\textbf{thin homotopy}と呼ばれる同値関係で割ることで得られる\hyperref[def:groupoid]{groupoid}のことであり，\textbf{path groupoid}と呼ぶ（\url{https://ncatlab.org/nlab/show/path+groupoid}）．}．
...
\begin{myprop}[label=prop:connection-functor]{関手としての接続}
    ...
    \begin{enumerate}
        ...
        \item $\forall x,\, y \in \Obj{\PATH{\mfdcal{M}{d}}},\; \forall \sigma \in \Hom{\PATH{\mfdcal{M}{d}}} (x,\, y)$ に対して\footnote{$\Phi_u(\sigma) \in G$ はループ $\sigma$ の\hyperref[prop:holonomy]{ホロノミー}．} $\Gamma_\omega(\sigma) \coloneqq \bigl( u \lmto u \btl \Phi_u(\sigma)\bigr)$
    \end{enumerate}
    と定義される対応
    \begin{align}
        \Gamma_\omega \colon \PATH{\mfdcal{M}{d}} \lto \VEC{\mathbb{K}}
    \end{align}
    は関手である．
    ...
    逆に，関手
    \begin{align}
        \Gamma \colon \PATH{\mfdcal{M}{d}} \lto \VEC{\mathbb{K}}
    \end{align}
...
水平持ち上げの一意性から $\widetilde{(\sigma' * \sigma)} (1) = s(z) \btl g_\sigma g_{\sigma'}$ が成り立つ．従って，$P$ が自明束ならば命題\ref{prop:connection-functor}の関手 $\Gamma_\omega \colon \PATH{\mfdcal{M}{d}} \lto \VEC{\mathbb{K}}$ は関手
\begin{align}
    \label{eq:connection-Wilsonline}
    \Gamma_\omega \colon \PATH{\mfdcal{M}{d}} \lto \deloop{G}
\end{align}
と見做せる．ただし $\deloop{G}$ は
\begin{itemize}
    \label{def:delooping-groupoid}
    \item 一点のみを対象に持つ：$\Obj{\deloop{G}} = \{*\}$
    \item $G$ の元を射に持つ．射の合成は $G$ の積として定める
\end{itemize}
ことで構成される圏である\footnote{この圏は\hyperref[def:groupoid]{groupoid}を成し，群 $G$ の\textbf{delooping groupoid}と呼ばれる．}．
\hyperref[def:connection]{接続} $\omega \in \Omega^1(P;\, \mathfrak{g})$ と関手\eqref{eq:connection-Wilsonline}の同一視を物理語に翻訳すると，\hyperref[def:gauge-field]{ゲージ場} $A$ そのものと，ゲージ場の\textbf{Wilson line}
\begin{align}
    \mathcal{P} \exp \left( - \int_\gamma A \right) \WHERE \gamma \in \Hom{\PATH{\mfdcal{M}{d}}} \bigl( \gamma(0),\, \gamma(1) \bigr) 
\end{align}
を同一視することに他ならない．
...
i.e. 変換関数 $g_{12} \colon M \lto G$ は，自然変換
\begin{center}
    \begin{tikzcd}
...
Lie群 $G$ がコンパクトかつ単連結ならば，$\mfdcal{M}{3}$ 上の任意の主束は自明束と同型になり\footnote{詳細は\url{https://math.stackexchange.com/questions/370313/every-principal-g-bundle-over-a-surface-is-trivial-if-g-is-compact-and-simpl}を参照．}，Chern-Simons作用
\begin{align}
    S[A] \coloneqq \frac{k}{8\pi^2} \int_{\mfdcal{M}{3}} \Tr \left( A\wedge A + \frac{2}{3} A \wedge A \wedge A \right) 
\end{align}
は $\mfdcal{M}{3}$ 上大域的にwell-definedである．
しかし，$G$ が一般のコンパクトLie群である場合にはそう上手くいかない．

この問題を解決する方法としては，$\mfdcal{M}{3}$ を境界に持つ4次元多様体 $\mfdcal{N}{4}$ を持ってきて，
\begin{align}
...
しかし，この構成が可能であるためには任意の $\mfdcal{M}{3}$ 上の主束 $G \hookrightarrow P \xrightarrow{\pi} \mfdcal{M}{3}$ に対して，$\mfdcal{N}{4}$ 上の主束 $G \hookrightarrow P' \xrightarrow{\pi'} \mfdcal{N}{4}$ であって，$\partial \mfdcal{N}{4} = \mfdcal{M}{3}$ への制限が $P'|_{\partial \mfdcal{N}{4}} = P$ を充たすようなものが存在しなくてはいけない．
...
この不定性を排除するためには，$\Omega(F)$ が特性類なので自然な単射 $H^4(BG,\, \mathbb{Z}) \hookrightarrow H^4(BG;\, \mathbb{R})$ を使って $\Omega(F) \cong \rho ([\omega])$ と書けるような $[\omega] \in H^4(BG,\, \mathbb{Z})$ が存在することを使い\footnote{$\Omega(F) \in \dR{4}{\mfdcal{M}{3}} \cong \dR{4}{BG} \cong H^4(BG;\, \mathbb{R})$ である．}
\begin{align}
...
もしくは，\textbf{Cheegar Simons differential character}~\cite{CheegerSimons1985differential}, ~\cite{SimonsSullivan2007diff} $\alpha \in \widehat{H^3} \bigl(BG,\, \mathbb{R}/\mathbb{Z}\bigr)$ を使って
\begin{align}
...
\end{align}
であることが分かった．
% ところで，\hyperref[prop:universal-basic]{普遍束の定義}から，$\mfdcal{M}{d}$ 上の主束の同型類全体の集合 $\mathrm{Prin}_G (\mfdcal{M}{d})$ とホモトピー集合 $\comm{\mfdcal{M}{d}}{BG}$ は一対一対応するのだった．
...
群コホモロジーの表式を使って分配関数を計算するには，まず $\mfdcal{M}{d}$ の三角形分割 $\abs{\mathcal{T}} \xrightarrow{\approx} \mfdcal{M}{d}$ をとる．
簡単のため $d=3$ とする．
$\mfdcal{M}{3}$ は連結だとしていたので，$BG$ の基点 $\mathrm{pt} \in BG$ を１つ固定することで，\hyperref[prop:universal-basic]{分類写像} $\gamma \colon \mfdcal{M}{3} \lto BG$ をホモトピックに変形して，任意の0単体 $\sigma_0 \in \mathcal{T}$ を$\gamma[\sigma_0] = \mathrm{pt} \in BG$ に写すようにできる．
すると，任意の1単体 $\sigma_1 \in \mathcal{T}$ は分類写像 $\gamma$ のホモトピー類によって $[\gamma \circ \sigma_1] \in \pi_1(BG,\, \mathrm{pt})$ と対応付くが，命題\ref{prop:classifying-space-Kpi1}より $\pi_1(BG,\, \mathrm{pt}) \cong G$ であるから，結局分類写像 $\gamma$ によって $\forall \sigma_1 \in \mathcal{T}$ と何かしらの $g_{\sigma_1} \in G$ が対応付くことになる．
この $g_{\sigma_1}$ は格子ゲージ理論におけるリンク変数（i.e. ゲージ場）と見做せる．
しかるに，$G$ が有限群であることから主束の接続は平坦でなくてはいけない．そのため，もし3つの1-単体 $\sigma^{(1)}_1,\, \sigma^{(2)}_1,\, \sigma^{(3)}_1 \in \mathcal{T}$ がある2単体 $\sigma_2$ について $\partial(\sigma_2) = \sigma^{(1)}_1 \cup \sigma^{(2)}_1 \cup \sigma^{(3)}_1$ をみたすならば
\begin{align}
    \label{eq:cocycle-discrete}
    g_{\sigma_1^{(1)}} g_{\sigma_1^{(2)}} g_{\sigma_1^{(3)}} = \gamma(\sigma^{(1)}_1 \cup \sigma^{(2)}_1 \cup \sigma^{(3)}_1) = 1
\end{align}
にならなくてはいけない．
このとき任意の3単体 $\sigma_3 \in \mathcal{T}$ に対して $\alpha \in \coGrp{3}{G,\, \LU(1)}$ は $W(\sigma_3) \in \LU(1)$ を対応づけることができる．つまり，任意の3単体 $\sigma^{(i)}_4$ を四面体と見做したとき，その6つある辺のうち3つが $\gamma$ によって $g_i,\, h_i,\, k_i \in G$ と同定されていれば\eqref{eq:cocycle-discrete}から残りの3辺も決定できるということであり，$W(\sigma_3^{(i)}) \coloneqq \alpha (g_i,\, h_i,\, k_i) \in \LU(1)$ として定まる．
...
\end{align}
と同値である．今，$\Familyset[\big]{\Lambda_{\alpha_0\alpha_1}}{(\alpha_0,\, \alpha_1) \in \Lambda^2} \in \Cech{1}{\mathcal{U};\, \Omega_{\mfdcal{M}{d}}^0}$ と見做そう．
...
特に $\irm{\mathbb{Z}(2)}{D}^1$ の元だと見做すとこれはコサイクルである．
...
よって
\begin{align}
    (\mathcal{U},\, P,\, \{A_\alpha\}) \overset{1:1}{\longleftrightarrow} \DB{2}{\mathcal{U};\, \irm{\mathbb{Z}(2)}{D}}
\end{align}
なる一対一対応があることが分かった．
...
\begin{align}
    ...
    (\mathcal{U},\, P^{(p)},\, \{A^{(p)}_\alpha\}) \overset{1:1}{\longleftrightarrow} \DB{p+1}{\mathcal{U};\, \irm{\mathbb{Z}(p+1)}{D}}
\end{align}
の一対一対応がある．逆に，\eqref{predef:pform-gauge-field}を $p$-formゲージ場の定義として採用することにしよう．
...
時空を $\mfdcal{M}{d}$ と書く．これまでは高次のゲージ場の配位空間\footnote{物理では\textbf{モジュライ空間} (moduli space) とも言う．}について全く触れずに計算を進めてきた．しかるに，これらの計算は実質的に $\LU(1)$ ゲージ場についての経路積分になっており，ill-definedである．
% 特に平坦接続の拘束条件がどの程度厳密に課されているのかには疑問が残る．
...
この節では，~\cite{alfonsi2023higher}を参考に高次のゲージ場の配位空間の構造をできるだけ正確に記述することを試みる．
...
時空 $\mfdcal{M}{d}$ の三角形分割を与える\hyperref[def:SimpSet]{単体的集合} $T \in \Obj{\sSet}$ をとり，勝手な\hyperref[def:SimpSet]{$d$-単体} $\sigma \in T_d$ を1つ固定する．$\sigma$ はちょうど $d+1$ 個の $0$-単体 $\{0,\, 1,\, \dots,\, d\}$ からなる．
...
\begin{mytheo}[label=thm:inftyGrpd]{}
    単体的集合 $X^{(\textcolor{red}{p})} \in \Obj{\sSet}$ は\hyperref[def:infinity-1]{$\infty$-groupoid}である．
\end{mytheo}

\begin{proof}
    $\forall n \ge 0$ および $0 \le \forall i \le n$ を1つ固定する．勝手な\hyperref[def:SimpSet]{圏 $\sSet$}の射
    \begin{align}
        f \colon \Lambda^n_i \lto X^{(\textcolor{red}{p})}
    \end{align}
    を1つ固定する．このとき，\hyperref[def:SimpSet]{圏 $\sSet$ }における可換図式
    \begin{center}
        \begin{tikzcd}[row sep=large, column sep=large]
...
\begin{myexample}[label=ex:CGa]{圏 $\VecG{G}^\alpha$ とアノマリー}
    $\Cat{C} = \VecG{G}^\alpha$ とする．このとき，$\alpha \in \irm{H}{Grp}^3 \bigl( G;\, \LU(1) \bigr)$ は\hyperref[def:Dijkgraaf-Witten]{Dijkgraaf-Witten}理論を定める．
    i.e. Dijkgraaf-Wittem理論を\hyperref[hypo:anomaly-inflow]{アノマリー流入}に持つ't Hooftアノマリーを1つ指定している．
    % \exref{def:CG-functor}により
...
\begin{myexample}[label=ex:CGa]{圏 $\VecG{G}^\alpha$ と $G$-SPTのスタック}
    ...
    これは2次元の理論 $T$ に2次元のボゾニック $G$-SPTを\hyperref[def:stacking]{スタック}する操作に他ならない．
\end{myexample}
...
\begin{mydefph}[label=def:gauging]{ゲージ化}
    ...
    % \begin{itemize}
    %     \item 
    %     \item $\mfd{M}{2}$ の\underline{十分細かい}三角形分割\footnote{より正確には，Poincar\'{e}双対をとることで $K$ を再現するような，trivalent junctionからなるメッシュのことを指す．} $K \in \Obj{\sSet}$
    % \end{itemize}
    ゲージ化された理論のことを $\bm{T/A}$ と書く．
\end{mydefph}

定義\ref{def:gauging}を解読する．
まず $(1+1)$-次元時空 $\mfd{M}{2}$ の任意の三角形分割 $K \in \Obj{\sSet}$ 
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}
        ...
        +(1,1) coordinate (y_2)
        +(1,-1) coordinate (y_1)
        coordinate[bullet,red] (g_1) at (barycentric cs:x_1=1,x_2=1,y_1=1)
        coordinate[bullet,red] (g_2) at (barycentric cs:x_1=1,x_2=1,y_2=1)
        ;
        \draw (x_1) -- (y_1) -- (x_2) -- (y_2) -- cycle;
        \draw (x_1) -- (x_2);   
        \draw[red,thick] (g_1.center) -- (g_2.center);
        \draw[red,thick] (g_1.center) -- +(-45:1);
        \draw[red,thick] (g_1.center) -- +(-135:1);
        \draw[red,thick] (g_2.center) -- +(45:1);
        \draw[red,thick] (g_2.center) -- +(135:1);
    \end{tikzpicture}
...
\begin{proof}
    $\mathbb{F}_A (K)$ が\hyperref[def:Pachner]{双星状移動}に関して不変であることを示せば良い．
    2次元\hyperref[def:PLmfd]{PL多様体}の双星状移動は以下の2つである：
    \begin{figure}[H]
        \centering
        \begin{subfigure}{0.4\columnwidth}
            ...
            \begin{tikzpicture}
                \path coordinate (x_1)
                    ...
                    +(1,1) coordinate (z_2)
                    +(0,1) coordinate (w_2)
                    coordinate[bullet,red] (v_1) at (barycentric cs:x_1=1,y_1=1,w_1=1)
                    coordinate[bullet,red] (v_2) at (barycentric cs:z_1=1,y_1=1,w_1=1)
                    coordinate[bullet,red] (u_1) at (barycentric cs:x_2=1,y_2=1,z_2=1)
                    coordinate[bullet,red] (u_2) at (barycentric cs:x_2=1,w_2=1,z_2=1)
                ;
                \draw (x_1) -- (y_1) -- (z_1) -- (w_1) --cycle;
                \draw (y_1) -- (w_1);
                \node at (barycentric cs:y_1=1,w_2=1) {$\lmto$};
                \draw (x_2) -- (y_2) -- (z_2) -- (w_2) --cycle;
                \draw (x_2) -- (z_2);
                \draw[red,thick] (v_1.center) -- (v_2.center);
                \draw[red,thick] (v_1.center) -- ++(180:1);
                \draw[red,thick] (v_1.center) -- ++(-90:1);
                \draw[red,thick] (v_2.center) -- ++(90:1);
                \draw[red,thick] (v_2.center) -- ++(0:1);
                \draw[red,thick] (u_1.center) -- (u_2.center);
                \draw[red,thick] (u_1.center) -- ++(0:1);
                \draw[red,thick] (u_1.center) -- ++(-90:1);
                \draw[red,thick] (u_2.center) -- ++(90:1);
                \draw[red,thick] (u_2.center) -- ++(180:1);
            \end{tikzpicture}
            \caption{2-4型のPachner移動}
            \label{fig:type2-4Pachner}
        \end{subfigure}
        \hspace{5mm}
        \begin{subfigure}{0.4\columnwidth}
            \centering
            \begin{tikzpicture}
                \path coordinate (g_1)
                    +(-150:1) coordinate (x_1)
                    +(-30:1) coordinate (y_1)
                    +(90:1) coordinate (z_1)
                    ++(3,0) coordinate (g_2)
                    +(-150:1) coordinate (x_2)
                    +(-30:1) coordinate (y_2)
                    +(90:1) coordinate (z_2)
                    coordinate[bullet,red] (v_1) at (barycentric cs:x_1=1,y_1=1,g_1=1)
                    coordinate[bullet,red] (v_2) at (barycentric cs:z_1=1,y_1=1,g_1=1)
                    coordinate[bullet,red] (v_3) at (barycentric cs:x_1=1,z_1=1,g_1=1)
                    coordinate[bullet,red] (u) at (barycentric cs:x_2=1,y_2=1,z_2=1)
                ;
                \foreach \i in {1,2} {
                    \draw (x_\i) -- (y_\i) -- (z_\i) -- cycle;
                }
                \foreach \v in {x,y,z} {
                    \draw (g_1) -- (\v_1);
                }
                \node at (barycentric cs:g_1=1,g_2=1) {$\lmto$};
                \draw[red,thick] (v_1.center) -- (v_2.center) -- (v_3.center) -- cycle;
                \foreach \i in {1,...,3} {
                    \draw[red,thick] (v_\i.center) -- +({150+120*\i}:.5);
                }
                \foreach \q in {30,150,-90} {
                    \draw[red,thick] (g_2) -- +(\q:1);
                }
            \end{tikzpicture}
            \caption{1-3型のPachner移動}
            \label{fig:type1-3Pachner}
        \end{subfigure}
        \caption{2次元PL多様体の双星状移動 (Pachner移動)}
        \label{fig:Pachner2d}
    \end{figure}%
    \hyperref[def:Frobenius-alg]{Frobenius代数}の定義\textsf{\textbf{(Frob-2)}}より，\hyperref[fig:type2-4Pachner]{2-4型の双星状移動}の下で $\mathbb{F}_A (K)$ は不変である．
    \hyperref[fig:type1-3Pachner]{1-3型の双星状移動}に関しては，
    \begin{align}
        \begin{tikzpicture}[baseline={([yshift=-.5ex]current bounding box.center)}]
            \path coordinate (g)
                +(-90:1) coordinate[bullet,label=below left:$\Delta$] (x)
                +(30:1) coordinate[bullet,label=below:$\mu$] (y)
                +(150:1) coordinate[bullet,label=below:$\mu$] (z)
            ;
            \coordinate (a) at ($0.5*(x) + 0.5*(y)$);
            \coordinate (b) at ($0.5*(y) + 0.5*(z)$);
            \coordinate (c) at ($(z) + (150:.5)$);
            % \draw[dashed] (O) circle[radius=1.3];
            \draw[->-=.5] (x) -- (y);
            \draw[-<-=.5] (y) -- (z);
            \draw[-<-=.5] (z) -- (x);
            \draw[-<-=.5] (x) -- +(-90:1);
...
従来のゲージ化の定義との対応は，次の\exref{ex:gauging-VecG}によって分かる．
逆に言うと，定義\ref{def:gauging}は\exref{ex:gauging-VecG}を一般化したものである．

\begin{myexample}[label=ex:gaiging-VecG]{$\VEC{G}$ におけるゲージ化}
$G$ を可換とは限らない有限群とし，\exref{def:CG}で定義したユニタリフュージョン圏 $\Cat{C} = \VEC{G}$ を考える．
...
\begin{mypropph}[label=prop:domainwall-gauged]{half-space gauging}
    時空多様体 $M$ が1次元部分多様体 $D$ によって2つの領域 $M_1,\, M_2$ に分割されているとする．左側の $M_1$ には理論 $T$ を，右側の $M_2$ には\hyperref[def:gauging]{ゲージ化}された理論 $T/\textcolor{red}{A}$ を置く（図\ref{fig:half-spage-gauging}）\footnote{この設定を\textbf{half-space gauging}と呼ぶ．}．
    
    このとき，ドメインウォール $D$ が成す圏は $\MODCR{\Cat{C}}{\textcolor{red}{A}}$ であり，\hyperref[def:modulecat]{$(\Cat{C},\, \BIMODC{\Cat{C}}{\textcolor{red}{A}}{\textcolor{red}{A}})$-両側加群圏}の構造を持つ．
    \begin{figure}[H]
        \centering
        \begin{tikzpicture}
            \pgfmathsetmacro{\fac}{0.3}
            \pgfmathsetmacro{\dx}{cos(60)}
            \pgfmathsetmacro{\dy}{sin(60)}
            \pgfmathsetmacro{\dr}{1/sqrt(3)}
        
            \path coordinate (d)
                ++ (0,4) coordinate[label=above:$D$] (u)
            ;
            \draw[thick] (d) -- (u);
            \begin{scope}[on background layer]
                \fill[white] (d) rectangle (u -| -4,0);
                \fill[white] (d) rectangle (u -| 4,0);
                \clip (d) rectangle (u -| 4,0);
                \foreach \x in {0,1,...,15}
                    \foreach \y in {0,1,...,10}
                {
                    \foreach \a in {30,150,-90} {
                        \draw[red] ($\fac*(d) + \fac*(-1,-3*\dr) + \fac*(\x, 3*\dr*\y)$) -- +(\a:\fac*\dr);
                    }
                }
                \foreach \x in {0,1,...,15}
                    \foreach \y in {0,1,...,10}
                {
                    \foreach \a in {30,150,-90} {
                        \draw[red] ($\fac*(d) + \fac*(-\dr*\dy,\dr*\dx) + \fac*(-1,-2*\dr) + \fac*(\x, 3*\dr*\y)$) -- +(\a:\fac*\dr);
                    }
                }
            \end{scope}
            \node[fill=white] at ($(u) + (-2,-0.2)$) {$T$};
            \node[fill=white] at ($(u) + (2,-0.2)$) {$T/\textcolor{red}{A}$};
        \end{tikzpicture}
        \caption{half-space gauging．ドメインウォール $D$ には右から $\textcolor{red}{A}$ が作用するので右 $\textcolor{red}{A}$-加群である．さらに，左からは $T$ のトポロジカル演算子が，右からは $T/\textcolor{red}{A}$ のトポロジカル演算子が作用するため，$(\Cat{C},\, \BIMODC{\Cat{C}}{\textcolor{red}{A}}{\textcolor{red}{A}})$-両側加群でもある．}
        \label{fig:half-spage-gauging}
    \end{figure}%
\end{mypropph}
...
\begin{myexample}[label=ex:gaiging-RepG]{$\REP{G}$ のゲージ化}
    $G$ を有限群とする．$\REP{G}$ の\hyperref[def:sym-Frobenius]{対象Frobenius代数}として $G$ の\textbf{正則表現} (regular representation)
    \begin{align}
    ...
    \end{align}
    である．$A$ による $\REP{G}$ のゲージ化は $\VEC{G}$ になり，\exref{ex:gauging-VecG}の逆操作を与える~\cite{BhardwajTachikawa2017gauging2d}．
...
\begin{mytheo}[label=thm:modulecat]{加群圏の特徴付け}
    \begin{itemize}
        \item \hyperref[def:finite-abcat]{有限}な\hyperref[def:tensorfusion-cat]{多重テンソル圏} $(\Cat{C},\, \otimes,\, 1,\, a,\, l,\, r,\, \lev,\, \lcoev,\, \rev,\, \rcoev)$
        \item \hyperref[def:modulecat-indecomposable]{既約}かつ\hyperref[def:modulecat-exact]{完全}な\hyperref[def:modulecat]{左 $\Cat{C}$-加群圏} $(\Cat{M},\, \btr,\, \alpha,\, \lambda)$
        % \footnote{~\cite[p.152]{etingof2015tensor}}
    ...
    関手
    \begin{align}
        F \colon \Cat{M} &\lto \MODCR{\Cat{C}}{(m \multimap m)},\\ 
        n &\lmto m \multimap n
    \end{align}
    は\hyperref[def:module-functor]{左 $\Cat{C}$-加群圏同値}である．
...
命題\ref{prop:symcat-2d-gauged}, \ref{prop:domainwall-gauged}および定理\ref{thm:modulecat}から，ある\hyperref[def:sym-Frobenius]{対称Frobenius代数対象} $A \in \Obj{\Cat{C}}$ が存在して左 $\Cat{C}$-加群圏の同値 $\Cat{M} \cong \MODCR{\Cat{C}}{A}$ が成り立ち，ゲージ化された理論 $T/\Cat{M}$ は $\Cat{C}' \coloneqq \BIMODC{\Cat{C}}{A}{A}$ の対称性を持ち，$\Cat{M}$ は自然に\hyperref[def:bimodulecat]{$(\Cat{C},\, \BIMODC{\Cat{C}}{A}{A})$-両側加群圏}の構造を持つ．
...
\hyperref[def:bimodulecat]{$(\Cat{C},\, \Cat{C}'')$-両側加群圏}の構造を持つ\hyperref[def:DeligneProduct]{Deligneのテンソル積} $\Cat{M} \boxtimes_{\Cat{C}'} \Cat{M}'$ によって $T/(\Cat{M} \boxtimes_{\Cat{C}'} \Cat{M}')$ とゲージ化することに等しい．
...
\begin{proof}
    \begin{enumerate}
        ...
        　以上の考察から $\forall \varphi \in \Hom{\Cat{C}}(a,\, b)$ は零射であるか同型射であるかのどちらかである\footnote{これを\textbf{Schurの補題}と呼ぶ．}から，$a \neq b$ ならば $\Hom{\Cat{C}}(a,\, b) = 0$ である．$a = b$ のとき，$\Hom{\Cat{C}}(a,\, b) \eqqcolon \End_{\Cat{C}}(a)$ は射の合成に関して体 $\mathbb{K}$ 上の可除代数 (division algebra) を成す．ところで\hyperref[def:tensorfusion-cat]{フュージョン圏} $\Cat{C}$ は\hyperref[def:finite-abcat]{有限}なので $\End_{\Cat{C}}(a)$ は $\mathbb{K}$-ベクトル空間として有限次元であり，かつ仮定より $\mathbb{K}$ は代数閉体であるから，$\End_{\Cat{C}}(a) = \mathbb{K}$ しかありえない．
        ...
        \begin{center}
            \begin{tikzcd}[row sep=large, column sep=large]
                &a \ar[r, "i^{(a)}_\beta"] \ar[dr, "\delta_{\alpha\beta}\Id_a"'] &x \ar[d, red, "p^{(a)}_\alpha"] \\
                & &a
            \end{tikzcd}
        \end{center}
...
$\mathbb{K}$ を代数閉体とするとき，アーベル圏の構造を使わずに\hyperref[def:additive-cat]{$\mathbb{K}$-線形な加法圏}における\hyperref[def:semisimple-cat]{半単純性}を特徴付ける方法が存在する~\cite[p.6]{Mueger2002subfactor}．
...
\begin{proof}
    ...
    \begin{enumerate}
        ...
        \begin{center}
            \begin{tikzcd}[row sep=large, column sep=large]
                &a \ar[r, "i_\alpha"]\ar[dr, "i_\alpha"'] &x \ar[d,red, "m"] \\
                & &x
            \end{tikzcd}
        \end{center}
        ...
        \begin{center}
            \begin{tikzcd}[row sep=large, column sep=large]
                &a \ar[r, "i_\alpha"]\ar[dr, "i_\alpha"'] &x \ar[d,red, "\Id_x"] \\
                & &x
            \end{tikzcd}
        \end{center}
        然るに\hyperref[def:product-coproduct]{直和の普遍性}より赤色の射は一意であるから，$m = \Id_x$ である．
        % \begin{align}
        %     m \coloneqq \sum_{\alpha \in \Lambda} i_\alpha \circ p_\alpha \colon x \lto x
        % \end{align}
        % が成り立つ．
...
\begin{mylem}[label=lem:TVBW-1]{技術的な補題2}
    ...
    \begin{enumerate}
        \item $\forall a,\, b \in \mathrm{Simp} (\Cat{C}),\, \forall x \in \Obj{\Cat{C}}$ および $\forall \varphi \in \expval{b,\, x},\; \forall\psi \in \expval{x^*,\, a^*}$ に対して
        \begin{align}
            \begin{tikzpicture}[baseline={([yshift=-.5ex]current bounding box.center)}]
                \path coordinate[bullet,label=below:$\varphi$] (f)
                +(-0.5,1.6) coordinate[label=left:$b$] (a)
            ...
            \end{tikzpicture}
            &= \delta_{a,\, b}\frac{\omega(\varphi,\, \psi)}{\dim_p(a)}
            \begin{tikzpicture}[baseline={([yshift=-.5ex]current bounding box.center)}]
...
\begin{proof}
    \begin{enumerate}
        \item $a\neq b$ ならば補題\ref{lem:simple-hom}-(1) より両辺は $0$ になる．
        $a=b$ とする．左辺は $\Hom{\Cat{C}}(1,\, a \otimes a^*)$ の元だが，補題\ref{lem:rigid-hom}の同型 $\alpha^{-1}_{1,a,a} \colon \Hom{\Cat{C}} (1,\, a\otimes a^*) \xrightarrow{\cong} \Hom{\Cat{C}} (a,\, a) = \mathbb{K}$ から，ある定数 $A_a \in \mathbb{K}$ を用いて
        \begin{align}
...
2つの単体的複体を取り，その幾何学的実現をそれぞれ $\abs{K},\, \abs{L}$ と書く．
\begin{itemize}
...
\begin{mydef}[label=def:polyhedron,breakable]{2-多面体}
    \begin{itemize}
        ...
        \item 2-多面体の\textbf{層状化} (stratification) とは，$P$ に埋め込まれた\underline{無向グラフ}\footnote{ここでは，位相空間としてのグラフを考えている．} $P^{(1)} \subset P$ であって $P \setminus \Int(P) \subset P^{(1)}$ を充たすもののこと．
...
\begin{mytheo}[label=thm:MP-polytope,breakable]{多面体分割のPL不変性}
    ...
    このとき，$(M,\, G)$ の任意の2つの\hyperref[def:polytope-decomp]{多面体分割} $(P,\, P^{(1)}),\, (P',\, P'{}^{(1)})$ は，以下の操作（\textbf{Matveev-Piergallini move}と呼ばれる）の有限回の組み合わせで互いに写り合う：
    \begin{description}
        \item[\textbf{(T0)}] 
        % $\partial D^2 = D^2 \cap P \subset P \setminus P^{(1)}$ を充たすような2次元円板 $D^2$ を $P$ に付け足す．さらにグラフ $P^{(1)}$ に1つの頂点とループ辺 $\partial D^2$ を付け足す．
        \begin{align}
            T_0 \colon
            \begin{tikzpicture}[baseline={([yshift=-.5ex]current bounding box.center)}]
                \path coordinate (a)
            ...
            \begin{tikzpicture}[baseline={([yshift=-.5ex]current bounding box.center)}]
                ...
                \draw[spath/save=R,red,thick] ($(g)+(0.8,0)$) arc (0:360:0.8 and 0.2);
                \node[bullet,red] at ($(g) + (0,-0.2)$) {};
                \begin{scope}[on background layer]
                    \fill[gray!20, spath/use=frame];
                    \fill[blue!60, spath/use=R];
                    \fill[red!20,opacity=0.6, spath/use=bubble];
                \end{scope}
            \end{tikzpicture}
        \end{align}
        ...
        \begin{align}
            ...
            \begin{tikzpicture}[baseline={([yshift=-.5ex]current bounding box.center)}]
                ...
                \begin{scope}[on background layer]
                    \fill[gray!20, spath/use=frame];
                \end{scope}
                \node at ($(p) + (-0.5,-0.3)$) {$\vdots$};
                \node at ($(p) + (-0.5,0.3)$) {$\vdots$};
                \node at ($(q) + (0.5,-0.3)$) {$\vdots$};
                \node at ($(q) + (0.5,0.3)$) {$\vdots$};
            \end{tikzpicture}
            \lmto 
            \begin{tikzpicture}[baseline={([yshift=-.5ex]current bounding box.center)}]
        ...
        \begin{align}
            ...
            \begin{tikzpicture}[baseline={([yshift=-.5ex]current bounding box.center)}]
                ...
                \begin{scope}[on background layer]
                    \fill[gray!20, spath/use=frame];
                \end{scope}
                \node at ($(g) + (-0.5,-0.3)$) {$\vdots$};
                \node at ($(g) + (-0.5,0.4)$) {$\vdots$};
                \node at ($(g) + (0.5,-0.3)$) {$\vdots$};
                \node at ($(g) + (0.5,0.4)$) {$\vdots$};
            \end{tikzpicture}
        \end{align}
        ...
        \begin{align}
            ...
            \begin{tikzpicture}[baseline={([yshift=-.5ex]current bounding box.center)}]
                ...
                \draw[dashed,spath/save=frame] (a) to[out=-10,in=-170] (b) to[out=30,in=-60] (c) to[out=170,in=10] (d) to[out=-150,in=120] (a);
                \clip[spath/use=frame];
                \draw[spath/save=R] (g.center) -- +(30:2) -- +(-30:2) -- (g.center) --cycle;
                ...
                \begin{scope}[on background layer]
                    \fill[gray!20, spath/use=frame];
                    \clip[spath/use=frame];
                    \fill[red!20, spath/use=R];
                \end{scope}
                \node at (barycentric cs:a=1,b=1,g=1) {$\cdots$};
                \node at (barycentric cs:c=1,d=1,g=1) {$\cdots$};
            \end{tikzpicture}
            \lmto 
            \begin{tikzpicture}[baseline={([yshift=-.5ex]current bounding box.center)}]
                \path coordinate (a)
                ...
                \draw[dashed,spath/save=frame] (a) to[out=-10,in=-170] (b) to[out=30,in=-60] (c) to[out=170,in=10] (d) to[out=-150,in=120] (a);
                \clip[spath/use=frame];
                \draw[spath/save=S] (g.center) -- +(30:2) -- +(-30:2) -- (g.center) --cycle;
                \draw (g.center) to[out=90,in=-90] ++(150:2);
                \draw (g.center) to[out=-120,in=90] ++(-150:2);
                \draw[spath/save=reg] ($(g)+(-0.8,0)$) to[out=90,in=180] ($(g) + (-0.2,0.6)$) to[out=0,in=180] ($(g) + (0.2,0.4)$) to[out=0,in=-150] ($(g) + (30:2)$);
                \draw[spath/save=R,red,thick] (g) arc (0:360:0.4 and 0.1);
                \begin{scope}[on background layer]
                    \fill[gray!20, spath/use=frame];
                    \clip[spath/use=frame];
                    \fill[blue!60, spath/use=R];
                    \fill[white, spath/use=S];
                    \fill[red!20, opacity=0.6, spath/use=reg] -- ($(g) + (-30:2)$) -- (g) arc(0:-180:0.4 and 0.1);
                \end{scope}
                \node at (barycentric cs:a=1,b=1,g=1) {$\cdots$};
                \node at (barycentric cs:c=1,d=1,g=1) {$\cdots$};
            \end{tikzpicture}
...
\subsection{Turaev-Viro不変量の構成：閉多様体}
...
\begin{description}
    ...
    \underline{無向辺} $\forall e \in \mathsf{E}(P^{(1)})$ は，丁度2通りの方法で有向辺になる．こうして得られる2つの有向辺をそれぞれ $\tilde{e}_1,\, \tilde{e}_2$ と書くと，\hyperref[TV:STEP1]{\textsf{\textbf{STEP1.}}} の構成により
    \begin{align}
...
\begin{proof}
    $2+1$ 次元多様体 $M$ の勝手な\hyperref[def:polyhedron]{多面体分割} $(P,\, P^{(1)})$ を1つ固定し，そこから構成した $\irm{Z}{\text{TV},\Cat{C}}(M;\, P)$ が定理\ref{thm:MP-polytope}の操作 \textsf{\textbf{(T0)-(T3)}} に関して不変であることを示せば良い．
    \begin{description}
    \item[\textbf{(T0)}]　
    
    \begin{align}
        T_0 \colon
        \begin{tikzpicture}[baseline={([yshift=-.5ex]current bounding box.center)},scale=1.5]
            \path coordinate (a)
            ++(2,0) coordinate (b)
            ++(0.5,1.5) coordinate (c)
            ++(-2,0) coordinate (d);
            ...
            \begin{scope}[on background layer]
                \fill[gray!20, spath/use=frame];
            \end{scope}
            \node at ($0.5*(a) + 0.5*(c)$) {$R$};
            \draw[->] ($0.1*(a) + 0.9*(b) + (0,0.3)$) arc (0:270:0.1);
        \end{tikzpicture}
        \lmto 
        \begin{tikzpicture}[baseline={([yshift=-.5ex]current bounding box.center)},scale=1.5]
            \path coordinate (a)
            ...
            \coordinate (g) at (barycentric cs:a=1,b=1,c=1,d=1);
            \draw[dashed,spath/save=frame] (a) to[out=-10,in=-170] (b) to[out=30,in=-60] (c) to[out=170,in=10] (d) to[out=-150,in=120] (a);
            \draw[spath/save=bubble] ($(g)+(-0.8,0)$) to[out=90,in=90,looseness=1.5] node[red,midway,below] {$R_1$} ($(g)+(0.8,0)$) arc (0:-180:0.8 and 0.2);
            \draw[spath/save=R,red,thick] ($(g)+(0.8,0)$) arc (0:360:0.8 and 0.2);
            \node[bullet,red] at ($(g) + (0,-0.2)$) {};
            \begin{scope}[on background layer]
                \fill[gray!20, spath/use=frame];
                ...
                \fill[red!20, opacity=0.6, spath/use=bubble];
            \end{scope}
            \node[below,red] at ($(g) + (0,-0.2)$) {$v$};
            \node at ($0.8*(a) + 0.2*(b) + (0,0.3)$) {$R$};
            \node[blue] at (g) {$R_2$};
            \draw[->] ($0.1*(a) + 0.9*(b) + (0,0.3)$) arc (0:270:0.1);
            \draw[->,blue] ($(g) + (0.5,0)$) arc (0:270:0.1);
            \draw[->,red] ($(g) + (0.5,0.4)$) arc (0:270:0.1);
        \end{tikzpicture}
    \end{align}
    によって $M$ の多面体分割 $(P,\, P^{(1)})$ には
    \begin{itemize}
        \item $\mathsf{V}(P^{(1)})$ に新しく1つの頂点 $v$ が，
        \item $\mathsf{E}(P^{(1)})$ に新しく1つの無向辺 $e$ が，
        \item $\mathrm{Reg}(P)$ に新しく2つの\hyperref[def:polyhedron]{領域} $R_1,\, R_2$ が
    \end{itemize}
    追加される．\hyperref[def:coloring]{彩色}
    \begin{align}
        \Gamma \colon \mathrm{Reg}\bigl( T_0(P) \bigr) \lto \Simp(\Cat{C})
    \end{align}
    を1つ固定する．
    すると，新たに追加された無向辺 $e \in \mathsf{E}\bigl( T_0(P^{(1)}) \bigr)$ から\hyperref[TV:STEP3]{\textsf{\textbf{STEP3.}}}によって構成されるcontraction vectorは
    \begin{align}
        \Omega_{\Gamma,\, e}(1_{\mathbb{K}}) \in \expval{\Gamma(R_2)^*,\, \Gamma(R),\, \Gamma(R_1)^*} \otimes_{\mathbb{K}} \expval{\Gamma(R_1),\, \Gamma(R)^*,\, \Gamma(R_2)}
    \end{align}
    であるが，補題\ref{lem:spherical-partition}-(5) によってこれは
    \begin{align}
        \Omega_{\Gamma,\, e}(1_{\mathbb{K}}) = 
        \begin{tikzpicture}[baseline={([yshift=-.5ex]current bounding box.center)}]
            \draw[-<-=.5] (0,0) -- node[midway,right] {$\Gamma(R_1)$} ++(-90:1.5);
            \draw[-<-=.5] (0,0) -- node[midway,above] {$\Gamma(R_2)$} ++(150:1.5);
            \draw[->-=.5] (0,0) -- node[midway,above] {$\Gamma(R)$} ++(30:1.5);
            \node[circlenode] at (0,0) {$\varphi^\bullet$};
        \end{tikzpicture}
        \otimes_{\mathbb{K}}
        \begin{tikzpicture}[baseline={([yshift=-.5ex]current bounding box.center)}]
            \draw[->-=.5] (0,0) -- node[midway,right] {$\Gamma(R_1)$} ++(-90:1.5);
            \draw[-<-=.5] (0,0) -- node[midway,above] {$\Gamma(R)$} ++(150:1.5);
            \draw[->-=.5] (0,0) -- node[midway,above] {$\Gamma(R_2)$} ++(30:1.5);
            \node[circlenode] at (0,0) {$\varphi_\bullet$};
        \end{tikzpicture}
    \end{align}
    と計算できる．
    \hyperref[TV:STEP4]{\textsf{\textbf{STEP4.}}}によって\hyperref[def:linkedG]{絡みグラフ} $\mathrm{Lk} \bigl( v;\, T_0(P) \bigr)$ から得られるストリング図式は
    \begin{align}
        \mathbb{F}_{\Cat{C},\, \Gamma} \Bigl( \mathrm{Lk} \bigl(v;\, T_0(P) \bigr)  \Bigr) &=
        \begin{tikzpicture}[baseline={([yshift=-.5ex]current bounding box.center)}]
            \path coordinate (v_1)
            ++(3,0) coordinate (v_2)
            ;
            \draw[->-=.5] (v_1) -- node[midway,above]{$\Gamma(R)$}(v_2);
            \draw[-<-=.5] (v_1) to[out=60,in=120] node[midway,above]{$\Gamma(R_2)$} (v_2);
            \draw[-<-=.5] (v_1) to[out=-60,in=-120] node[midway,below]{$\Gamma(R_1)$} (v_2);
            \node[circlenode] at (v_1) {};
            \node[circlenode] at (v_2) {};
        \end{tikzpicture} \\
        &\in \Hom{\VEC{\mathbb{K}}} \Bigl( \expval{\Gamma(R_2)^*,\, \Gamma(R),\, \Gamma(R_1)^*} \otimes_{\mathbb{K}} \expval{\Gamma(R_1),\, \Gamma(R)^*,\, \Gamma(R_2)},\, \mathbb{K} \Bigr) 
    \end{align}
    であるから，結局\hyperref[TV:STEP5]{\textsf{\textbf{STEP5.}}}の計算は
    \begin{align}
        Z \bigl(\Gamma;\, T_0(P)\bigr)
        &= \mathbb{F}_{\Cat{C},\, \Gamma} \Bigl( \mathrm{Lk} \bigl(v;\, T_0(P) \bigr)  \Bigr) \Bigl( \Omega_{\Gamma,\, e}(1_{\mathbb{K}}) \Bigr) \otimes_{\mathbb{K}} Z (\Gamma|_{\mathrm{Reg}(P)};\, P) \\
        &= \quad \begin{tikzpicture}[baseline={([yshift=-.5ex]current bounding box.center)}]
            ...
            \draw[->-=.5] (v_1) to[out=60,in=120] node[midway,above]{$\Gamma(R_2)$} (v_2);
            \draw[-<-=.5] (v_1) to[out=-60,in=-120] node[midway,below]{$\Gamma(R_1)$} (v_2);
            \node[circlenode] at (v_1) {$\varphi^\bullet$};
            \node[circlenode] at (v_2) {$\varphi_\bullet$};
        \end{tikzpicture} 
        \quad \otimes_{\mathbb{K}} Z (\Gamma|_{\mathrm{Reg}(P)};\, P) \\
        &= \dim_{\mathbb{K}} \bigl( \expval{\Gamma(R_1),\, \Gamma(R)^*,\, \Gamma(R_2)} \bigr) Z (\Gamma|_{\mathrm{Reg}(P)};\, P)  &&\because\;\text{補題\ref{lem:TVBW-1}-(1)} \\
        &= N^{\Gamma(R)}_{\Gamma(R_1),\, \Gamma(R_2)} Z (\Gamma|_{\mathrm{Reg}(P)};\, P)
    \end{align}
    と実行できる．一方で
    \begin{align}
        \prod_{S \in \mathrm{Reg}\bigl(T_0(P)\bigr)} \dim \bigl( \Gamma(S) \bigr)^{\chi(S)}
        &= \frac{\dim \bigl(\Gamma(R_1)\bigr) \dim \bigl(\Gamma(R_2)\bigr)}{\dim \bigl( \Gamma(R) \bigr) }\prod_{S \in \mathrm{Reg}(P)} \dim \bigl( \Gamma(S) \bigr)^{\chi(S)}, \\
        \abs{M \setminus T_0(P)} &= \abs{M \setminus P} + 1
    \end{align}
    であるから，
    \begin{align}
        &Z_{\text{TV},\Cat{C}} \bigl( M;\, T_0(P) \bigr)  \\
        &= \dim(\Cat{C})^{-\abs{M \setminus P}} \sum_{\Gamma \colon \mathrm{Reg}(P) \to \Simp(\Cat{C})} Z(\Gamma;\, P) \prod_{S \in \mathrm{Reg}(P)} \dim \bigl( \Gamma(S) \bigr)^{\chi(S)} \\
        &\qquad \times \left(\frac{1}{\dim \bigl( \Gamma(R) \bigr) \dim(\Cat{C})}\sum_{a,\, b \in \Simp(\Cat{C})} \dim (a) \dim (b) N^{\Gamma(R)}_{ab}\right) \\
        &= Z_{\text{TV},\Cat{C}} (M;\, P) \qquad \because\; \text{補題\ref{lem:basedring}-(1) \&}\, \dim(x) = \dim (x^*)
    \end{align}
    が言えた\footnote{$\dim (x) = \dim(x^*)$ が成り立つのは $\Cat{C}$ が\hyperref[def:spherical]{球状テンソル圏}であるため．}．
    ...
    \begin{align}
        ...
        \begin{tikzpicture}[baseline={([yshift=-.5ex]current bounding box.center)}]
            ...
            ++(-2,0) coordinate (d);
            \coordinate (g) at (barycentric cs:a=1,b=1,c=1,d=1);
            \coordinate[bullet,label=above left:$u$] (p) at ($(g) + (-0.5,0)$);
            \coordinate[bullet,label=above right:$v$] (q) at ($(g) + (0.5,0)$);
            \draw[dashed,spath/save=frame] (a) to[out=-10,in=-170] (b) to[out=30,in=-60] (c) to[out=170,in=10] (d) to[out=-150,in=120] (a);
            \clip[spath/use=frame];
            \draw (p) -- ++(120:2);
            \draw (p) -- ++(180:2);
            ...
            \begin{scope}[on background layer]
                \fill[gray!20, spath/use=frame];
            \end{scope}
            \node at (barycentric cs:a=1,c=1) {$R$};
            \draw[->] ($0.5*(a) + 0.5*(b) + (0,0.3)$) arc (0:270:0.1);
            \node at ($(p) + (-0.5,-0.3)$) {$\vdots$};
            \node at ($(p) + (-0.5,0.3)$) {$\vdots$};
            \node at ($(q) + (0.5,-0.3)$) {$\vdots$};
            \node at ($(q) + (0.5,0.3)$) {$\vdots$};
        ...
        \begin{tikzpicture}[baseline={([yshift=-.5ex]current bounding box.center)}]
            ...
            \end{scope}
            \node at ($(g) + (0,-0.3)$) {$R$};
            \draw[->] ($0.6*(a) + 0.4*(b) + (0,0.3)$) arc (0:270:0.1);
            \node[red] at ($(g) + (0,0.3)$) {$R'$};
            \draw[->] ($0.7*(c) + 0.3*(d) + (0,-0.3)$) arc (0:270:0.1);
            \node at ($(p) + (-0.5,-0.3)$) {$\vdots$};
            \node at ($(p) + (-0.5,0.3)$) {$\vdots$};
            \node at ($(q) + (0.5,-0.3)$) {$\vdots$};
            \node at ($(q) + (0.5,0.3)$) {$\vdots$};
    ...
    \begin{itemize}
        \item $\mathsf{E}(P^{(1)})$ に新しく1つの無向辺 $e$ が，
        \item $\mathrm{Reg}(P)$ に新しく1つの領域 $R'$ が\footnote{実は $R'$ と $R$ が連結である場合もありえる．}
    \end{itemize}
    追加される．彩色
    \begin{align}
        \Gamma \colon \mathrm{Reg} \bigl( T_1(P) \bigr)  \lto \Simp(\Cat{C})
    \end{align}
    を1つ固定する．すると，新たに追加された無向辺 $e \in \mathsf{E}\bigl( T_0(P^{(1)}) \bigr)$ から\hyperref[TV:STEP3]{\textsf{\textbf{STEP3.}}}によって構成されるcontraction vectorはちょうど
    \begin{align}
        \Omega_{\Gamma,\, e} (1_{\mathbb{K}}) \in \expval{\Gamma(R)^*,\, \Gamma(R')} \otimes_{\mathbb{K}} \expval{\Gamma(R),\, \Gamma(R')^*}
    \end{align}
    であり，$\Gamma(R) = \Gamma(R')$ でない限り $0$ になる．
    このことから，\hyperref[TV:STEP5]{\textsf{\textbf{STEP5.}}}の計算のうち2頂点 $u,\, v \in \mathsf{V}(P^{(1)})$ に関連する部分は
    \begin{align}
        &\Bigl(\mathbb{F}_{\Cat{C},\, \Gamma} \left( 
            \begin{tikzpicture}[baseline={([yshift=-.5ex]current bounding box.center)}]
                \path coordinate (A)
                ++(1,0) coordinate[bullet] (x)
                ;
                \draw[->-=.5] (A) to[out=60,in=120,looseness=2] node[midway,above right,red] {$R'$} (x);
                \draw[-<-=.5] (A) to[out=-60,in=-120,looseness=2] node[midway,below right] {$R$} (x);
                \node[circle,fill=black!20,minimum size=10mm] at (A) {};
            \end{tikzpicture}
        \right) \otimes_{\mathbb{K}} \mathbb{F}_{\Cat{C},\, \Gamma} \left( 
            \begin{tikzpicture}[baseline={([yshift=-.5ex]current bounding box.center)}]
                \path coordinate (A)
                ++(-1,0) coordinate[bullet] (x)
                ;
                \draw[-<-=.5] (A) to[out=120,in=60,looseness=2] node[midway,above left,red] {$R'$} (x);
                \draw[->-=.5] (A) to[out=-120,in=-60,looseness=2] node[midway,below left] {$R$} (x);
                \node[circle,fill=black!20,minimum size=10mm] at (A) {};
            \end{tikzpicture}
         \right)\Bigr) \Bigl( \Omega_{\Gamma,\, e} (1_{\mathbb{K}}) \Bigr) \\
         &= \begin{tikzpicture}[baseline={([yshift=-.5ex]current bounding box.center)}]
            \path coordinate (A)
            ++(2,0) coordinate (x)
            ++(1,0) coordinate (y)
            ++(2,0) coordinate (B)
            ;
            \draw[-<-=.5] (A) to[out=60,in=120] node[midway,above right] {$\Gamma(R)$} (x);
            \draw[->-=.5] (A) to[out=-60,in=-120] node[midway,below right] {$\Gamma(R)$} (x);
            \draw[->-=.5] (B) to[out=120,in=60] node[midway,above left] {$\Gamma(R)$} (y);
            \draw[-<-=.5] (B) to[out=-120,in=-60] node[midway,below left] {$\Gamma(R)$} (y);
            \node[circlenode] at (x) {$\varphi^\bullet$};
            \node[circlenode] at (y) {$\varphi_\bullet$};
            \node[circle,fill=black!20,minimum size=10mm] at (A) {};
            \node[circle,fill=black!20,minimum size=10mm] at (B) {};
        \end{tikzpicture} &&\because\; \text{補題\ref{lem:spherical-partition}-(5)}\\
        % &= \begin{tikzpicture}[baseline={([yshift=-.5ex]current bounding box.center)}]
        ...
        %     ++(2,0) coordinate (B)
        %     ;
        %     \draw[-<-=.5] (A) to[out=60,in=120,looseness=2] node[midway,above] {$\Gamma(R)$} (B);
        %     \draw[->-=.5] (A) to[out=-60,in=-120,looseness=2] node[midway,below] {$\Gamma(R)$} (B);
        %     \node[circle,fill=black!20,minimum size=10mm] at (A) {};
        %     \node[circle,fill=black!20,minimum size=10mm] at (B) {};
        % \end{tikzpicture} &&\because\; \text{補題\ref{lem:TVBW-1}-(3)}\\
        % &= \frac{1}{\dim \bigl( \Gamma(R) \bigr)}
        &= \frac{1}{\dim \bigl( \Gamma(R) \bigr) }\begin{tikzpicture}[baseline={([yshift=-.5ex]current bounding box.center)}]
            \path coordinate (A)
            ++(4,0) coordinate (B)
            ;
            \draw[->-=.5] ($(A)+(60:0.5)$) to[out=0,in=0,looseness=2] node[midway,right] {$\Gamma(R)$} ($(A)+(-60:0.5)$);
            \draw[->-=.5] ($(B)+(-120:0.5)$) to[out=180,in=180,looseness=2] node[midway,left] {$\Gamma(R)$} ($(B)+(120:0.5)$);
            \node[circle,fill=black!20,minimum size=10mm] at (A) {};
            \node[circle,fill=black!20,minimum size=10mm] at (B) {};
        \end{tikzpicture}
    \end{align}
    だと分かる．ただし，最後の等号で\hyperref[lem:nondegen-pairing]{非退化な双線形形式 $\omega$} に関する双対基底として具体的に
    \begin{align}
        \varphi^\bullet &= \frac{1}{\dim_p \bigl( \Gamma(R) \bigr)}
        \begin{tikzpicture}[baseline={([yshift=-.5ex]current bounding box.center)}]
            \path coordinate[bullet,label=left:$p_{\Gamma(R)}$] (x_1)
            ++(1,0) coordinate (x_2)
            ++(1,0) coordinate (x_3)
            ++(1,0) coordinate (x_4)
            ;
            ...
            \draw[->-=.5] (x_3) -- ++(0,1);
            \draw[-<-=.5] (x_4) -- ++(0,1);
            \node at (0.5,1) {$\lev_{\Gamma(R)^*}$};
            \node at (1.5,-1) {$\lcoev_{\Gamma(R)^*}$};
            \node at (1.5,-2) {$\lcoev_{\Gamma(R)}$};
        \end{tikzpicture}
        \in \expval{\Gamma(R),\, \Gamma(R)^*}^*, \\ 
        \varphi_\bullet &= \lcoev_{\Gamma(R)} \in \expval{\Gamma(R),\, \Gamma(R)^*}
    \end{align}
    を取れることを用いた\footnote{$\dim_{\mathbb{K}} \bigl( \expval{\Gamma(R),\, \Gamma(R)^*} \bigr) = 1$ である！なお，$p_{\Gamma(R)} \in \Hom{\Cat{C}} \bigl( \Gamma(R),\, \Gamma(R)^{**} \bigr) $ は $\Cat{C}$ の\hyperref[def:spherical]{球状構造}である．}．
    最右辺は $\dim \bigl( \Gamma(R) \bigr)^{-1}$ 倍を除いて $T_1$ を施す前と等しい．

    ところで，$\Gamma(R) = \Gamma(R')$ のとき
    \begin{align}
        \prod_{S \in \mathrm{Reg}\bigl(T_1(P)\bigr)} \dim \bigl( \Gamma(S) \bigr)^{\chi(S)}
        &= \dim \bigl( \Gamma(R) \bigr) \prod_{S \in \mathrm{Reg}(P)} \dim \bigl( \Gamma(S) \bigr)^{\chi(S)}, \\
        \abs{M \setminus T_1(P)} &= \abs{M \setminus P}
    \end{align}
    が成り立つので $Z_{\text{TV},\Cat{C}} \bigl( M;\, T_1(P) \bigr)  = Z_{\text{TV},\Cat{C}} (M;\, P)$ が言えた．
    ...
    \begin{align}
        ...
        \begin{tikzpicture}[baseline={([yshift=-.5ex]current bounding box.center)}]
            ...
            ++(0.5,1.5) coordinate (c)
            ++(-2,0) coordinate (d);
            \coordinate[bullet,label=right:$u$] (g) at (barycentric cs:a=1,b=1,c=1,d=1);
            \draw[dashed,spath/save=frame] (a) to[out=-10,in=-170] (b) to[out=30,in=-60] (c) to[out=170,in=10] (d) to[out=-150,in=120] (a);
            \clip[spath/use=frame];
            \draw (g) -- ++(120:2);
            \draw (g) -- ++(180:2);
            ...
            \end{scope}
            \node at ($(g) + (0,-0.3)$) {$R$};
            \draw[->] ($0.4*(a) + 0.6*(b) + (0,0.15)$) arc (0:270:0.1);
            \node at ($(g) + (0,0.3)$) {$R'$};
            \draw[->] ($0.5*(c) + 0.5*(d) + (0,-0.15)$) arc (0:270:0.1);
            \node at ($(g) + (-0.5,-0.3)$) {$\vdots$};
            \node at ($(g) + (-0.5,0.4)$) {$\vdots$};
            \node at ($(g) + (0.5,-0.3)$) {$\vdots$};
            \node at ($(g) + (0.5,0.4)$) {$\vdots$};
    ...
    \end{align}
    である．
    \hyperref[TV:STEP5]{\textsf{\textbf{STEP5.}}}の計算のうち非自明な部分は頂点 $u \in \mathsf{V} \bigl( T_2(P^{(1)}) \bigr)$ に関連する部分のみであり，
    \begin{align}
            \begin{tikzpicture}[baseline={([yshift=-.5ex]current bounding box.center)}]
                \path coordinate (A)
                ++(2,0) coordinate (B)
                ;
                \draw[-<-=.5] (A) to[out=60,in=120] node[midway,above right] {$\Gamma(R')$} (B);
                \draw[->-=.5] (A) to[out=-60,in=-120] node[midway,below right] {$\Gamma(R)$} (B);
                \node[circle,fill=black!20,minimum size=10mm] at (A) {};
                \node[circle,fill=black!20,minimum size=10mm] at (B) {};
            \end{tikzpicture}
            &= \begin{tikzpicture}[baseline={([yshift=-.5ex]current bounding box.center)}]
            ...
            &= \Bigl(\mathbb{F}_{\Cat{C},\, \Gamma} \left( 
                \begin{tikzpicture}[baseline={([yshift=-.5ex]current bounding box.center)}]
                    ...
                    ++(1,0) coordinate[bullet] (x)
                    ;
                    \draw[->-=.5] (A) to[out=60,in=120,looseness=2] node[midway,above right] {$R'$} (x);
                    \draw[-<-=.5] (A) to[out=-60,in=-120,looseness=2] node[midway,below right] {$R$} (x);
                    \node[circle,fill=black!20,minimum size=10mm] at (A) {};
                \end{tikzpicture}
            \right) \otimes_{\mathbb{K}} \mathbb{F}_{\Cat{C},\, \Gamma} \left( 
                \begin{tikzpicture}[baseline={([yshift=-.5ex]current bounding box.center)}]
                    ...
                    ++(-1,0) coordinate[bullet] (x)
                    ;
                    \draw[-<-=.5] (A) to[out=120,in=60,looseness=2] node[midway,above left] {$R'$} (x);
                    \draw[->-=.5] (A) to[out=-120,in=-60,looseness=2] node[midway,below left] {$R$} (x);
                    \node[circle,fill=black!20,minimum size=10mm] at (A) {};
                \end{tikzpicture}
             \right)\Bigr) \Bigl( \Omega_{\Gamma,\, e} (1_{\mathbb{K}}) \Bigr)
             &&\because\; \text{補題\ref{lem:spherical-partition}-(5)}\\
    \end{align}
    と計算できるが，\textsf{\textbf{(T1)}}と同様に最右辺は $T_2$ を施す前と等しい．Euler標数の変化はないので $Z_{\text{TV},\Cat{C}} \bigl( M;\, T_2(P) \bigr)  = Z_{\text{TV},\Cat{C}} (M;\, P)$ が言えた．
    ...
    \begin{align}
        ...
        \begin{tikzpicture}[baseline={([yshift=-.5ex]current bounding box.center)},scale=1.5]
            ...
            \begin{scope}[on background layer]
                ...
                \fill[red!20, spath/use=R];
            \end{scope}
            \node (R1) at (barycentric cs:a=1,g=1,d=1) {$R_1$};
            % \node (R2) at (barycentric cs:a=1,g=1,b=1) {$R_2$};
            \node[red] (R2) at (barycentric cs:b=1,g=1,c=1) {$R_2$};
            % \node (R4) at (barycentric cs:c=1,g=1,d=1) {$R_4$};
            \draw[->] ($(R1) + (-0.2,0)$) arc (0:270:0.1);
            % \draw[->] ($(R2) + (0,-0.2)$) arc (0:270:0.1);
            \draw[->,red] ($(R2) + (0.4,0)$) arc (0:270:0.1);
            % \draw[->] ($(R4) + (0,0.2)$) arc (0:270:0.1);
            \node at (barycentric cs:a=1,b=1,g=1) {$\cdots$};
            \node at (barycentric cs:c=1,d=1,g=1) {$\cdots$};
        \end{tikzpicture}
        \lmto 
        \begin{tikzpicture}[baseline={([yshift=-.5ex]current bounding box.center)},scale=1.5]
            ...
            \draw (g.center) to[out=-120,in=90] ++(-150:2);
            \draw[spath/save=reg] ($(g)+(-0.8,0)$) to[out=90,in=180] ($(g) + (-0.2,0.6)$) to[out=0,in=180] ($(g) + (0.2,0.4)$) to[out=0,in=-150] ($(g) + (30:2)$);
            \draw[spath/save=R,red,thick] (g) arc (0:360:0.4 and 0.1) --cycle;
            \begin{scope}[on background layer]
                \fill[gray!20, spath/use=frame];
                \fill[blue!60, spath/use=R];
                \clip[spath/use=frame];
                \fill[white, spath/use=S];
                \fill[red!20, spath/use=reg, opacity=0.6] -- ($(g) + (-30:2)$) -- (g) arc(0:-180:0.4 and 0.1);
            \end{scope}
            \node[blue] (R3) at ($(g) + (-0.3,0)$) {$R_3$};
            \draw[->,blue] ($(R3) + (-0.2,0)$) arc (0:270:0.1);
            \node at (barycentric cs:a=1,b=1,g=1) {$\cdots$};
            \node at (barycentric cs:c=1,d=1,g=1) {$\cdots$};
        \end{tikzpicture}
    \end{align}
    ...
    によって $M$ の多面体分割 $(P,\, P^{(1)})$ には
    \begin{itemize}
        \item $\mathsf{E}(P^{(1)})$ に新しく1つの無向辺 $\textcolor{red}{e}$ が，
        \item $\mathrm{Reg}(P)$ に新しく1つの領域 $\textcolor{blue}{R_3}$ が
    \end{itemize}
    追加される．彩色
    \begin{align}
        \Gamma \colon \mathrm{Reg} \bigl( T_3(P) \bigr) \lto \Simp(\Cat{C})
    \end{align}
    を1つ固定しよう．
    \hyperref[TV:STEP5]{\textsf{\textbf{STEP5.}}}の計算のうち非自明な部分は，
    図中の頂点から構成される\hyperref[def:linkedG]{絡みグラフ}のうち，新しく追加された辺 $\textcolor{red}{e}$ と関連する部分のみであり，
    \begin{align}
        \mathbb{F}_{\Cat{C},\, \Gamma} \left( 
            \begin{tikzpicture}[baseline={([yshift=-.5ex]current bounding box.center)}]
                \path coordinate[bullet] (x)
                +(0,2) coordinate[bullet] (y)
                +(1.5,1) coordinate (A)
                ;
                \draw[->-=.5] (x) to[out=120,in=-120] node[midway,left,blue] {$R_3$} (y);
                \draw[->-=.5] (x) to[out=60,in=-60] node[midway,right,red] {$R_2$} (y);
                \draw[-<-=.5] (x) to[out=-90,in=-90] node[midway,below right] {$R_1$} (A);
                \draw[->-=.5] (y) to[out=90,in=90] node[midway,above right] {$R_1$} (A);
                \node[circle,fill=black!20,minimum size=10mm] at (A) {};
            \end{tikzpicture}
        \right) \bigl( \Omega_{\Gamma,\, e}(1_{\mathbb{K}}) \bigr) 
        &= \begin{tikzpicture}[baseline={([yshift=-.5ex]current bounding box.center)}]
            \path coordinate (x)
            +(0,2) coordinate (y)
            +(2,1) coordinate (A)
            ;
            \draw[-<-=.5] (x) to[out=120,in=-120] node[midway,left,blue] {$\Gamma(R_3)$} (y);
            \draw[-<-=.5] (x) to[out=60,in=-60] node[midway,right,red] {$\Gamma(R_2)$} (y);
            \draw[->-=.5] (x) to[out=-90,in=-90] node[midway,below right] {$\Gamma(R_1)$} (A);
            \draw[-<-=.5] (y) to[out=90,in=90] node[midway,above right] {$\Gamma(R_1)$} (A);
            \node[circle,fill=black!20,minimum size=10mm] at (A) {};
            \node[circlenode] at (x) {$\varphi^\bullet$};
            \node[circlenode] at (y) {$\varphi_\bullet$};
        \end{tikzpicture} &&\because\; \text{補題\ref{lem:spherical-partition}-(5)} \\ 
        &= \frac{N^{\Gamma(R_1)}_{\Gamma(\textcolor{blue}{R_3}) \Gamma(\textcolor{red}{R_2})}}{\dim \bigl( \Gamma(R_1) \bigr) } \quad
        \begin{tikzpicture}[baseline={([yshift=-.5ex]current bounding box.center)}]
            \draw[->-=.5] (90:0.5) to[out=120,in=-120,looseness=5] node[midway,left] {$\Gamma(R_1)$} (-90:0.5);
            \node[circle,fill=black!20,minimum size=10mm] at (0,0) {};
        \end{tikzpicture} &&\because\; \text{補題\ref{lem:TVBW-1}-(1)}
    \end{align}
    と計算できる．
    ところで，
    \begin{align}
        \prod_{S \in \mathrm{Reg}\bigl(T_3(P)\bigr)} \dim \bigl( \Gamma(S) \bigr)^{\chi(S)}
        &= \frac{\dim \bigl( \Gamma(\textcolor{blue}{R_3}) \bigr)}{\dim \bigl( \Gamma(R_1) \bigr) \dim \bigl( \Gamma(\textcolor{red}{R_2}) \bigr) } \prod_{S \in \mathrm{Reg}(P)} \dim \bigl( \Gamma(S) \bigr)^{\chi(S)}, \\
        \abs{M \setminus T_3(P)} &= \abs{M \setminus P}
    \end{align}
    が成り立つので，
    \begin{align}
        &Z_{\text{TV},\Cat{C}} \bigl( M;\, T_3(P) \bigr)  \\
        &= \dim (\Cat{C})^{-\abs{M \setminus P}} \sum_{\Gamma \colon \mathrm{Reg}(P) \to \Simp(\Cat{C})} Z(\Gamma;\, P) \prod_{S \in \mathrm{Reg}(P)} \dim \bigl( \Gamma(S) \bigr)^{\chi(S)} \\ 
        &\qquad \times \left(\frac{1}{\dim \bigl( \Gamma(R_1) \bigr) \dim \bigl( \Gamma(\textcolor{red}{R_2}) \bigr)} \sum_{\textcolor{blue}{a} \in \Simp(\Cat{C})} N^{\Gamma(R_1)}_{\textcolor{blue}{a} \Gamma(\textcolor{red}{R_2})} \dim (\textcolor{blue}{a})\right) \\
        &= Z_{\text{TV},\Cat{C}} \bigl( M;\, P \bigr) \qquad\because\; \text{補題\ref{lem:basedring}-(1) \&}\, \dim(x) = \dim (x^*)
    \end{align}
    が言えた．
\end{proof}

\subsection{Turaev-Viro不変量の構成：境界付き多様体}
...
\begin{description}
    ...
    \begin{mydef}[label=def:coloring-bdy]{境界付き彩色}
        ...
        \begin{description}
            ...
            $e$ が一意的に\hyperref[def:polyhedron]{隣接}する\footnote{\hyperref[def:graph-bdy]{境界グラフ}の定義}\hyperref[def:polyhedron]{領域} $R_e \in \mathrm{Reg}(P)$ に対して，$\Gamma(R_e) = x_e$ （図\ref{fig:bdy-coloring}）
        \end{description}
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}
        ...
        +(0:3) coordinate (v_1)
        +(-130:3) coordinate (v_2)
        +(120:2) coordinate (v_3)
        ++(0,-5) coordinate (vint)
        +(0:3) coordinate (vint_1)
        +(-130:3) coordinate (vint_2)
        +(120:2) coordinate (vint_3)
        ;
        % \draw[->-=.5] (v.center) -- (v_1);
        % \draw[->-=.5] (v.center) -- (v_2);
        % \draw[-<-=.5] (v.center) -- (v_3);
        \foreach \i in {1,...,3} {
            \draw[spath/save=R_\i, red] (vint) -- (vint_\i) -- (v_\i) -- (v.center) -- cycle;
        }
        \draw[name path=e_1, ->-=.5] (v.center) -- (v_1);
        \draw[name path=e_2, ->-=.5] (v.center) -- (v_2);
        \draw[name path=e_3, -<-=.5] (v.center) -- (v_3);
        \draw[name path=eint_3,red] (v_3) -- (vint_3);
    
        \begin{scope}
            \fill[white,name intersections={of=e_2 and eint_3}] (intersection-1) circle (5pt);
            \draw[->-=.5] (v.center) -- (v_2);
        \end{scope}
        
        \foreach \i in {1,...,3} {
            \node[red] at ($0.5*(vint) + 0.5*(vint_\i) + (0,1)$) {$\Gamma (R_\i)$};
            \node[below] at ($0.4*(v) + 0.6*(v_\i)$) {$x_{e_\i} = \Gamma (R_\i)$};
        }
    \end{tikzpicture}
    \caption{彩色の境界条件}
    \label{fig:bdy-coloring}
\end{figure}%
...
\begin{description}
    ...
    　有向グラフ $G$ の辺のうち，$v \in \mathsf{V}(G)$ を端点に持つもの全体が成す集合を $E_v$ と書く．
    $E_v$ には，\underline{内向き}法線ベクトルが定める\footnote{つまり，$\partial M$ の向きが定めるものとは逆向きである．}サイクリックな順序 $<$ を与える．そして $\forall e \in E_v$ に対して
    \begin{align}
        \epsilon_e \coloneqq
        \begin{cases}
    ...
    \begin{figure}[H]
        ...
        \begin{tikzpicture}
            ...
            \foreach \i in {1,...,3} {
                \node[red] at ($0.5*(vint) + 0.5*(vint_\i) + (0,1)$) {$\Gamma (R_\i)$};
                \node[below] at ($0.4*(v) + 0.6*(v_\i)$) {$x_{e_\i}$};
            }
        
            % normal vector
            \draw[ultra thick, blue,->] (v) -- ++(0,-1);

            \node at ($(vint_1) + (-1,-2)$) {$\mathcal{H}_v \bigl( G;\, \{x_e\} \bigr) = \expval{x_{e_1}^{*},\, x_{e_2}^*,\, x_{e_3}}$};
        \end{tikzpicture}
        \caption{境界のベクトル空間．内向き法線ベクトルを青色で描いた．このとき $e_1 < e_2 < e_3$ である．}
        \label{fig:TVbdy-STEP1}
    \end{figure}%

    \item[\textbf{STEP2. 内部グラフの向き付け}]　
    
    　\underline{無向}グラフ\footnote{\hyperref[def:polytope-decomp]{多面体分割の定義}から，\hyperref[def:graph-bdy]{境界グラフ} $\partial P$ は有向グラフとして常に $G$ と等しい．} $P^{(1)} \setminus \partial P$ に勝手な向き $\mathcal{O} \in \{\pm\}^{\abs{\mathsf{E} (P^{(1)} \setminus \partial P)}}$ を与えることで，$P^{(1)} \setminus \partial P$ を有向グラフ $P^{(1)}{}^{\mathcal{O}} \setminus \partial P$ にする．
    そして
    \begin{align}
    ...
    \end{align}
    とおくと，与えられた有向辺の始点を返す関数 $\mathrm{s} \colon e \lmto \mathrm{s}(e)$ によって集合 $\bigcup_{\mathcal{O}} \mathsf{E}_{\partial}^{\mathcal{O}}$ と集合 $\mathsf{V}(G)$ は1対1対応する（図\ref{fig:TVbdy-STEP2}）．
    ...
    \begin{figure}[H]
        ...
        \begin{subfigure}{0.4\columnwidth}
            ...
            \begin{tikzpicture}[scale=0.6]
                \path coordinate[bullet,label=above right:${v = \mathrm{s}(\textcolor{red}{e})}$] (v)
                +(0:3) coordinate (v_1)
                +(-130:3) coordinate (v_2)
                +(120:2) coordinate (v_3)
                ++(0,-5) coordinate (vint)
                ...
                % label of coloring
                \foreach \i in {1,...,3} {
                    \node[red] at ($0.5*(vint) + 0.5*(vint_\i) + (0,1)$) {};
                    \node[below] at ($0.4*(v) + 0.6*(v_\i)$) {$x_{e_\i}$};
                }
            
                % orientation 
                \draw[ultra thick, red,->-=.5] (v.center) -- node[midway,right] {$\textcolor{red}{\bm{e}}$} (vint);
                % normal vector
                \draw[ultra thick, blue,->] (v.center) -- ++(0,-1);
            
                \node at ($(vint_1) + (-1,-2)$) {$\mathcal{H}_{\mathrm{s}(\textcolor{red}{e})} \bigl( G;\, \{x_e\} \bigr) = \mathcal{H} (\textcolor{red}{e};\, \Gamma)$};
            \end{tikzpicture}
            \caption{$\textcolor{red}{e} \in E^{\mathcal{O}}_{\partial}$ の場合．}
            \label{fig:edgebdy}
        \end{subfigure}
        \hspace{5mm}
        \begin{subfigure}{0.4\columnwidth}
            ...
            \begin{tikzpicture}[scale=0.6]
                ...
                +(-130:3) coordinate (v_2)
                +(120:2) coordinate (v_3)
                ++(0,-5) coordinate[bullet,label=below right:$\mathrm{s}(\textcolor{red}{e})$] (vint)
                +(0:3) coordinate (vint_1)
                +(-130:3) coordinate (vint_2)
                +(120:2) coordinate (vint_3)
                ;
                ...
                % internal regions
                \foreach \i in {1,...,3} {
                    \draw[spath/save=R_\i, red] (vint.center) -- (vint_\i) -- (v_\i) -- (v.center) -- cycle;
                }
                % boundary graphs
                \draw[name path=e_1, ->-=.5] (v.center) -- (v_1);
                \draw[name path=e_2, ->-=.5] (v.center) -- (v_2);
                ...
                % orientation 
                \draw[ultra thick, red,-<-=.5] (v.center) -- node[midway,right] {$\textcolor{red}{\bm{e}}$} (vint.center);
                % normal vector
                \draw[ultra thick, blue,->] (v.center) -- ++(0,-1);
            
                \node at ($(vint_1) + (-1,-2)$) {$\mathcal{H}_{\mathrm{s}(\textcolor{red}{e})} \bigl( G;\, \{x_e\} \bigr) \neq \mathcal{H} (\textcolor{red}{e};\, \Gamma)$};
            \end{tikzpicture}
            \caption{$\textcolor{red}{e} \in E^{\mathcal{O}}_{\text{int}}$ の場合．}
            \label{fig:edgeint}
        \end{subfigure}
        \caption{$E^{\mathcal{O}}_{\partial}$ と $E^{\mathcal{O}}_{\text{int}}$．}
        \label{fig:TVbdy-STEP2}
    \end{figure}%
...
\begin{align}
    \label{eq:preTV-cob}
    Z_{\text{preTV}}\bigl(M;\, G_0,\, G_1;\, \{x^0_e\},\, \{x^1_e\}\bigr) &\coloneqq \frac{\sqrt{\dim(\Cat{C})}^{\abs{\Sigma_1 \setminus G_1}} \sqrt{\dim(\Cat{C})}^{\abs{\Sigma_0 \setminus G_0}}}{\displaystyle\prod_{e \in \mathsf{E} (G_1)} \sqrt{\dim (x^1_e)} \prod_{e \in \mathsf{E} (G_0)} \sqrt{\dim (x^0_e)}} Z_{\text{TV},\, \Cat{C}} (M,\, \OP{G_0} \cup G_1)
\end{align}
とおく．ただし $\abs{\Sigma_1 \setminus G_1}$ は $\Sigma_1 \setminus G_1$ の連結成分の個数である．
$Z_{\text{preTV}}\bigl(M;\, G_0,\, G_1;\, \{x^0_e\},\, \{x^1_e\}\bigr)$ は
\begin{align}
...
\begin{marker}
    $Z_{\text{preTV}}\bigl(M;\, G_0,\, G_1;\, \{x^0_e\},\, \{x^1_e\}\bigr)$ の規格化定数には不定性がある．例えば~\cite[p.282]{Turaev2017}では
    \begin{align}
        Z_{\text{preTV}}\bigl(M;\, G_0,\, G_1;\, \{x^0_e\},\, \{x^1_e\}\bigr) &\coloneqq \frac{\dim(\Cat{C})^{\abs{\Sigma_1 \setminus G_1}}}{\displaystyle\prod_{e \in \mathsf{E} (G_1)} \dim (x_e)} Z_{\text{TV},\, \Cat{C}} (M,\, \OP{G_0} \cup G_1)
    \end{align}
...
\begin{mylem}[label=lem:statesum-preTQFT]{state sum TQFTの雛形}
    ...
    を与える．このとき，
    \begin{align}
        &Z_{\text{preTV}}\bigl(M_1 \circ M_0;\, G_0,\, G_1;\, \{x^0_e\},\, \{x^1_e\}\bigr) \\
        &= \sum_{\{x_e\}_{e \in \mathsf{E}(G)}} Z_{\text{preTV}}\bigl(M_1;\, G,\, G_1;\, \{x_e\},\, \{x^1_e\}\bigr) \circ Z_{\text{preTV}}\bigl(M_0;\, G_0,\, G;\, \{x^0_e\},\, \{x_e\}\bigr)
    \end{align}
    が成り立つ．
\end{mylem}
...
\begin{mylem}[label=lem:statesum-premon]{state sum TQFTの雛形}
    ...
    を与える．このとき，
    \begin{align}
        Z_{\text{preTV}}\bigl(M \sqcup M';\, G_0 \sqcup G'_0,\, G_1 \sqcup G'_1\bigr) 
        = Z_{\text{preTV}}\bigl(M;\, G_0,\, G_1\bigr) \otimes_{\mathbb{K}} Z_{\text{preTV}} \bigl(M';\, G'_0,\, G'_1\bigr)
    \end{align}
...
このとき， $\forall M \in \Hom{\BORD{2}{3}^{\LSO}}(\Sigma_0,\, \Sigma_1)$ およびその境界の\hyperref[def:skeleton-2mfd]{多面体分割} $G_i \subset \Sigma_i$ に対して，
\hyperref[def:product-coproduct]{直和の普遍性}を利用して $\mathbb{K}$-線型写像\eqref{eq:preTV-cob}を拡張した
\begin{align}
...
\begin{align}
    \sum_{\{x^1_e\}_{e \in \mathsf{E} (G_1)}} Z_{\text{preTV}}\bigl(M;\, G_0,\, G_1;\, \{x^0_e\},\, \{x^1_e\}\bigr) = \textcolor{red}{\tilde{Z}_{\text{TV},\, \Cat{C}} (M;\, G_0,\, G_1)} \circ \iota_{\{x^0_e\}}
\end{align}
...
\end{align}
が成り立つこともわかる．
% 成り立つ，圏 $\VEC{\mathbb{K}}$ における可換図式
% \begin{center}
%     \begin{tikzcd}[row sep=large, column sep=large]
%          &Z_{\text{TV}, \Cat{C}} (\Sigma_0,\, G_0) \ar[r, red, dashed, "\exists ! Z_{\text{TV}, \Cat{C}} (M; G_0, G_1)"] &Z_{\text{TV},\Cat{C}} (\Sigma_1, G_1) \\
%          &\mathcal{H} \bigl( G_0; \{x_e\} \bigr) \ar[ur, "\sum_{\{x^1_e\}} \abs{M;G_0,G_1;\{x^0_e\},\{x^1_e\}}"']\ar[u, "\iota_{\{x^0_e\}}"] &
%     \end{tikzcd}
% \end{center}
% によって定義する．
...
\end{align}
をとることができる．物理的には，境界の理論の連続極限を取ることに対応する．射影極限は\hyperref[def:lim]{極限}なので，錐 $\bigl(Z_{\text{TV},\, \Cat{C}} (\Sigma),\, \{\tau_G \colon Z_{\text{TV},\, \Cat{C}} (\Sigma) \xrightarrow{\cong} Z_{\text{TV},\, \Cat{C}} (\Sigma,\, G)\}_{G \subset \Sigma}\bigr)$ が存在する．
...
\subsection{三角形分割の場合の計算}

Turaev-Viroが最初に発見したTQFTは，多様体 $M$ の\hyperref[def:polytope-decomp]{多面体分割} $(P,\, P^{(1)})$ として三角形分割をとる場合のものであった~\cite{TuraevViro1992TQFT}．
三角形分割に対して\hyperref[def:TV-closed]{Turaev-Viro不変量}を計算してみよう．
これまでと同様に，\hyperref[def:spherical]{球状フュージョン圏} $(\Cat{C},\, \otimes,\, 1,\, a,\, l,\, r,\, \lcoev,\, \lev,\, \rcoev,\, \rev)$ とその\hyperref[def:pivotal]{旋回構造} $p$ を1つ固定する．

\begin{mylem}[label=lem:trivalent-ladder,breakable]{梯子型ストリング図式の計算}
    \begin{itemize}
        \item $\forall a_0,\, a_1,\, \dots,\, a_n \in \Simp(\Cat{C})$
        \item $\forall b_0,\, b_1,\, \dots,\, b_n \in \Simp(\Cat{C})$ であって，$b_0=a_0,\, b_n=a_n$ を充たすもの
        \item $\forall c_1,\, \dots,\, c_n \in \Simp(\Cat{C})$
        \item $\forall \varphi_j \in \expval{a_{j-1}^* ,\, c_j ,\, a_j},\; \forall \psi_j \in \expval{b_j^* ,\, c_j^* ,\, b_{j-1}}$
    \end{itemize}
    に対して，以下が成り立つ：
    \begin{align} 
    &
    \begin{tikzpicture}[baseline={([yshift=-.5ex]current bounding box.center)}]
        ...
        \path coordinate (b_0)
            foreach \r in {1,...,4} {
                +(30:{\fac*\r}) coordinate (b_\r)
            }
            ++(0,{5*\fac}) coordinate (u_0)
            foreach \r in {1,...,4} {
                +(-30:{\fac*\r}) coordinate (u_\r)
            }
        ;
        \draw[->-=.5] (b_0) -- node[midway,below] {$a_1$} (b_1);
        \draw[->-=.5] (b_1) -- node[midway,below] {$a_2$} (b_2);
        \draw[->-=.5] (b_2) -- node[midway,below right] {$a_{n-2}$} (b_3);
        \draw (b_1) -- (b_3) node[midway,sloped,fill=white] {$\cdots$};
        \draw[->-=.5] (b_3) -- node[midway,below right] {$a_{n-1}$} (b_4);
        \draw[-<-=.5] (u_0) -- node[midway,above] {$b_1$} (u_1);
        \draw[-<-=.5] (u_1) -- node[midway,above] {$b_2$} (u_2);
        \draw[-<-=.5] (u_2) -- node[midway,above right] {$b_{n-2}$} (u_3);
        \draw (u_1) -- (u_3) node[midway,sloped,fill=white] {$\cdots$};
        \draw[-<-=.5] (u_3) -- node[midway,above right] {$b_{n-1}$} (u_4);
        \draw[->-=.5] (b_0) -- node[midway,left] {$c_1$} (u_0);
        \draw[->-=.5] (b_1) -- node[midway,left] {$c_2$} (u_1);
        \node at (barycentric cs:b_2=1,u_2=1) {$\cdots$};
        \draw[->-=.5] (b_3) -- node[midway,left] {$c_{n-1}$} (u_3);
        \draw[->-=.5] (b_4) -- node[midway,left] {$c_n$} (u_4);
        \draw[->-=.5] (b_4) to[out=30,in=-30,looseness=2] node[midway,right] {$a_n=b_n$} (u_4);
        \draw[->-=.5] (u_0) to[out=180,in=180] node[midway,left] {$a_0=b_0$} (b_0);
        \node[circlenode] at (b_0) {$\varphi_1$};
        \node[circlenode] at (b_1) {$\varphi_2$};
        \node[circlenode] at (b_3) {$\varphi_{n-1}$};
        \node[circlenode] at (b_4) {$\varphi_n$};
        \node[circlenode] at (u_0) {$\psi_1$};
        \node[circlenode] at (u_1) {$\psi_2$};
        \node[circlenode] at (u_3) {$\psi_{n-1}$};
        \node[circlenode] at (u_4) {$\psi_n$};
    \end{tikzpicture}% 
    \\
    &= \dim_p (a_0) \prod_{j=1}^n \delta_{a_{j-1},\, b_{j-1}} \frac{\omega (\psi_j,\, \varphi_j)}{\dim_p (a_{j-1})}
    \end{align}    
\end{mylem}

\begin{proof}
    補題\ref{lem:TVBW-1}-(1) を繰り返し用いることで，
    \begin{align}
    &\begin{tikzpicture}[baseline={([yshift=-.5ex]current bounding box.center)}]
        \pgfmathsetmacro{\fac}{1.75}
        \path coordinate (b_0)
            foreach \r in {1,...,4} {
                +(30:{\fac*\r}) coordinate (b_\r)
            }
        ...
        \draw[->-=.5] (b_0) -- node[midway,below] {$a_1$} (b_1);
        \draw[->-=.5] (b_1) -- node[midway,below] {$a_2$} (b_2);
        \draw[->-=.5] (b_2) -- node[midway,below] {$a_{n-2}$} (b_3);
        \draw (b_1) -- (b_3) node[midway,sloped,fill=white] {$\cdots$};
        \draw[->-=.5] (b_3) -- node[midway,below right] {$a_{n-1}$} (b_4);
        \draw[-<-=.5] (u_0) -- node[midway,above] {$b_1$} (u_1);
        \draw[-<-=.5] (u_1) -- node[midway,above] {$b_2$} (u_2);
        \draw[-<-=.5] (u_2) -- node[midway,above] {$b_{n-2}$} (u_3);
        \draw (u_1) -- (u_3) node[midway,sloped,fill=white] {$\cdots$};
        \draw[-<-=.5] (u_3) -- node[midway,above right] {$b_{n-1}$} (u_4);
        \draw[->-=.5] (b_0) -- node[midway,left] {$c_1$} (u_0);
        \draw[->-=.5] (b_1) -- node[midway,left] {$c_2$} (u_1);
        ...
        \draw[->-=.5] (b_3) -- node[midway,left] {$c_{n-1}$} (u_3);
        \draw[->-=.5] (b_4) -- node[midway,left] {$c_n$} (u_4);
        \draw[->-=.5] (b_4) to[out=30,in=-30,looseness=2] node[midway,right] {$a_n$} (u_4);
        \draw[->-=.5] (u_0) to[out=180,in=180] node[midway,left] {$a_0$} (b_0);
        \node[circlenode] at (b_0) {$\varphi_1$};
        \node[circlenode] at (b_1) {$\varphi_2$};
        \node[circlenode] at (b_3) {$\varphi_{n-1}$};
        \node[circlenode] at (b_4) {$\varphi_n$};
    ...
    \end{tikzpicture}% 
    \\
    &= \delta_{a_{n-1},\, b_{n-1}} \frac{\omega (\psi_n,\, \varphi_n)}{\dim_p (a_{n-1})} 
        \begin{tikzpicture}[baseline={([yshift=-.5ex]current bounding box.center)}]
        \pgfmathsetmacro{\fac}{1.75}
        \path coordinate (b_0)
            foreach \r in {1,...,4} {
        ...
        \draw[->-=.5] (b_3) -- node[midway,left] {$c_{n-1}$} (u_3);
        % \draw[->-=.5] (b_4) -- node[midway,left] {$c_n$} (u_4);
        \draw[->-=.5] (b_3) to[out=30,in=-30,looseness=1.5] node[midway,right] {$a_{n-1}$} (u_3);
        \draw[->-=.5] (u_0) to[out=180,in=180,looseness=0.8] node[midway,left] {$a_0$} (b_0);
        \node[circlenode] at (b_0) {$\varphi_1$};
        \node[circlenode] at (b_1) {$\varphi_2$};
        \node[circlenode] at (b_3) {$\varphi_{n-1}$};
        % \node[circlenode] at (b_4) {$\varphi_n$};
...
いま，$\forall a,\, b,\, c \in \Simp(\Cat{C})$ に対して，有限次元 $\mathbb{K}$-ベクトル空間 $\Hom{\Cat{C}} (a,\, b \otimes c) \cong \expval{b,\, c,\, a^*}$ の基底を1つ選んできて固定する．それを $\bm{\Basis (a,\, b \otimes c)}$ と書く．
\hyperref[def:spherical]{球状フュージョン圏}の\hyperref[def:F-symbol]{\textbf{F-シンボル}} (F-symbol) とは，
\begin{itemize}
    \item $x,\, y,\, z,\, u,\, \textcolor{red}{a},\, \textcolor{blue}{b} \in \Simp(\Cat{C})$
    \item $\textcolor{red}{\varphi_1{}_{\mu_1}} \in \Basis (u,\, \textcolor{red}{a} \otimes z),\, \textcolor{red}{\varphi_2{}_{\mu_2}} \in \Basis (\textcolor{red}{a},\, x \otimes y)$
    \item $\textcolor{blue}{\varphi_3{}_{\mu_3}} \in \Basis (u,\, x \otimes \textcolor{blue}{b}),\, \textcolor{blue}{\varphi_4{}_{\mu_4}} \in \Basis (\textcolor{red}{b},\, y \otimes z)$
\end{itemize}
を指定することによって定まる $(F^{xyz}_u)_{(\textcolor{red}{a};\, \textcolor{red}{\varphi_1{}_{\mu_1}},\, \textcolor{red}{\varphi_2{}_{\mu_2}}),\, (\textcolor{blue}{b};\, \textcolor{blue}{\varphi_3{}_{\mu_3}},\, \textcolor{blue}{\varphi_4{}_{\mu_4}})} \in \mathbb{K}$ のことであった：
\begin{align}
    \label{eq:F-symbol}
    \begin{tikzpicture}[baseline={([yshift=-.5ex]current bounding box.center)}]
        \pgfmathsetmacro{\fac}{1.5}
        \path coordinate (f_1)
        +(60:{2*\fac}) coordinate (z)
        ++(120:\fac) coordinate (f_2)
        +(60:\fac) coordinate (y)
        +(120:\fac) coordinate (x)
        ;
        \draw[-<-=.5] (f_1) -- node[midway,right] {$u$} ++(-90:\fac);
        \draw[->-=.5,red] (f_1) -- node[midway,below left] {$a$} (f_2);
        \draw[->-=.5] (f_1) -- node[midway,below right] {$z$} (z);
        \draw[->-=.5] (f_2) -- node[midway,below right] {$y$} (y);
        \draw[->-=.5] (f_2) -- node[midway,below left] {$x$} (x);
        \node[circlenode] at (f_1) {$\textcolor{red}{\varphi_1{}_{\mu_1}}$};
        \node[circlenode] at (f_2) {$\textcolor{red}{\varphi_2{}_{\mu_2}}$};
    \end{tikzpicture}
    &\eqqcolon \sum_{
        \substack{
            \textcolor{blue}{b} \in \Simp(\Cat{C}) \\ 
            \textcolor{blue}{\varphi_3{}_{\mu_3}} \in \Basis (u,\, x \otimes \textcolor{blue}{b}) \\ 
            \textcolor{blue}{\varphi_4{}_{\mu_4}} \in \Basis (\textcolor{blue}{b},\, y \otimes z)
        }
    }
    (F^{xyz}_u)_{(\textcolor{red}{a};\, \textcolor{red}{\varphi_1{}_{\mu_1}},\, \textcolor{red}{\varphi_1{}_{\mu_1}}),\, (\textcolor{blue}{b};\, \textcolor{blue}{\varphi_3{}_{\mu_3}},\, \textcolor{blue}{\varphi_4{}_{\mu_4}})}
    \begin{tikzpicture}[baseline={([yshift=-.5ex]current bounding box.center)}]
        \pgfmathsetmacro{\fac}{1.5}
        \path coordinate (f_3)
        +(120:{2*\fac}) coordinate (x)
        ++(60:\fac) coordinate (f_4)
        +(60:\fac) coordinate (z)
        +(120:\fac) coordinate (y)
        ;
        \draw[-<-=.5] (f_3) -- node[midway,right] {$u$} ++(-90:\fac);
        \draw[->-=.5,blue] (f_3) -- node[midway,below right] {$b$} (f_4);
        \draw[->-=.5] (f_3) -- node[midway,below left] {$x$} (x);
        \draw[->-=.5] (f_4) -- node[midway,below left] {$y$} (y);
        \draw[->-=.5] (f_4) -- node[midway,below right] {$z$} (z);
        \node[circlenode] at (f_3) {$\textcolor{blue}{\varphi_3{}_{\mu_3}}$};
        \node[circlenode] at (f_4) {$\textcolor{blue}{\varphi_4{}_{\mu_4}}$};
    \end{tikzpicture}
\end{align}

\begin{mylem}[label=lem:trivalent-Lk]{三角形分割の絡みグラフと $F$-シンボル}
    \begin{align}
        \begin{tikzpicture}[baseline={([yshift=-.5ex]current bounding box.center)}]
            \path coordinate (x)
            +(30:2) coordinate (y)
            ...
            +(-90:2) coordinate (w)
            ;
            \draw[->-=.5] (x) -- node[midway,below right] {$y$} (y);
            \draw[->-=.5] (x) -- node[midway,below] {$x$} (z);
            \draw[-<-=.5,red] (x) -- node[midway,right] {$a$} (w);
            \draw[-<-=.5] (y) arc (30:-90:2) node[midway,right] {$z$};
            \draw[->-=.5,blue] (y) arc (30:150:2) node[midway,above] {$b$};
            \draw[-<-=.5] (w) arc (-90:-210:2) node[midway,left] {$u$};
            \node[circlenode] at (x) {$\textcolor{red}{\varphi_2{}_{\mu_2}}$};
            \node[circlenode] at (y) {$\textcolor{blue}{\varphi_4{}^{\mu_4}}$};
            \node[circlenode] at (z) {$\textcolor{blue}{\varphi_3{}^{\mu_3}}$};
            \node[circlenode] at (w) {$\textcolor{red}{\varphi_1{}_{\mu_1}}$};
        \end{tikzpicture}
        &= \frac{1}{\dim_p (\textcolor{blue}{b})} (F^{xyz}_u)_{(\textcolor{red}{a};\, \textcolor{red}{\varphi_1{}_{\mu_1}},\, \textcolor{red}{\varphi_2{}_{\mu_2}}),\, (\textcolor{blue}{b};\, \textcolor{blue}{\varphi_3{}_{\mu_3}},\, \textcolor{blue}{\varphi_4{}_{\mu_4}})}
    \end{align}
    ただし，$\textcolor{blue}{\varphi_3{}^{\mu_3}} \in \Basis (x \otimes \textcolor{blue}{b},\, u),\; \textcolor{blue}{\varphi_4{}^{\mu_4}} \in \Basis (y \otimes z,\; \textcolor{blue}{b})$ はそれぞれ $\textcolor{blue}{\varphi_3{}_{\mu_3}} \in \Basis (u,\, x \otimes \textcolor{blue}{b}),\, \textcolor{blue}{\varphi_4{}_{\mu_4}} \in \Basis (\textcolor{blue}{b},\, y \otimes z)$ の，\hyperref[lem:nondegen-pairing]{ペアリング} $\omega$ に関する双対基底である．
\end{mylem}

\begin{proof}
    式\eqref{eq:F-symbol}のストリング図式の上から
    \begin{align}
        \begin{tikzpicture}[baseline={([yshift=-.5ex]current bounding box.center)}]
            \pgfmathsetmacro{\fac}{1.75}
            \path coordinate (f_3)
            ...
            ;
            \draw[->-=.5] (f_3) -- node[midway,right] {$u$} ++(90:\fac);
            \draw[->-=.5] (x) -- node[midway,left] {$x$} (f_3);
            \draw[->-=.5,blue] (f_4) -- node[midway,right] {$b$} (f_3);
            \draw[->-=.5] (z) -- node[midway,right] {$z$} (f_4);
            \draw[->-=.5] (y) -- node[midway,left] {$y$} (f_4);
            \node[circlenode] at (f_3) {$\textcolor{blue}{\varphi_3{}^{\mu_3}}$};
            \node[circlenode] at (f_4) {$\textcolor{blue}{\varphi_4{}^{\mu_4}}$};
        \end{tikzpicture}
    \end{align}
    を合成して\hyperref[def:trace]{トレース}をとると，左辺は
    \begin{align}
    \begin{tikzpicture}[baseline={([yshift=-.5ex]current bounding box.center)}]
        \pgfmathsetmacro{\fac}{1.75}
        \path coordinate (f_1)
        +(60:{2*\fac}) coordinate (f_4)
        ++(120:\fac) coordinate (f_2)
        +(60:{2*\fac}) coordinate (f_3)
        ;
        \path[use as bounding box]
          ($(f_1)+(0,-\fac)$) ($(f_2)+(-0.5*\fac,0)$) ($(f_3)+(0,\fac)$) ($(f_4)+(1.1*\fac,0)$)
        ;
        \draw[->-=.5] (f_1) to[out=60,in=-60] node[midway,right] {$z$} (f_4);
        \draw[->-=.5,red] (f_1) -- node[midway,below left] {$a$} (f_2);
        \draw[->-=.5] (f_2) to[out=60,in=-120] node[midway,below] {$y$} (f_4);
        \draw[->-=.5] (f_2) to[out=120,in=-120] node[midway,above left] {$x$} (f_3);
        \draw[->-=.5,blue] (f_4) -- node[midway,above right] {$b$} (f_3);
        \draw[->-=.5] (f_3) to[out=60,in=-60,looseness=4] node[midway,right] {$u$} (f_1);
        % \draw[->-=.5] (f_3) .. controls +(2,4) and +(4,-4) .. (f_1);
        \node[circlenode] at (f_1) {$\textcolor{red}{\varphi_1{}_{\mu_1}}$};
        \node[circlenode] at (f_2) {$\textcolor{red}{\varphi_2{}_{\mu_2}}$};
        \node[circlenode] at (f_4) {$\textcolor{blue}{\varphi_4{}^{\mu_4}}$};
        \node[circlenode] at (f_3) {$\textcolor{blue}{\varphi_3{}^{\mu_3}}$};
    ...
    \begin{tikzpicture}[baseline={([yshift=-.5ex]current bounding box.center)}]
        ...
        ;
        \path[use as bounding box]
          ($(f_1)+(0,-\fac)$) ($(f_2)+(-1.1*\fac,0)$) ($(f_3)+(0,\fac)$) ($(f_4)+(0.5*\fac,0)$)
        ;
        \draw[->-=.5] (f_1) to[out=60,in=-60] node[midway,right] {$z$} (f_4);
        \draw[->-=.5,red] (f_1) -- node[midway,below left] {$a$} (f_2);
        \draw[->-=.5] (f_2) to[out=60,in=-120] node[midway,below] {$y$} (f_4);
        \draw[->-=.5] (f_2) to[out=120,in=-120] node[midway,above left] {$x$} (f_3);
        \draw[->-=.5,blue] (f_4) -- node[midway,above right] {$b$} (f_3);
        \draw[->-=.5] (f_3) to[out=120,in=-120,looseness=4] node[midway,left] {$u$} (f_1);
        % \draw[->-=.5] (f_3) .. controls +(2,4) and +(4,-4) .. (f_1);
        \node[circlenode] at (f_1) {$\textcolor{red}{\varphi_1{}_{\mu_1}}$};
        \node[circlenode] at (f_2) {$\textcolor{red}{\varphi_2{}_{\mu_2}}$};
        \node[circlenode] at (f_4) {$\textcolor{blue}{\varphi_4{}^{\mu_4}}$};
        \node[circlenode] at (f_3) {$\textcolor{blue}{\varphi_3{}^{\mu_3}}$};
    \end{tikzpicture} &&\because\quad \text{\hyperref[def:spherical]{spherical}} \\
    &=
        \begin{tikzpicture}[baseline={([yshift=-.5ex]current bounding box.center)}]
            \path coordinate (x)
            +(30:2) coordinate (y)
        ...
        \end{tikzpicture}
    \end{align}
    と計算できる．一方右辺は，補題\ref{lem:trivalent-ladder}から
    \begin{align}
    &\sum_{
        \substack{
            \textcolor{DarkGreen}{c} \in \Simp(\Cat{C}) \\ 
            \textcolor{DarkGreen}{\varphi_3{}_{\nu_3}} \in \Basis (u,\, x \otimes \textcolor{DarkGreen}{c}) \\ 
            \textcolor{DarkGreen}{\varphi_4{}_{\nu_4}} \in \Basis (\textcolor{DarkGreen}{c},\, y \otimes z)
        }
    }
        (F^{xyz}_u)_{(\textcolor{red}{a};\, \textcolor{red}{\varphi_1{}_{\mu_1}},\, \textcolor{red}{\varphi_1{}_{\mu_1}}),\, (\textcolor{DarkGreen}{c};\, \textcolor{DarkGreen}{\varphi_3{}_{\nu_3}},\, \textcolor{DarkGreen}{\varphi_4{}_{\nu_4}})}
    \begin{tikzpicture}[baseline={([yshift=-.5ex]current bounding box.center)}]
        \pgfmathsetmacro{\fac}{1.75}
        \path coordinate (f_1)
        ++(60:\fac) coordinate (f_2)
        ++(90:\fac) coordinate (f_3)
        +(120:\fac) coordinate (f_4)
        ;
        \path[use as bounding box]
          ($(f_1)+(-0.7*\fac,-\fac)$) ($(f_2)+(1.5*\fac,0)$) ($(f_3)+(1.5*\fac,0)$) ($(f_4)+(-0.7*\fac,\fac)$)
        ;
        \draw[->-=.5,DarkGreen] (f_1) -- node[midway,below right] {$c$} (f_2);
        \draw[->-=.5,blue] (f_3) -- node[midway,above right] {$b$} (f_4);
        \draw[->-=.5] (f_1) to[out=120,in=-120] node[midway,left] {$x$} (f_4);
        \draw[->-=.5] (f_2) to[out=120,in=-120] node[midway,left] {$y$} (f_3);
        \draw[->-=.5] (f_2) to[out=60,in=-60] node[midway,right] {$z$} (f_3);
        \draw[->-=.5] (f_4) to[out=60,in=-60,looseness=4] node[midway,right] {$u$} (f_1);
        \node[circlenode] at (f_1) {$\textcolor{DarkGreen}{\varphi_3{}_{\nu_3}}$};
        \node[circlenode] at (f_2) {$\textcolor{DarkGreen}{\varphi_4{}_{\nu_4}}$};
        \node[circlenode] at (f_4) {$\textcolor{blue}{\varphi_3{}^{\mu_3}}$};
        \node[circlenode] at (f_3) {$\textcolor{blue}{\varphi_4{}^{\mu_4}}$};
    \end{tikzpicture} \\
    &= \sum_{
        \substack{
            \textcolor{DarkGreen}{c} \in \Simp(\Cat{C}) \\ 
    ...
    \begin{tikzpicture}[baseline={([yshift=-.5ex]current bounding box.center)}]
        \pgfmathsetmacro{\fac}{1.75}
        \path coordinate (f_1)
        ++(30:\fac) coordinate (f_2)
        ++(90:\fac) coordinate (f_3)
        +(150:\fac) coordinate (f_4)
        ;
        \draw[->-=.5,DarkGreen] (f_1) -- node[midway,below right] {$c$} (f_2);
        \draw[->-=.5,blue] (f_3) -- node[midway,above right] {$b$} (f_4);
        \draw[->-=.5] (f_1) -- node[midway,left] {$x$} (f_4);
        \draw[->-=.5] (f_2) -- node[midway,left] {$y$} (f_3);
        \draw[->-=.5] (f_2) to[out=30,in=-30,looseness=2] node[midway,right] {$z$} (f_3);
        \draw[->-=.5] (f_4) to[out=180,in=-180] node[midway,left] {$u$} (f_1);
        \node[circlenode] at (f_1) {$\textcolor{DarkGreen}{\varphi_3{}_{\nu_3}}$};
        \node[circlenode] at (f_2) {$\textcolor{DarkGreen}{\varphi_4{}_{\nu_4}}$};
        \node[circlenode] at (f_4) {$\textcolor{blue}{\varphi_3{}^{\mu_3}}$};
        \node[circlenode] at (f_3) {$\textcolor{blue}{\varphi_4{}^{\mu_4}}$};
    ...
    (F^{xyz}_u)_{(\textcolor{red}{a};\, \textcolor{red}{\varphi_1{}_{\mu_1}},\, \textcolor{red}{\varphi_1{}_{\mu_1}}),\, (\textcolor{DarkGreen}{c};\, \textcolor{DarkGreen}{\varphi_3{}_{\nu_3}},\, \textcolor{DarkGreen}{\varphi_4{}_{\nu_4}})}
    \dim_p(u)\delta_{\textcolor{DarkGreen}{c},\, \textcolor{blue}{b}} \frac{\delta_{\textcolor{DarkGreen}{\nu_3},\, \textcolor{blue}{\mu_3}}\delta_{\textcolor{DarkGreen}{\nu_4},\, \textcolor{blue}{\mu_4}}}{\dim_p (u) \dim_p (\textcolor{blue}{b})} 
    \\
    &= \frac{1}{\dim_p (\textcolor{blue}{b})} (F^{xyz}_u)_{(\textcolor{red}{a};\, \textcolor{red}{\varphi_1{}_{\mu_1}},\, \textcolor{red}{\varphi_2{}_{\mu_2}}),\, (\textcolor{blue}{b};\, \textcolor{blue}{\varphi_3{}_{\mu_3}},\, \textcolor{blue}{\varphi_4{}_{\mu_4}})}
    \end{align}
    と計算できるので，示された．
