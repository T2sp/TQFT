...
\begin{mydefph}[label=def:gauging]{ゲージ化}
    ...
    　$T$ の\textbf{ゲージ化} (gauging) とは，圏 $\Cat{C}$ における\hyperref[def:separable-alg]{分離可能}な\hyperref[def:sym-Frobenius]{対称Frobenius代数} $(A,\, \mu,\, \upsilon,\, \Delta,\, \epsilon)$ を1つ指定することを言う．
    % \begin{itemize}
    %     \item 
...
次の命題\ref{prop:gaiging-graph-invariance}より，$A$ の\hyperref[def:separable-alg]{分離可能性}と\hyperref[def:sym-Frobenius]{対称Frobenius代数}は，このストリング図式 $\mathbb{F}_A (K)$ が位相不変量であること（i.e. ゲージ不変性）を保証するものだと分かる．
\begin{figure}[H]
    \centering
    \begin{tikzpicture}
        \path coordinate (x_1)
...
\begin{proof}
    ...
    \begin{align}
        ...
        &= \begin{tikzpicture}[baseline={([yshift=-.5ex]current bounding box.center)}]
            ...
            ;
            \draw[->-=.5] (a) -- (b);
            \draw[->-=.5] (b) to[out=90,in=120] (c);
            \draw[->-=.5] (b) to[out=-30,in=-90] (c);
            \draw[->-=.5] (c) -- +(1,1);
            \draw[-<-=.5] (a) -- +(180:1);
            \draw[-<-=.5] (a) -- +(-90:1);
        ...
        &= \begin{tikzpicture}[baseline={([yshift=-.5ex]current bounding box.center)}]
            ...
            \draw[-<-=.5] (g) -- +(150:1);
            \draw[-<-=.5] (g) -- +(-90:1);
        \end{tikzpicture} &&\because\quad \text{\hyperref[def:separable-alg]{分離可能性}}
    \end{align}
    により不変であることがわかった．
\end{proof}
% 以下では，トポロジカル演算子は常に 
...
\begin{align}
    ...
    &= \abs{G} \Id_A (e_g)
\end{align}
が成り立つので組 $(A,\, \mu,\, \upsilon,\, \Delta,\, \epsilon)$ は\hyperref[def:separable-alg]{分離可能}であり，
\begin{align}
...
\begin{align}
    ...
    &= e_g
\end{align}
が成り立つので\hyperref[def:sym-Frobenius]{対称Frobenius代数}であることがわかった．
以上の考察から，組 $(A,\, \mu,\, \upsilon,\, \Delta,\, \epsilon)$ は定義\ref{def:gauging}の意味で対称性 $\VEC{G}$ のゲージ化である．

    　従来の意味のゲージ化とは，\hyperref[def:p-form-sym]{0-form symmetry}の全てのトポロジカル演算子のあらゆる可能な配位に関する足し上げであった．
