...
\begin{align}
    Z[A_1, + \dd{\lambda_0}] = e^{\iunit \int_{\mfdcal{M}{D+1}} \mathcal{A}[A_1,\, \lambda_0]} Z[A_1] \WHERE  \mathcal{A}[A_1,\, 0]  = 0
\end{align}
...
\end{align}
この $\dd{\mathcal{A}[A_1,\, \lambda_0]}$ を $\widehat{A}[A_1 + \dd{\lambda_0}] \in \Omega^{D+2}(\mfdcal{N}{D+2})$ と見做すのである．従って，元の理論の分配関数を
\begin{align}
    \widehat{Z}[A_1] \coloneqq Z[A_1] e^{\iunit \int_{\mfdcal{N}{D+2}} \widehat{A}[A_1]}
\end{align}
と再定義すれば，アノマラスな位相を消去できる．位相 $e^{\iunit \int_{\mfdcal{N}{D+2}} \widehat{A}[A_1]}$ は\textbf{SPT}と呼ばれる．
