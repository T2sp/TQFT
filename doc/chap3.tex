...
$M$ の多面体分割 $(P,\, P^{(1)})$ の任意の頂点 $v \in \mathsf{V}(P^{(1)})$ に対して，$v$ の閉球近傍 $B_v \subset M$ を十分小さく取ることで右下図のようなグラフを得る：
\begin{align}
    \label{def:linkedG}
    \begin{tikzpicture}[baseline={([yshift=-.5ex]current bounding box.center)}]
        \coordinate[bullet, label=above:$v$] (O) at (0,0,0);
        \shade[ball color = lightgray,opacity = 0.5] (O) circle (1.6);
        \node at (2,2,0) {$B_v$};
        \draw[->] (2,2,0) -- ++(-1,-1,0);
...
\begin{description}
    ...
    まず，\underline{無向}グラフ $P^{(1)}$ に勝手な向き $\mathcal{O} \in \{\pm\}^{\abs{\mathsf{E}(P^{(1)})}}$ を与えて有向グラフ $P^{(1)}{}^{\mathcal{O}}$ にする．
    そして\underline{有向}辺 $\tilde{e} \in \mathsf{E}(P^{(1)}{}^{\mathcal{O}})$ の\hyperref[def:polyhedron]{分岐}の集合 $P^{(1)}_{\tilde{e}}$ に，右ネジの法則に従うサイクリックな順序 $<$ を与える．
    さらに $\forall R \in P_{\tilde{e}}^{(1)}$ に対して，$\epsilon_R \in \{\pm\}$ を，$M$ から誘導される向きと $\tilde{e}$ の向きが整合しているならば $+$，不整合ならば $-$ となるように定義することで，
    ...
    \end{align}
    が成り立つ．よって補題\ref{lem:nondegen-pairing}の非退化な双線形形式 $\omega_{\Gamma,\, e} \colon \mathcal{H} (\tilde{e}_2;\, \Gamma) \otimes_{\mathbb{K}} \mathcal{H} (\tilde{e}_1;\, \Gamma) \lto \mathbb{K}$ を考えることができ，
    ...
    $\forall v \in \mathsf{V}(P^{(1)})$ に対しては，十分小さな開球近傍 $B_v$ およびその\hyperref[def:linkedG]{絡みグラフ} $\mathrm{Lk}(v;\, P) \subset \partial B_v$ をとる．
    $\forall a \in \mathsf{E}\bigl(\mathrm{Lk}(v;\, P)\bigr)$ はある1つの\hyperref[def:polyhedron]{領域} $R_a \in \mathrm{Reg}(P)$ に含まれているから，$a$ には $\Gamma(R_a) \in \Simp(\Cat{C})$ を割り当てる．
    % $\partial B_v$ には $M$ から誘導される向きを入れ，
    $a$ には $R_a \setminus \Int(B_a)$ から誘導される向きを入れることで，$\mathrm{Lk}(v;\, P)$ を\underline{有向グラフ}にする．
    
    　ここで，$\forall x \in \mathsf{V}\bigl(\mathrm{Lk}(v;\, P)\bigr)$ に対して勝手な $\alpha_x \in \expval{\Gamma(a_1)^{\varepsilon_{a_1}},\, \dots,\, \Gamma(a_m)^{\varepsilon_{a_m}}} \eqqcolon \mathcal{H}_x\bigl(\mathrm{Lk}(v;\, P);\, \Gamma\bigr)$ を割り当てることで，絡みグラフ $\mathrm{Lk}(v;\, P) \subset \partial B_v$ をストリング図式に読み替える．
    ただし，$\varepsilon_{a_i}$ は\underline{有向辺} $a_i \in \mathsf{E} \bigl( \mathrm{Lk}(v;\, P) \bigr)$ が $v$ を始点に持つならば $+$，終点に持つならば $-$ を与える．
    特に $M$ が境界を持たないことから絡みグラフは配位数が $2$ 以上なので，このようにして得られたストリング図式は端点を持たない．
    i.e. ストリング図式に対応する射は $\mathbb{F}_{\Cat{C},\, \Gamma} \bigl( \mathrm{Lk}(v;\, P) \bigr)\bigl( \bigotimes_{x \in \mathsf{V} \bigl( \mathrm{Lk}(v;\, P) \bigr) } \alpha_x \bigr) \in \Hom{\Cat{C}} (1,\, 1)$ と書ける．これを $\mathbb{K}$-線形に拡張することで
    ，$\mathbb{K}$-線型写像
    \begin{align}
        \mathbb{F}_{\Cat{C},\, \Gamma} \bigl( \mathrm{Lk}(v;\, P) \bigr) \in \Hom{\VEC{\mathbb{K}}} \Bigl( \mathcal{H}\bigl(\mathrm{Lk}(v;\, P);\, \Gamma\bigr),\, \mathbb{K}\Bigr) 
    \end{align}
    を得る．ただし，
    \begin{align}
        \mathcal{H}\bigl(\mathrm{Lk}(v;\, P) \bigr) \coloneqq \bigotimes_{x \in \mathsf{V} \bigl( \mathrm{Lk}(v;\, P) \bigr)} \mathcal{H}_x\bigl(\mathrm{Lk}(v;\, P) \bigr)
    \end{align}
    とおいた．
    ...
    \begin{align}
        \mathcal{H}\bigl(\mathrm{Lk}(v;\, P);\, \Gamma\bigr) \cong \bigotimes_{\tilde{e}_v;\, v\, \text{を始点に持つ}} \mathcal{H}(\tilde{e}_v;\, \Gamma)
    \end{align}
    に注意すると
    \begin{align}
        \bigotimes_{v \in \mathsf{V}(P^{(1)})}\Hom{\VEC{\mathbb{K}}} \Bigl( \mathcal{H}\bigl(\mathrm{Lk}(v;\, P);\, \Gamma\bigr),\, \mathbb{K}\Bigr)  
        &\cong \bigotimes_{v} \bigotimes_{\tilde{e}_v;\, v\, \text{を始点に持つ}} \Hom{\VEC{\mathbb{K}}} \left( \mathcal{H}(\tilde{e}_v;\, \Gamma),\, \mathbb{K}\right) \\
        &\cong \Hom{\VEC{\mathbb{K}}} \bigl(\mathcal{H}(\Gamma),\, \mathbb{K}\bigr)
    \end{align}
    が成り立つことが分かる．よって
    \begin{align}
        z(\Gamma;\, P) \coloneqq \bigotimes_{v}\mathbb{F}_{\Cat{C},\, \Gamma} \bigl( \mathrm{Lk}(v;\, P) \bigr) \in \Hom{\VEC{\mathbb{K}}} \bigl(\mathcal{H}(c),\, \mathbb{K}\bigr)
    \end{align}
...
\begin{mydef}[label=def:TV-closed]{閉多様体におけるTV不変量}
    ...
    \begin{align}
        \irm{Z}{\text{TV},\Cat{C}}(M) \coloneqq \left( \sum_{a \in \mathrm{Simp}(\Cat{C})} \dim(a)^2\right)^{-v(M)} \sum_{\Gamma \colon \mathrm{Reg} \lto \Simp(\Cat{C})} Z(\Gamma;\, P) \prod_{R \in \mathrm{Reg}(P)} \dim \bigl( \Gamma(R) \bigr)^{\chi(R)}
    \end{align}
...
\begin{mytheo}[label=thm:TV-closed]{}
    Turaev不変量 $\irm{Z}{\text{TV},\Cat{C}}(M)$ は\hyperref[def:polytope-decomp]{多面体分割} $(P,\, P^{(1)})$ の取り方によらない．
\end{mytheo}
