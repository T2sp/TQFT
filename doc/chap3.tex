...
$\mathcal{M}$ の $p$ 次元部分多様体\footnote{コンパクトかつ向き付け可能だとしておけば問題が起こりにくい．} $N \subset \mathcal{M}$ 上で $p$-形式 $\omega \in \Omega^p(\mathcal{M})$ を積分する場合，包含写像 $\iota \colon N \hookrightarrow \mathcal{M}$ による引き戻し $\iota^* \colon \Omega^p(\mathcal{M}) \lto \Omega^p(N)$ を用いて
\begin{align}
    \int_{N} \omega \coloneqq \int_N \iota^*\omega
\end{align}
と定義する．このとき，Poincar\"{e}双対を考えることで
\begin{align}
    \int_N \omega = \int_{\mathcal{M}} \omega \wedge \delta(N)
\end{align}
を充たす $\delta(N) \in \Omega^{D+1-p}(\mathcal{M})$ （\textbf{デルタ関数 $p$-形式}と呼ぶ）の存在がわかる\footnote{厳密な扱いは~\cite[p.270]{Nicolaescu2022lecture}を参照．ここでは雑に扱う．}．
Stokesの定理から
\begin{align}
    \int_{\mathcal{M}} \omega \wedge \delta(\partial N) 
    &= \int_{\partial N} \omega 
    = \int_{N} \dd{\omega} 
    = \int_\mathcal{M} \dd{\omega} \wedge \delta(N) \\
    &= \int_{\mathcal{M}} \dd \bigl( \omega \wedge \delta(N) \bigr) + (-1)^{p+1} \int_{\mathcal{M}} \omega \wedge \dd(\delta(N)) \\
    &= (-1)^{p+1} \int_{\mathcal{M}} \omega \wedge \dd (\delta(N))
\end{align}
がわかるので，
\begin{align}
    \dd{\delta(N)} = (-1)^{p+1} \delta(\partial N)
\end{align}
が成り立つ．
...
\begin{myexample}[label=ex:3-1]{$D+1$ 次元自由フェルミオン系}
    ...
    この系のon shellな対称性変換は，例えば $e^{\iunit \theta} \in \LU(1)$ による
    \begin{align}
    ...
    よってon shellな対称性変換 $\mathcal{T}$ に対応するNoetherカレント\eqref{eq:Noether-current}は
    \begin{align}
...
\exref{ex:3-1}は，荷電物体が $0$ 次元に分布していた．これを $p$ 次元に分布した物体に置き換えることで $p$-form symmetry の概念が得られる~\cite[p.10]{GaiottoKapustinSeibergWillett2015}：
...
\begin{mydef}[label=def:p-form-sym]{$p$-form symmetry}
    ...
    \begin{itemize}
        ...
        \item $\forall g,\, g' \in G$ に対して群の規則 (group law)
        \begin{align}
            U_g (\mathcal{M}^{(D-p)}) U_{g'} (\mathcal{M}^{(D-p)}) = U_{gg'} (\mathcal{M}^{(D-p)})
        \end{align}
        が成り立つ．
        \item 任意の $p$ 次元部分多様体 $\mathcal{C}^{(p)} \subset \mathcal{M}$ に対して，それに「絡む」十分小さな $D-p$-球 $S^{D-p} \subset \mathcal{M}$ に対して，\eqref{eq:0-form-topo-ops}と同じく経路積分の期待値の意味で
        \begin{align}
            U_g (S^{D-p}) V(\mathcal{C}^{(p)}) = R_g (V) V(\mathcal{C}^{(p)})
        \end{align}
...
\begin{myexample}[label=ex:3-2]{$3+1$ 次元 $\LU(1)$ ゲージ理論}
    ...
    \begin{align}
        ...
        \dd f &= 0
    \end{align}
    が言える（Maxwell方程式）．これをカレントの保存則と見做して，任意の $3-1 = 2$ 次元部分多様体 $\mathcal{M}^{(2)} \subset \mathcal{M}$ 上のトポロジカル演算子を
    \begin{align}
    ...
    　簡単のため $S \coloneqq \mathcal{M}^{(2)},\, C \coloneqq \mathcal{C}^{(1)}$ と略記する．今，絡み数が
    \begin{align}
        \mathrm{Link}(S,\, C) = \int_{\mathcal{M}} \delta(C) \wedge \delta(V) = 1
    \end{align}
    となるように $S,\, C$ をとる．ただし $C \cap V$ が一点集合で，かつ $\partial V = S$ を充たすような $3$ 次元部分多様体 $V \subset \mathcal{M}$ をとった．
    ゲージ場の変換
    \begin{align}
    ...
    \begin{align}
        ...
        & \lmto \exp \left( \iunit \int_C a + \iunit \theta \int_{\mathcal{M}}\delta(V) \wedge \delta(C) \right) \\
        & \lmto \exp \left( \iunit \int_C a - \iunit \theta \mathrm{Link}(S,\, C) \right) \\
        & \lmto e^{-\iunit \theta} \exp \left( \iunit \int_C a \right)
    \end{align}
