...
\begin{align}
    ...
    \begin{tikzpicture}[baseline={([yshift=-.5ex]current bounding box.center)}]
        ...
        ++ (-2,1) coordinate[label=above:$x_1$] (a)
        ++ (1,0) coordinate[label=above:$x_2$] (b)
        ++ (1,0) coordinate (c)
        ++ (1.5,0) coordinate[label=above:$\dots$] (c)
        ++ (1,0) coordinate (d)
        ++ (1,0) coordinate[label=above:$x_n$] (e);
...
\end{align}
のような略記をすることがある．
さらに，補題\ref{lem:nondegen-pairing}により，有限次元 $\mathbb{K}$-ベクトル空間 $\expval{x_1,\, \dots,\, x_n}$ の基底を $\{\varphi_\alpha\}$ と書くと，それの $\omega$ に関する双対基底 $\{\varphi^\alpha\}$ は $\expval{x_n^*,\, \dots,\, x_1^*}$ の基底をなす．
...
\begin{mylem}[label=lem:triangle-rule,breakable]{技術的な補題1}
    ...
    \begin{enumerate}
        \item \begin{align}
            \begin{tikzpicture}[baseline={([yshift=-.5ex]current bounding box.center)}]
                ...
                \fill[red, spath/use=inj];
            \end{tikzpicture}
            \quad = \dim_{\mathbb{K}} \Hom{\Cat{C}} (a,\, x) \quad 
            \begin{tikzpicture}[baseline={([yshift=-.5ex]current bounding box.center)}]
                \path coordinate (a)
                +(0,3) coordinate (b);
...
\begin{proof}
    ...
    を1つ固定する．
    \begin{enumerate}
        \item 補題\ref{lem:simple-hom}-(2) の \textsf{\textbf{(part-4)}} より明らか．
        \item 直和の特徴付けより
        \begin{align}
            \sum_{\alpha \in \Lambda} i_\alpha \circ p_\alpha = \Id_x
        \end{align}
        が成り立つ．
        \item 補題\ref{lem:simple-hom}より $\Hom{\Cat{C}} (1,\, a) = \Hom{\Cat{C}} (a,\, 1) = \delta_{a,\, 1}\mathbb{K}$ なので，(2) において 
        \begin{align}
            \begin{tikzpicture}[baseline={([yshift=-.5ex]current bounding box.center)}]
                \path coordinate[bullet] (a)
                +(0,3) coordinate (b);
...
\begin{proof}
    \begin{enumerate}
        ...
        \end{align}
        と求まる．
        \item 補題\ref{lem:triangle-rule}-(4) により，
        \begin{align}
            &\sum_{a \in \mathrm{Simp}(\Cat{C})} \dim_p(a)\;
            \begin{tikzpicture}[baseline={([yshift=-.5ex]current bounding box.center)}]
                \path coordinate (f)
            ...
            \begin{tikzpicture}[baseline={([yshift=-.5ex]current bounding box.center)}]
                ...
                ++(1,0) coordinate (g)
                +(0.5,1.5) coordinate (r3)
                +(1.5,1.5) coordinate (r2)
                +(2,1.5) coordinate (r1);
        ...
        \item (1) より明らか．
        \item (2) と補題\ref{lem:simple-hom}-(1) より従う．
    \end{enumerate}
...
彩色 $c \colon \mathrm{Reg}(P) \lto \Simp(\Cat{C})$ を1つ固定したとき，対応する $Z(c;\, P) \in \mathbb{K}$ を構成しよう．
まず $\mathsf{V}(P^{(1)}) = \emptyset$ のときは　$\abs{c} \coloneqq 1$ と定義する．
$\mathsf{V}(P^{(1)}) \neq \emptyset$ の場合を考える．
...
\begin{align}
    ...
    &\cong \Hom{\VEC{\mathbb{K}}} \bigl(\mathcal{H}(c),\, \mathbb{K}\bigr)
\end{align}
であるから，
\begin{align}
    \bigotimes_{v}\mathbb{F}_{\Cat{C}} \bigl( \mathrm{Lk}(v) \bigr) \in \Hom{\VEC{\mathbb{K}}} \bigl(\mathcal{H}(c),\, \mathbb{K}\bigr)
\end{align}
がわかる．これで\eqref{eq:contraction}を写像することで
\begin{align}
    Z(c;\, P) \coloneqq \left(\bigotimes_{v}\mathbb{F}_{\Cat{C}} \bigl( \mathrm{Lk}(v) \bigr)\right)(*_c) \in \mathbb{K}
\end{align}
を得る．
...
\begin{mydef}[label=def:TV-closed]{閉多様体におけるTV不変量}
    ...
    \begin{align}
        \irm{Z}{\text{TV},\Cat{C}}(M) \coloneqq \left( \sum_{a \in \mathrm{Simp}(\Cat{C})} \dim(a)^2\right)^{-v(M)} \sum_{c \colon \mathrm{Reg} \lto \Simp(\Cat{C})} \dim(c) Z(c;\, P)
    \end{align}
    で定義する．ただし $v(M)$ は $M \setminus P$ の連結成分の個数である．
\end{mydef}
