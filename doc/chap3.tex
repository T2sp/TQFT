...
\begin{description}
    ...
    元の理論 $S[\psi]$ に\textbf{ゲージ化} $\partial_\mu \lmto \partial_\mu + A_\mu$ の操作を施して得られた理論 $\irm{S}{gauged}[\psi,\, A]$ の分配関数
    \begin{align}
    ...
    \end{align}
    と定義したとき，ある $g \in G$ が存在してゲージ変換 $A \lmto A^g \coloneqq \Adj(g^{-1})(A) + g^* \theta$ の下で $Z[A] \neq Z[A^g]$ となること．$A$ は単なる外場なので，理論が矛盾したわけではない．
...
\end{align}
となる．i.e. $\mfdcal{M}{d}$ のアノマリーはある $d+1$ 次元のIFT $\mathcal{Z} \colon \COB_{d+2} \lto \HILB$ の分配関数である．この事実を\textbf{バルク-エッジ対応}と言う．
結局，アノマリーを列挙する問題は，$d+1$ 次元のIFTを列挙する問題に帰着されたと言うことである．

さらに，次の有名な予想がある~\cite{FreedHopkins_2021}：

\begin{myconj}[label=conj:Freed-Hopkins]{Freed-Hopkinsの予想}
    $d+1$ 次元IFTの空間の群構造は，$d+2$ 次元のbordism群のAnderson双対
    \begin{align}
