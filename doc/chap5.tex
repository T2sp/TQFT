...
After~\cite{gaiotto2025condensationshighercategories}, we denote an $n$-condensation of $x$ onto $y$ in $\Cat{C}$ as $x \lcond y$.
...
\begin{myexample}[label=def:1-cond]{1-condensation}
    ...
    \begin{itemize}
        ...
        \item A 0-condensation of $r \circ i$ onto $\Id_y$ in a $(0,\, 0)$-category $\Hom{\Cat{C}} (y,\, y)$. 
        i.e. $r \circ i = \Id_y$.
    \end{itemize}
...
\begin{myexample}[label=def:2-cond]{2-condensation}
    ...
    \begin{itemize}
        \item A 1-morphism $r \in \inftyMap{\Cat{C}} (x,\, y)_0$
        \item A 1-morphism $i \in \inftyMap{\Cat{C}} (y,\, x)_0$
        \item A 1-condensation of $r \circ i$ onto $\Id_y$ in a $(1,\, 1)$-category $\inftyMap{\Cat{C}} (y,\, y)$.
    \end{itemize}
    By \exref{def:1-cond}, 
    \begin{itemize}
        \item A 1-morphism $r \in \inftyMap{\Cat{C}} (x,\, y)_0$
...
\subsection{Condensation algebra / monad}

Fix an $(n,\, n)$-category $\Cat{C}$. 
Now we will outline the definition of \textbf{condensation monad in $\Cat{C}$}. 

Let's pick out an object $x \in \Cat{C} _0$.
Recall that the endmorphism $(n-1,\, n-1)$-category $\End_{\Cat{C}} (x) \coloneqq \inftyMap{\Cat{C}} (x,\, x)$ has a natural monoidal structure induced by a composition of $1$-morphisms in $\Cat{C}$.
Let $\clubsuit_{n} \subset \spadesuit_{n}$ be a \Caution{subcategory}\footnote{(18 February, 2026) Full-subness of $\clubsuit_n$ is still a conjecture~\cite{gaiotto2025condensationshighercategories}.} which consists of walking $n$-condensations with a single object.
A \textbf{condensation monad} in $\Cat{C}$ is a functor $(x \in \Cat{C}_0;\, e \in \End_{\Cat{C}}(x)_0,\, e^{\circ 2} \cond e,\, \dots) \colon \clubsuit_{n} \lto \Cat{C}$.
...
\begin{mydef}[label=def:cond-monad]{condensation algebra / monad}
    Let $n \ge 0$ and
    let $\Cat{V}^{\otimes}$ be a \underline{monoidal} $(n,\, n)$-category
    \footnote{\textbf{Monoidal $\bm{(0,\, 0)}$-category} is a set $\Cat{V}_0$ with a map $\otimes \colon \Cat{V}_0 \times \Cat{V}_0 \lto \Cat{V}_0$ and a distinguished element $1 \in \Cat{V}_0$, which makes up of a monoid in $\SETS = \CAT_{(0,\, 0)}$.}. 
    A \textbf{condensation algebra} in $\Cat{V}^{\otimes}$ is a sequence of ``commuting condensation cubes":
    % \begin{align}
    %     \spadesuit^{\times 0} &\overset{A}{\lto} \Cat{C}, \\
    %     \spadesuit^{\times 1} &\lto \Cat{C}_{/A}, \\
    %     \spadesuit^{\times 2} &\lto \Cat{C}_{/A}, \\
    %     &\vdots
    % \end{align}
    % More explicitly, condensation algebra in $\Cat{C}$ consists of the following diagrams:
    \begin{align}
        &A \in (\Cat{V}^{\otimes})_0,
        &&A \otimes A \lcond A,
        &&\begin{tikzcd}[
            baseline={([yshift=-.5ex]current bounding box.center)},
            ampersand replacement=\&
        ]
            A^{\otimes 3} \arrow[r,cond] \arrow[d,cond] \& A^{\otimes 2} \arrow[d,cond] \\
            A^{\otimes 2} \arrow[r,cond]                   \& A                  
        \end{tikzcd},
        &&% https://tikzcd.yichuanshen.de/#N4Igdg9gJgpgziAXAbVABwnAlgFyxMJZABgBpiBdUkANwEMAbAVxiRAEEA9YAHR4jwBbeAAIALAF8QE0uky58hFAEZSyqrUYs2XXvyGiAzFJlzseAkQBM5DfWatEHbnwFZhcEcemyQGc4pEZFZ2Wo7Oem4eXia+-gqWKIZqoQ46LvruolaxZglKyKqGqdpOuq4Gnjk+eRYFNiHU9qURFVlVuX7ydUTJxU1hOtIaMFAA5vBEoABmAE4QgkhkIDgQSDm+cwtL1KtIyqYgW4uIy3uIxpvzJzYra4iSV9uIqndIjzPXSMlviACsh2O6129wAbICvhcQUhwU8Tn9oYgAOwQ56gxEouHvDGok6vc4AigSIA
        \begin{tikzcd}[
            baseline={([yshift=-.5ex]current bounding box.center)},
            ampersand replacement=\&
        ]
            A^{\otimes 4} \arrow[cond,rr] \arrow[cond,rd] \arrow[cond,dd] \&                                     \& A^{\otimes 3} \arrow[cond,rd] \arrow[cond,dd] \&                          \\
                                                        \& A^{\otimes 3} \arrow[cond,rr,crossing over] \&                                     \& A^{\otimes 2} \arrow[cond,dd] \\
            A^{\otimes 3} \arrow[cond,rd] \arrow[cond,rr]            \&                                     \& A^{\otimes 2} \arrow[cond,rd]            \&                          \\
                                                        \& A^{\otimes 2} \arrow[cond,rr] \arrow[cond,from=uu,crossing over]           \&                                     \& A                       
        \end{tikzcd},
        &&\cdots
    \end{align}
    in which the faces of the $k$-dimensional cube are either whiskerings of lower-dimensional cubes or are identities~\cite[Definition 2.2.1.]{gaiotto2025condensationshighercategories}.

    \tcblower

    Let $\Cat{C}$ be an $(n,\, n)$-category\footnote{Not necessarily monoidal.}. 
    A \textbf{condensation monad} in $\Cat{C}$ is a pair of an object $x \in \Cat{C}_0$ 
    and a condensation algebra in the monoidal $(n-1,\, n-1)$-category $\End_{\Cat{C}} (x)$.
\end{mydef}

We would like to construct a \textbf{higher Morita category of \hyperref[def:cond-monad]{condensation algebras}} in $\Cat{V}^{\otimes}$.

\begin{mydef}[label=def:cond-bimod,breakable]{condensation bimodule}
    Let $n \ge 0$ and
    let $\Cat{V}^{\otimes}$ be a \underline{monoidal} $(n,\, n)$-category. 
    Pick two \hyperref[def:cond-monad]{condensation algebra}s 
    $(A,\, A^{\otimes 2} \cond A,\, \dots),\; (B,\, B^{\otimes 2} \cond B,\, \dots)$
    in $\Cat{V}^\otimes$.
    
    A \textbf{condensation $\bm{(A,\, B)}$-bimodule} in $\Cat{V}^{\otimes}$ is a sequence of ``commuting condensation cubes"~\cite[Definition 2.3.3.]{gaiotto2025condensationshighercategories}:
    \begin{align}
        &M \in (\Cat{V}^\otimes)_0,
        &&
        A \otimes M \lcond M,
        &&
        M \otimes B \lcond M, \\
        &% https://tikzcd.yichuanshen.de/#N4Igdg9gJgpgziAXAbVABwnAlgFyxMJZABgBpiBdUkANwEMAbAVxiRAEEA9YAHR4jwBbeAAIATAF8RfAVmFwRAWRATS6TLnyEUARnJVajFm3bT+Q0ctXrseAkT06D9Zq0QgrakBltaiZJ2oXY3dTGQsFKwMYKABzeCJQADMAJwhBJDIQHAgkAGZrEFT0zOocpB1C4ozEPWzcxEkvavyyhskKCSA
        \begin{tikzcd}[
            baseline={([yshift=-.5ex]current bounding box.center)},
            ampersand replacement=\&
        ]
        A^{\otimes 2} \otimes M \arrow[cond,d] \arrow[cond,r] \& A \otimes M \arrow[cond,d] \\
        A \otimes M \arrow[cond,r]                       \& M                    
        \end{tikzcd},
        &&% https://tikzcd.yichuanshen.de/#N4Igdg9gJgpgziAXAbVABwnAlgFyxMJZABgBpiBdUkANwEMAbAVxiRAEEA9YAHR4jwBbeAAIATAF8RfAVmFwRAWRATS6TLnyEUARnJVajFm3bT+Q0ctXrseAkT06D9Zq0QgrakBltaiZJ2oXY3dTGQsFKwMYKABzeCJQADMAJwhBJDIQHAgkAGZrEFT0zOocpB1C4ozEPWzcxEkvavyyhskKCSA
        \begin{tikzcd}[
            baseline={([yshift=-.5ex]current bounding box.center)},
        ...
        A \otimes M \otimes B \arrow[cond,d] \arrow[cond,r] \& M \otimes B \arrow[cond,d] \\
        A \otimes M \arrow[cond,r]                       \& M                    
        \end{tikzcd},
        &&% https://tikzcd.yichuanshen.de/#N4Igdg9gJgpgziAXAbVABwnAlgFyxMJZABgBpiBdUkANwEMAbAVxiRAEEA9YAHR4jwBbeAAIATAF8RfAVmFwRAWRATS6TLnyEUARnJVajFm3bT+Q0ctXrseAkT06D9Zq0QgrakBltaiZJ2oXY3dTGQsFKwMYKABzeCJQADMAJwhBJDIQHAgkAGZrEFT0zOocpB1C4ozEPWzcxEkvavyyhskKCSA
        \begin{tikzcd}[
        ...
        M \otimes B^{\otimes 2} \arrow[cond,d] \arrow[cond,r] \& M \otimes B \arrow[cond,d] \\
        M \otimes B \arrow[cond,r]                       \& M                    
        \end{tikzcd},
        &&\cdots
    \end{align}
    in which $\forall i,\, j \in \mathbb{Z}_{\ge 0}$, there is a $i + j$-dimensional cube which has $A^{\otimes i} \otimes M \otimes B^{\otimes j} \in (\Cat{V}^{\otimes})_0$ on its ``top" vertex and $M \in (\Cat{V}^{\otimes})_0$ on its ``bottom'' vertex.

    \tcblower

    Let $\Cat{C}$ be an $(n,\, n)$-category\footnote{Not necessarily monoidal.}
    and pick two \hyperref[def:cond-monad]{condensation monad}s:
    \begin{itemize}
        \item A condensation monad $(x \in \Cat{C}_0;\, e_1,\, e_1^{\circ 2} \cond e_1,\, \dots)$ in $\End_{\Cat{C}} (x)$
        \item A condensation monad $(y \in \Cat{C}_0;\, e_2,\, e_2^{\circ 2} \cond e_2,\, \dots)$ in $\End_{\Cat{C}} (y)$
    \end{itemize}
    
    % $(x \in \Cat{C}_0;\, e_1,\, e_1^{\circ 2} \cond e_1,\, \dots),\, (y \in \Cat{C}_0;\, e_2,\, e_2^{\circ 2} \cond e_2,\, \dots)$
    % in $\Cat{V}^{\otimes}$.
    A \textbf{condensation $(e_1,\, e_2)$-bimodule} is a sequence of ``commuting condensation cubes" in the $(n-1,\, n-1)$-category $\inftyMap{\Cat{C}} (y,\, x)$~\cite[Definition 2.3.3.]{gaiotto2025condensationshighercategories}:
    \begin{align}
        &m \in \inftyMap{\Cat{C}} (y,\, x)_0,
        &&
        e_1 \circ m \lcond m,
        &&
        m \circ e_2 \lcond m, \\
        ...
        {e_1^{\circ 2} \circ m} \arrow[cond,d] \arrow[cond,r] \& {e_1 \circ m} \arrow[cond,d] \\
        {e_1 \circ m} \arrow[cond,r]                       \& m                    
        \end{tikzcd},
        &&% https://tikzcd.yichuanshen.de/#N4Igdg9gJgpgziAXAbVABwnAlgFyxMJZABgBpiBdUkANwEMAbAVxiRAEEA9YAHR4jwBbeAAIATAF8RfAVmFwRAWRATS6TLnyEUARnJVajFm3bT+Q0ctXrseAkT06D9Zq0QgrakBltaiZJ2oXY3dTGQsFKwMYKABzeCJQADMAJwhBJDIQHAgkAGZrEFT0zOocpB1C4ozEPWzcxEkvavyyhskKCSA
        \begin{tikzcd}[
            baseline={([yshift=-.5ex]current bounding box.center)},
        ...
        {e_1 \circ m \circ e_2} \arrow[cond,d] \arrow[cond,r] \& m \circ {e_2} \arrow[cond,d] \\
        {e_1 \circ m} \arrow[cond,r]                       \& m                    
        \end{tikzcd},
        &&% https://tikzcd.yichuanshen.de/#N4Igdg9gJgpgziAXAbVABwnAlgFyxMJZABgBpiBdUkANwEMAbAVxiRAEEA9YAHR4jwBbeAAIATAF8RfAVmFwRAWRATS6TLnyEUARnJVajFm3bT+Q0ctXrseAkT06D9Zq0QgrakBltaiZJ2oXY3dTGQsFKwMYKABzeCJQADMAJwhBJDIQHAgkAGZrEFT0zOocpB1C4ozEPWzcxEkvavyyhskKCSA
        \begin{tikzcd}[
        ...
        {m \circ e_2^{\circ 2}} \arrow[cond,d] \arrow[cond,r] \& {m \circ e_2} \arrow[cond,d] \\
        {m \circ e_2} \arrow[cond,r]                       \& m                    
        \end{tikzcd},
        &&\cdots
    \end{align}
    in which $\forall i,\, j \in \mathbb{Z}_{\ge 0}$, there is a $i + j$-dimensional cube which has $e_1^{\circ i} \circ m \circ e_2^{\circ j} \in \inftyMap{\Cat{C}} (y,\, x)_0$ on its ``top" vertex and $m \in \inftyMap{\Cat{C}} (y,\, x)_0$ on its ``bottom'' vertex.
\end{mydef}


\section{Physical interpretation}

\section{{The $(n+1)$-category of linear $n$-categories}}

\subsection{{The $(n+1)$-category of presentable $n$-categories}}

Let $\PrL$ be the symmetric monoidal $(\infty,\, 2)$-category of presentable $(\infty,\, 1)$-categories, colimit-preserving functors, and natural transformations~\cite[Definition 5.5.3.1.]{lurie2008higher}.
After ~\cite[Definition 1.2.]{Aoki2025nPr}~\cite[DEFINITION 1.15]{Scholze2026Gestalten}, we define the $(\infty,\, n+1)$-category $\nPr{n}$ of presentable $(\infty,\, n)$-categories as follows:

\begin{mydef}[label=def:nPr]{}
    Let $\nPr{1} \coloneqq \PrL$.
    For each $n \ge 2$,
    the symmetric monoidal ``$(\infty,\, n+1)$-category''\footnote{Strictly speaking, our definition presents $\nPr{n}$ as an $(\infty,\, 1)$-category. However, in some sense, it can also be regarded as an $(\infty,\, n+1)$-category ~\cite[REMARK 1.16.]{Scholze2026Gestalten}.} of presentable $(\infty,\, n)$-categories is defined inductively as
    \begin{align}
        \nPr{n} \coloneqq \RMod_{\nPr{(n-1)}} (\nPr{1}).
    \end{align}
\end{mydef}

\subsection{Enrichment vs. module structure}

\Caution{The following is a very rough and intuitive discussion.}

We can regard an arbitrary object $\Cat{C} \in \Obj{\nPr{n}}$ as an ``\emph{$(\infty,\, 1)$-category $\Cat{C} \in \Obj{\nPr{1}}$ enriched by the symmetric monoidal $(\infty,\, n)$-category $\nPr{(n-1)}$}''.

By \hyperref[def:nPr]{definition} we have a module action functor
\begin{align}
    \btl \colon \Cat{C} \times \nPr{(n-1)} \lto \Cat{C}.
\end{align}
\Caution{The representability of the functor (i.e. the existence of the internal hom)}\footnote{
    $\SPACES$ is the $(\infty,\, 1)$-category of Kan complexes (i.e. $(\infty,\, 0)$-categories). 
    Recently some higher categorists use the terminology $\mathcal{A}\bm{\mathrm{ni}}$ ($(\infty,\, 1)$-category of \emph{anima}s) instead of $\SPACES$.
}
\begin{align}
    \inftyMap{\Cat{C}} \bigl( x \btl \,\mhyphen \,,\, y \bigr) \colon \nPr{(n-1)} &\lto \SPACES, \\
    \Cat{V} &\lmto \inftyMap{\Cat{C}} \bigl( x \btl \Cat{V},\, y \bigr)
\end{align}
for each $x,\, y \in \Obj{\Cat{C}}$ 
guarantees the equivalence of mapping spaces
\begin{align}
    \label{eq:tensor-hom-adjunction}
    \inftyMap{\Cat{C}} (x \btl \Cat{V},\, y) \simeq \inftyMap{\nPr{(n-1)}} \bigl( \Cat{V},\, \underline{\inftyMap{\Cat{C}}} (x,\, y) \bigr) 
\end{align}
for an arbitrary presentable $(n-1)$-categories $\Cat{V} \in \Obj{\nPr{(n-1)}}$.
We then regard the equivalence \eqref{eq:tensor-hom-adjunction} as the definition of the ``\emph{mapping $(\infty,\, n-1)$-category}" $\underline{\inftyMap{\Cat{C}}} (x,\, y) \in \Obj{\nPr{(n-1)}}$.

\subsection{{The $(n+1)$-category of $\mathbb{K}$-linear presentable $n$-categories}}

A similar consideration leads us to the definition of ``$\mathbb{K}$-linear presentable $(\infty,\, n)$-categories''.
Underlying intuition is ``the presentable $(\infty,\, n)$-category enriched by $\mathbb{K}$-linear presentable $(\infty,\, n-1)$-categories".

\begin{mydef}[label=def:nPrK]{{~\cite[DEFINITION 1.17.]{Scholze2026Gestalten}}}
    Let $\mathbb{K}$ be \Caution{an algebraically closed} field.  
    Let $\nPrK{0} \coloneqq \VEC{\mathbb{K}}$.

    Then, for each $n \ge 1$, the symmetric monoidal ``$(\infty,\, n+1)$-category'' of $\mathbb{K}$-linear presentable $(\infty,\, n)$-categories is defined inductively as
    \begin{align}
        \nPrK{n} \coloneqq \RMod_{\nPrK{(n-1)}} (\nPr{n})
    \end{align}
\end{mydef}

\begin{myexample}[label=ex:1PrC]{The 2-category of 2-vector spaces $\nPrK{1}$}
    Assume $n=1$.
    Roughly speaking, we can regard $\nPr{1}$ as the bicategory of $(1,\, 1)$-categories, functors, and natural transformations. 
    For each $\Cat{C} \in \Obj{\nPrK{1}}$, we have a module action functor
    \begin{align}
        \btl \colon \Cat{C} \times \VEC{\mathbb{K}} \lto \Cat{C}.
    \end{align}
    If the functor 
    \begin{align}
        \Hom{\Cat{C}} (x \btl \, \mhyphen \, ,\, y) \colon 
        \VEC{\mathbb{K}} &\lto \SETS, \\
        V &\lmto \Hom{\Cat{C}} (x \btl V,\, y)
    \end{align}
    is representable for each $x,\, y \in \Obj{\Cat{C}}$, we have a bijection of $0$-categories (i.e. sets)
    \begin{align}
        \Hom{\Cat{C}} (x \btl V,\, y) \simeq \Hom{\VEC{\mathbb{K}}} \bigl(V,\,  \underline{\Hom{\Cat{C}}} (x,\, y)\bigr)
    \end{align}
    for an arbitrary vector space $V \in \Obj{\VEC{\mathbb{K}}}$.
    Such a bijection guarantees the existence of mapping vector space $\underline{\Hom{\Cat{C}}}(x,\, y) \in \Obj{\VEC{\mathbb{K}}}$, and therefore recovers $\Cat{C}$ as an ordinary $\VEC{\mathbb{K}}$-enriched category.

    Note that $\Vec{\mathbb{K}} \in \Obj{\nPrK{1}}$. The module action functor of $\VEC{\mathbb{K}} = \nPrK{0}$ is the ordinary tensor product 
    \begin{align}
        \otimes_{\mathbb{K}} \colon \VEC{\mathbb{K}} \times \VEC{\mathbb{K}} \lto \VEC{\mathbb{K}}
    \end{align}
    of vector spaces.
\end{myexample}

\begin{myexample}[label=ex:2PrC]{The 3-category of 3-vector spaces $\nPrK{2}$}
    Assume $n=2$.
    Roughly speaking, we can regard $\nPr{2}$ as the tricategory of bicategories, pseudofunctors, strong transformations, and modifications~\cite{Yau2020bicategory}.
    For each $\Cat{C} \in \Obj{\nPrK{2}}$, we have a module action pseudofunctor
    \begin{align}
        \btl \colon \Cat{C} \times \nPrK{1} \lto \Cat{C}.
    \end{align}
    If the pseudofunctor 
    \begin{align}
        \inftyMap{\Cat{C}} (x \btl \, \mhyphen \, ,\, y) \colon 
        \nPrK{1} &\lto \CAT, \\
        \Cat{V} &\lmto \inftyMap{\Cat{C}} (x \btl \Cat{V},\, y)
    \end{align}
    is representable for each $x,\, y \in \Obj{\Cat{C}}$, we have an equivalence of $1$-categories
    \begin{align}
        \inftyMap{\Cat{C}} (x \btl \Cat{V},\, y) \simeq \inftyMap{\nPrK{1}} \bigl(\Cat{V},\,  \underline{\inftyMap{\Cat{C}}} (x,\, y)\bigr)
    \end{align}
    for an arbitrary $\mathbb{K}$-linear $1$-category $\Cat{V} \in \Obj{\nPrK{1}}$.
    Such an equivalence guarantees the existence of mapping $\mathbb{K}$-linear $1$-category $\underline{\inftyMap{\Cat{C}}}(x,\, y) \in \Obj{\nPrK{1}}$.
    i.e. $\Cat{C}$ can be regarded as a bicategory whose collection of 2-morphisms constitutes a $\mathbb{K}$-vector space.

    Note that $\nPrK{1}$ itself is an object of $\nPrK{2}$.
    The module action pseudofunctor of $\nPrK{1}$ is the Lurie's tensor product\footnote{Analogous to the classical Deligne tensor product.}:
    \begin{align}
        \boxtimes \colon \nPrK{1} \times \nPrK{1} \lto \nPrK{1}
    \end{align}
\end{myexample}
...
\begin{marker}
    From now on, we use the terminology
    \begin{align}
        \nVec{n}_{\mathbb{K}} \coloneqq \nPrK{(n-1)}\quad (n \ge 1).
    \end{align}
    An object of $\nVec{n}_{\mathbb{K}}$ is called an \emph{$n$-vector space}.
\end{marker}

\section{Tannaka-Krein reconstruction of 3-vector spaces}
...
\subsection{The choice of the fiber 2-functor}

Fix a \hyperref[def:nPrK]{3-vector space} $\Cat{C} \in \Obj{\nVec{3}_{\mathbb{K}}}$. 
Tannaka-Krein reconstruction needs a faithful pseudofunctor $F^{(1)} \in \Obj{\inftyMap{\nVec{3}_{\mathbb{K}}}(\Cat{C},\, \nVec{2}_{\mathbb{K}})^{\text{faith}}}$.
We denote
\begin{align}
    T_{\Cat{C}} \coloneqq \End_{\inftyMap{\nVec{3}_{\mathbb{K}}}(\Cat{C},\, \nVec{2}_{\mathbb{K}})} (F^{(1)}) \in \Obj{\nVec{2}_{\mathbb{K}}}.
\end{align}

If $F^{(1)}$ admits a left adjoint $F^{(1)}{}^\vee \in \Obj{\inftyMap{\nVec{3}_{\mathbb{K}}} (\nVec{2}_{\mathbb{K}},\, \Cat{C})}$ \Caution{(automatic?)}, an isomorphism
\begin{align}
    T_{\Cat{C}} \simeq \inftyMap{\inftyMap{\nVec{3}_{\mathbb{K}}} (\nVec{2}_{\mathbb{K}},\, \nVec{2}_{\mathbb{K}})} (\Id_{\nVec{2}_{\mathbb{K}}},\, F^{(1)} \circ F^{(1)}{}^{\vee})
\end{align}
in $\nVec{2}_{\mathbb{K}}$
\footnote{
    \Caution{This is merely an analogue of \exref{ex:2PrC}. We need some references or a proof.}
    Note that $\nVec{3}_{\mathbb{K}} \in \Obj{\nVec{4}_{\mathbb{K}}}$.
    Therefore, by the ``tensor-hom adjunction'', we can regard $\inftyMap{\nVec{3}_{\mathbb{K}}}(\Cat{C},\, \nVec{2}_{\mathbb{K}}),\, \inftyMap{\nVec{3}_{\mathbb{K}}} (\nVec{2}_{\mathbb{K}},\, \nVec{2}_{\mathbb{K}}) \in \Obj{\nVec{3}_{\mathbb{K}}}$.
    Furthermore, according to \exref{ex:2PrC}, we can regard $T_{\Cat{C}} \in \Obj{\nVec{2}_{\mathbb{K}}}$.
}
exists.

\begin{mylem}[label=lem:End-1]{}
    There is an isomorphism
    \begin{align}
        I \colon \inftyMap{\nVec{3}_{\mathbb{K}}} (\nVec{2}_{\mathbb{K}},\, \nVec{2}_{\mathbb{K}}) \xrightarrow{\simeq} \nVec{2}_{\mathbb{K}}
    \end{align}
    in $\nVec{3}_{\mathbb{K}}$.
\end{mylem}

\begin{proof}
    \textcolor{red}{Probably in~\cite{Scholze2026Gestalten}}
\end{proof}

Equivalence $I$ in Lemma~\ref{lem:End-1} induces an isomorphism\footnote{Sice $I$ is a functor, $I(\Id_{\nVec{2}_{\mathbb{K}}}) \simeq \VEC{\mathbb{K}}$．}
\begin{align}
    T_{\Cat{C}} 
    \simeq \inftyMap{\nVec{2}_{\mathbb{K}}} \bigl(\VEC{\mathbb{K}},\, I(F^{(1)} \circ F^{(1)}{}^{\vee}) \bigr)
\end{align}
in $\nVec{2}_{\mathbb{K}}$.

\begin{myprop}[label=prop:closed-monoidal]{}
    \begin{align}
        T_{\Cat{C}} \simeq I(F^{(1)} \circ F^{(1)}{}^{\vee})
    \end{align}
\end{myprop}

\begin{proof}
    For each $\Cat{V} \in \Obj{\nVec{2}_{\mathbb{K}}}$, \exref{ex:2PrC} shows the natural isomorphism (i.e. a kind of tensor-hom adjunction)
    \begin{align}
        &\inftyMap{\nVec{2}_{\mathbb{K}}} \Bigl(\Cat{V},\, \inftyMap{\nVec{2}_{\mathbb{K}}} \bigl(\VEC{\mathbb{K}},\, I(F^{(1)} \circ F^{(1)}{}^\vee)\bigr)\Bigr) \\
        &\simeq \inftyMap{\nVec{2}_{\mathbb{K}}} \bigl(\VEC{\mathbb{K}} \boxtimes \Cat{V},\, I(F^{(1)} \circ F^{(1)}{}^\vee)\bigr) \\
        &\simeq \inftyMap{\nVec{2}_{\mathbb{K}}} \bigl(\Cat{V},\, I(F^{(1)} \circ F^{(1)}{}^\vee)\bigr)
    \end{align}
    in $\CAT$. i.e. there is an equivalence between two pseudofunctors;
    \begin{align}
        \inftyMap{\nVec{2}_{\mathbb{K}}} \Bigl(\,\mhyphen \,,\, \inftyMap{\nVec{2}_{\mathbb{K}}} \bigl(\VEC{\mathbb{K}},\, I(F^{(1)} \circ F^{(1)}{}^\vee)\bigr)\Bigr)
        \simeq \inftyMap{\nVec{2}_{\mathbb{K}}} \bigl(\, \mhyphen \,,\, I(F^{(1)} \circ F^{(1)}{}^\vee)\bigr).
    \end{align}
    Then, Yoneda lemma for bicategories~\cite[Lemma 8.3.11.]{Yau2020bicategory} induces the equivalence
    \begin{align}
        \inftyMap{\nVec{2}_{\mathbb{K}}} \bigl(\VEC{\mathbb{K}},\, I(F^{(1)} \circ F^{(1)}{}^\vee)\bigr) \simeq I(F^{(1)} \circ F^{(1)}{}^\vee)
    \end{align}
\end{proof}

\subsection{The second choice}

We have another choice of a faithful functor $F^{(2)} \in \Obj{\inftyMap{\nVec{2}_{\mathbb{K}}} (T_{\Cat{C}},\, \VEC{\mathbb{K}})^{\text{faith}}}$.
Combining the Lemma~\ref{lem:End-1} and the Proposition~\ref{prop:closed-monoidal}, such a choice is equivalent to the choice of 2-morphism
\begin{align}
    \widehat{F}^{(2)} \coloneqq I^{-1} (F^{(2)}) \colon F^{(1)} \circ F^{(1)}{}^{\vee} \Longrightarrow \Id_{\nVec{2}_{\mathbb{K}}}
\end{align}
in $\nVec{3}_{\mathbb{K}}$.

\section{Dualizability in higher Morita categories}
