...
\begin{myexample}[label=case:real]{実の場合}
    ...
    \begin{proof}
        ...
        \end{align}
        を考える．仮定より $\LGL(n,\, \mathbb{R}) \times \mathbb{R}^{n^2}$ 上で $q(A,\, X) = 0$ であるが，多項式函数 $q \colon \mathbb{R}^{n^2} \times \mathbb{R}^{n^2} \lto \mathbb{R}$ は連続関数であり，
        $\mathbb{R}^{n^2} \setminus \LGL(n,\, \mathbb{R}) = \det(\{0\})$ は $\mathbb{R}^{n^2}$ の余次元1の部分多様体だから稠密なので，$\mathbb{R}^{n^2} \times \mathbb{R}^{n^2}$ 上で $q(A,\, X) = 0$ が言える．
    ...
    結局，
    \begin{align}
        \mathrm{Inv} \bigl( \Lgl(n,\, \mathbb{R}) \bigr) = \mathbb{R} \bigl[ c_1(t^i{}_j),\, \dots,\, c_n(t^i{}_j) \bigr] 
    \end{align}
    が分かった．
\end{myexample}
...
\begin{myexample}[label=case:orthogonal]{直交群の場合}
    ...
    \begin{proof}
        ...
        \begin{align}
            c(X,\, \lambda)
            &=  \sum_{k=0}^n (-1)^k c_k(X) \lambda^{n-k} \\
    ...
    以下では $p_k \coloneqq c_{2k}$ とおく．

    \begin{myprop}[label=prop:inv-ortho]{}
        \begin{align}
            \mathrm{Inv} \bigl( \Lo(n) \bigr) \cong \mathbb{R}[p_1(t^i{}_j),\, \dots,\, p_{\floorf{n/2}}(t^i{}_j)]
        \end{align}
    \end{myprop}
    
    \begin{proof}
        自然な埋め込み $\rho \colon \mathbb{R}[p_1,\, \dots,\, p_{\floorf{n}}] \hookrightarrow \mathrm{Inv} \bigl( \Lo(n) \bigr)$ が全単射であることを示す．
