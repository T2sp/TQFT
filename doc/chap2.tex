...
\begin{itemize}
    ...
    \item \hyperref[def:associated-vect]{同伴ベクトル束} $V \hookrightarrow P \times_\rho V \xrightarrow{q} M$
\end{itemize}
を与える．$E \coloneqq P \times_\rho V$ とおく．定理\ref{thm:connection-assoc}-(2) より，
\begin{align}
...
\begin{proof}
    $\gamma,\, u$ を1つ固定する．命題\ref{prop:assoc-basic}-(2) の $\sharp$ の全射性の証明から
    \begin{align}
        (\nabla^E_X s)|_p
        &= (\nabla^E s)(X)|_p \\
        &= (\sharp^{-1} \circ \codd{} \circ \sharp s)_p (X_p) \\
        &= \Bigl( \sharp^{-1}\bigl(\codd{} \circ \sharp s \bigr)\Bigr)_{\gamma(0)} \bigl( \dot{\gamma}(0) \bigr) \\
        &= \tilde{\gamma}(0) \times_\rho \bigl(\codd{(\sharp s)}\bigr)_{\tilde{\gamma}(0)} \bigl( \dot{\tilde{\gamma}} (0) \bigr)  \\
        ...
        % &= \tilde{\gamma}(0) \times_\rho f_{\tilde{\gamma}(0)}^{-1} \Bigl( \dd (\pi^* s)|_{\tilde{\gamma}(0)} \bigl( \dot{\tilde{\gamma}}(0) \bigr) \Bigr) \\
        % &= \tilde{\gamma}(0) \times_\rho f_{\tilde{\gamma}(0)}^{-1} \Bigl( (\pi^*\dd{s})|_{\tilde{\gamma}(0)} \bigl( \dot{\tilde{\gamma}}(0) \bigr) \Bigr) \\
        % &= \tilde{\gamma}(0) \times_\rho f_{\tilde{\gamma}(0)}^{-1} \left( \bigl((\pi \circ \tilde{\gamma})^*\dd{s}\bigr)|_{0} \left( \eval{\dv{}{t}}_{t=0} \right) \right) \\ 
        % &= \tilde{\gamma}(0) \times_\rho f_{\tilde{\gamma}(0)}^{-1} \left( \bigl(\gamma^*\dd{s}\bigr)|_{0} \left( \eval{\dv{}{t}}_{t=0} \right) \right) \\ 
        % &= \tilde{\gamma}(0) \times_\rho f_{\tilde{\gamma}(0)}^{-1} \Bigl( \dd{s}|_{\gamma(0)} \bigl( \dot{\gamma}(0) \bigr)  \Bigr) \\
        % &= \tilde{\gamma}(0) \times_\rho f_{\tilde{\gamma}(0)}^{-1} \Bigl( \dot{(s \circ \gamma)} (0)  \Bigr) \\
        % &= 
    \end{align}
    ...
    が示された．
\end{proof}
% ここで\hyperref[prop:assoc-basic]{$\sharp$ の定義}を思い出すと，$\sharp s \in \Omega^0_\rho (P;\, V)$ は
% \begin{align}
%     (\sharp s)(u) 
%     &= f_u^{-1} \circ (\pi^* s)_u \\
%     &= f_u^{-1} \circ s \circ \pi (u) 
% \end{align}
% であるが，
% $\forall x \times_\rho v \in q^{-1}\bigl(\{\pi(u)\}\bigr)$ および $\pi(u) \in U_\alpha$ を充たす任意の $\alpha \in \Lambda$ に対して $x \times_\rho v = \Bigl( s_\alpha \bigl( \pi(x) \bigr) \btl g_\alpha(x) \Bigr) \times_\rho v = s_\alpha \bigl( \pi(u) \bigr) \times_\rho \rho\bigl(g_\alpha(x)\bigr)(v) = \Bigl( s_\alpha \bigl( \pi(u) \bigr) \btl g_\alpha(u)\Bigr)\times_\rho \rho\bigl(g_\alpha(u)^{-1} g_\alpha(x)\bigr)(v) = u \times_\rho \rho\bigl(g_\alpha(u)^{-1} g_\alpha(x)\bigr)(v)$ が成り立つので，
% \begin{align}
%     f^{-1}_u \circ s \circ \pi(u)
%     &= \rho\Bigl(g_\alpha(u)^{-1} g_\alpha \bigl( s_1(\pi(u)) \bigr) \Bigr)  \Bigl( s_2(\pi(u)) \Bigr) 
% \end{align}
...
%     \item $E$ の局所自明化 $\Familyset[\big]{\psi_\alpha \colon q^{-1}(U_\alpha) \lto U_\alpha \times V,\; s_\alpha(p) \times_\rho v \lmto (p,\, v)}{\alpha \in \Lambda}$
% \end{itemize}
% をとる．
% このとき\hyperref[prop:assoc-basic]{$\sharp$ の定義}から $\forall u \in \pi^{-1}(U_\alpha)$ において
% \begin{align}
%     \sharp s(u) 
%     &= f_u^{-1} \Bigl( s \bigl(\pi(u)\bigr) \Bigr)
% \end{align}
% である．
% ここで $s(x) \eqqcolon s_1 (x) \times_\rho s_2(x)$ および $g_\alpha \coloneqq \mathrm{proj}_2 \circ \varphi_\alpha \colon \pi^{-1}(U_\alpha) \lto G$ とおくと
% \begin{align}
%     s(x) 
%     &= \Bigl[s_\alpha \Bigl( \pi \bigl(s_1(x)\bigr) \Bigr) \btl g_\alpha \bigl( s_1(x) \bigr) \Bigr] \times_\rho s_2(x) \\
%     &= \bigl(s_\alpha (x) \btl g_\alpha \bigl( s_1(x) \bigr) \bigr) \times_\rho s_2(x) \\
%     &= s_\alpha (x) \times_\rho \rho\Bigl(g_\alpha \bigl( s_1(x) \bigr)\Bigr)\Bigl(s_2(x)\Bigr) \\
%     &= 
% \end{align}
% と書けるので，$\xi_\alpha (x) \coloneqq \rho\Bigl(g_\alpha \bigl( s_1(x) \bigr)\Bigr)\Bigl(s_2(x)\Bigr)$ とおくと
% \begin{align}
%     \sharp s(u) 
%     = 
% \end{align}
...
\end{align}
で定義する．また，$C^\infty$ 曲線 $\gamma \colon [0,\, 1] \lto M$ の像は $U_\alpha$ に含まれているとし\footnote{$\gamma([0,\, 1]) \subset M$ はコンパクト集合なので，有限個の点で区切ればこの要請を充たすことができる．}，$\gamma$ の\hyperref[def:horizontal-lift-curve]{水平持ち上げ}を
\begin{align}
...
\end{align}
と成分表示が求まった．特に $X$ として座標ベクトル場 $\pdv{}{x^\mu}$ をとると
\begin{align}
    \nabla^E_{\pdv{}{x^\mu}} e_\alpha{}_i = A_\alpha{}_a{}_\mu \bigl[\, \rho_* (T^a) \,\bigr]^j{}_i\,  e_\alpha{}_j|_p
\end{align}
となる．

次に，任意の切断 $s \in \Gamma(E)$ の場合を考える．$U_\alpha$ 上で $s(p) = s_\alpha(x) \times_\rho \xi_\alpha(x) = s_\alpha(x) \times_\rho \xi_\alpha(p)^i e_i = \xi_\alpha(p)^i e_\alpha{}_i$ と展開できるので，
...
\begin{mydef}[label=def:invariant]{不変多項式}
    ...
    \hyperref[def:polynomial]{多項式} $f \colon \mathfrak{g} \lto \mathbb{R}$ が $\bm{\mathrm{Ad}(G)}$\textbf{-不変}であるとは，$\forall g \in G,\; \forall X \in \mathfrak{g}$ に対して
    \begin{align}
...
\begin{mytheo}[label=thm:Chern-Weil]{Chern-Weilの定理}
    ...
    \begin{enumerate}
        ...
        \item de Rhamコホモロジー類 $\Lambda + \Im \dd{} \in \irm{H}{dR}^{2k} (M)$ は $\omega$ の取り方によらない．
        したがって結合代数の準同型（\textbf{Chern-Weil準同型}）
        \begin{align}
            w \colon \mathrm{Inv}(\mathfrak{g}) \lto H^\bullet (M),\; f \lmto \Lambda + \Im \dd{}
        \end{align}
        が定まる．
    \end{enumerate}
\end{mytheo}
...
\begin{mydef}[label=def:characteristic-class-polynomial]{特性類}
    \begin{itemize}
        ...
        \item \hyperref[def:invariant]{$k$ 次不変多項式} $f \colon \mathfrak{g} \lto \mathbb{R}$
    \end{itemize}
    を与える．このとき定理\ref{thm:Chern-Weil}の $\Lambda + \Im \dd{} \in \irm{H}{dR}^{2k} (M)$ のことを，$f$ による $P$ の\textbf{特性類} (characteristic class) と呼ぶ．
\end{mydef}
