...
\begin{mydef}[label=def:holonomy]{ホロノミー群}
    ...
    $\forall u \in P$ に対して，命題\ref{prop:holonomy}より部分集合
    \begin{align}
...
ここで，
\begin{align}
    c(t^i{}_j,\, \lambda) \coloneqq \det(\lambda \unity_n + \bigl[ \,t^i{}_j\, \bigr] ) \in \mathbb{Z}[t^i{}_j,\, \lambda]
\end{align}
を考える．$\lambda$ について項を整理することで $r$ 個の $c_k (t^i{}_j) \in \mathbb{Z}[t^i{}_j]$
\begin{align}
    c(t^i{}_j,\, \lambda) 
    &\eqqcolon \lambda^n + c_1(t^i{}_j) \lambda^{n-1} + \cdots \\
    &= \sum_{k=0}^n c_k(t^i{}_j) \lambda^{n-k}
\end{align}
を得る．補題\ref{lem:Z-extension}により $c(t^i{}_j,\, \lambda) \in \mathbb{K}[t^i{}_j,\, \lambda],\;  c_k (t^i{}_j) \in \mathbb{K}[t^i{}_j]$ と見做してもこの等式は成り立つ．

\begin{myprop}[label=prop:det-coeff-invariant]{$c_k(t^i{}_j)$ は不変多項式}
    $\mathbb{K}$ を標数 $0$ の体とする．このとき $c_k (t^i{}_j) \in \mathbb{K}[t^i{}_j]$ は\hyperref[label]{$\Adj(\LGL(n,\, \mathbb{K}))$-不変多項式}である．
\end{myprop}
...
\begin{proof}
    ...
    \begin{align}
        c(t^i{}_j,\, \lambda) = \det(X) = \det(gXg^{-1}) = c(\elem{gtg^{-1}}{i}{j},\, \lambda)
    \end{align}
    がわかり，示された．
\end{proof}


\begin{myexample}[label=def:Chern-class]{複素の場合}
    ...
    \begin{align}
        \uirm{\Lgl(n,\, \mathbb{C})}{d} \coloneqq \bigl\{\, X \in \Lgl(n,\, \mathbb{C}) \bigm| \text{対角化可能} \,\bigr\} 
    \end{align}
    を考える．$X \in \Lgl(n,\ \mathbb{C}) \setminus \uirm{\Lgl(n,\, \mathbb{C})}{d} \IFF X$ の特性方程式 $f_X(\lambda) \coloneqq \det(\lambda \unity_n - X)$ とその微分 $f_X'(\lambda)$ が共通の根を持つ $\IFF$ $f_X,\, f_X' \in \mathbb{C}[\lambda]$ のSylvester行列式 (\textbf{resultant}とも言う) $\mathrm{res}(f_X,\, f'_X) = 0$
    であるが，写像 $\varphi \colon \Lgl(n,\, \mathbb{C}) \lto \mathbb{C},\, X \lmto \mathrm{res}(f_X,\, f'_X)$ は行列の成分に関する多項式函数なので $C^\infty$ 関数であり，$\Lgl(n,\ \mathbb{C}) \setminus \uirm{\Lgl(n,\, \mathbb{C})}{d} = \varphi^{-1}(\{0\})$ は余次元 $1$ の部分多様体である．従って $\uirm{\Lgl(n,\, \mathbb{C})}{d}$ は $\Lgl(n,\, \mathbb{C})$ 上稠密である．

    　ここで，$\mathbb{C}$-結合代数の準同型
    \begin{align}
        \rho \colon \mathrm{Inv} \bigl( \Lgl(n,\, \mathbb{C}) \bigr) &\lto \mathbb{C}[t_1,\,\dots,\, t_n] \\
        P(t^i{}_j) &\lmto \left( (\lambda_1,\,\dots,\, \lambda_n) \lmto P|_{\uirm{\Lgl(n,\, \mathbb{C})}{d}} \left(\mqty[\dmat{\lambda_1,\ddots,\lambda_n}] \right)\, \right)
    \end{align}
    を考える．$P$ が不変多項式であることから，対称群 $\mathfrak{S}_n$ の $\mathbb{C}^n$ への作用
    \begin{align}
    ...
    ここで $\forall P \in \Ker \rho$ をとる．このとき $\forall X \in \uirm{\Lgl(n,\, \mathbb{C})}{d}$ について $P(X) = 0$ であるが，$\uirm{\Lgl(n,\, \mathbb{C})}{d}$ が稠密なので，多項式函数 $P(X)$ の連続性から $\forall X \in \Lgl(n,\, \mathbb{C})$ に対して $P(X) = 0$ が言える．i.e. $\rho$ は単射である．
    一方，$\forall \lambda \in \mathbb{C}$ に関して，$k$ 次の基本対称式 $\sigma_k \in \mathbb{Z}[t_1,\, \dots,\, t_n] \subset \mathbb{C} [t_1,\, \dots,\, t_n]$ を使って
    \begin{align}
        \rho \bigl( c(t^i{}_j,\, \lambda) \bigr) (\lambda_1,\, \dots,\, \lambda_n)
    ...
    と書けるので 
    \begin{align}
        \rho \bigl( c_k(t^i{}_j) \bigr) = \sigma_k
    \end{align}
    であり，$\mathbb{C}[t_1,\,\dots,\, t_n]^{\mathrm{Sym}}$ は $\sigma_1,\, \dots,\, \sigma_n$ によって生成されるので，$\rho$ が全射であることが分かった．つまり $\rho$ は $\mathbb{C}$-結合代数の同型写像であり，
    \begin{align}
        \mathrm{Inv} \bigl( \Lgl(n,\, \mathbb{C}) \bigr) = \mathbb{C} \bigl[ c_1(t^i{}_j),\, \dots,\, c_n(t^i{}_j) \bigr] 
    \end{align}
    が分かった．全く同様の議論により
    \begin{align}
        \mathrm{Inv} \bigl( \Lu(n,\, \mathbb{C}) \bigr) = \mathbb{C} \bigl[ c_1(t^i{}_j),\, \dots,\, c_n(t^i{}_j) \bigr] 
    \end{align}
    もわかる．
\end{myexample}

\begin{mydef}[label=def:Chern-class]{Chern類}
    \begin{itemize}
