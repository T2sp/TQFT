...
\begin{align}
    \irm{S}{CS} = \frac{k}{4\pi} \int_{\mathcal{M}} \Tr \left( a \wedge \dd{a} + \frac{2}{3} a \wedge a \wedge a \right) 
\end{align}
と書けることからわかる\footnote{...と言うのは微妙に的を外している．より正確には $2+1$ 次元多様体 $\mathcal{M}$ を教会に持つような4次元多様体 $\mathcal{N}$ を用意し，$\mathcal{N}$ の作用 $\mathcal{S}[a] \coloneqq k/(4\pi) \int_{\mathcal{N}} \Tr (F \wedge F)$ を部分積分することで $\mathcal{S}{CS}$ の別の定義が与えられる．}．
