...
簡単のため $G = \LSU(2)$ とする\footnote{このとき $G$ がコンパクトかつ単連結なので，3次元多様体 $\mathcal{M}$ 上の任意の\hyperref[def.PFD]{主 $G$ 束}は自明束 $\mathcal{M} \times G$ は自明束と同型になり，Chern-Simons作用は大域的にこの形で定義できる．詳細は \url{https://math.stackexchange.com/questions/370313/every-principal-g-bundle-over-a-surface-is-trivial-if-g-is-compact-and-simpl} を参照．一般のLie群 $G$ の場合はこう簡単にいかず，\textbf{微分コホモロジー} (differential cohomology) ~\cite{CheegerSimons1985differential}, ~\cite{HopkinsSinger2005quadraticfunctionsgeometrytopologyand}, ~\cite{SimonsSullivan2007diff}を使わなくてはいけない．}．
...
\begin{myexample}[label=ex:tangentbundle]{接束}
    $n$ 次元\cinfty 多様体 $M$ の\hyperref[prop:tangentbundle]{接束}は，構造群を $\LGL(n,\, \mathbb{R})$ とするベクトル束
...
\begin{proof}
	...
	開被覆 $\{U_\lambda\}$ の添字集合を $\Lambda$ とする．このとき $\forall \lambda \in \Lambda$ に対して，$U_\lambda \subset B$ には底空間 $B$ からの\hyperref[def.reltopo]{相対位相}を入れ，$U_\lambda \times F$ にはそれと $F$ の位相との\hyperref[def:product-coproduct]{積位相}を入れることで，\hyperref[def:product-coproduct]{直和位相空間}
	\begin{align}
...
定理\ref{thm:fundamental-flow}によって，\cinfty 多様体 $M$ 上の\hyperref[def:complete-vecf]{完備}なベクトル場 $X$ がLie群 $\mathbb{R}$ の $M$ への作用 $\theta \colon \mathbb{R} \times M \lto M$ を一意に定めることが分かる．
...
\end{align}
は\hyperref[def:global-flow]{大域的フロー}である
\footnote{
...
．
この大域的フローの\hyperref[thm:fundamental-flow]{無限小生成子}はベクトル場
\begin{align}
...
これまでの議論で主束，およびその同伴ベクトル束の接続の大域的な表式を得た．以降の小節では，物理の文脈で登場する接続の局所表示（それは\textbf{ゲージ場}と呼ばれる）および曲率とその局所表示（それは\textbf{場の強さ}と呼ばれる）を，主束およびその同伴ベクトル束の各々において，より直接的な形で定式化する．
...
\begin{mydef}[label=def:gauge-field]{ゲージ場}
    多様体 $M$ とLie群 $G$，および
    \begin{itemize}
        \item $M$ の開被覆 $\Familyset[big]{U_\alpha}{\alpha \in \Lambda}$
        \item 変換関数\footnote{i.e. コサイクル条件\eqref{eq.cocycle}を要請している．} $\Familyset[big]{t_{\alpha\beta} \colon U_\alpha \cap U_\beta \lto G}{\alpha,\, \beta \in \Lambda}$
    \end{itemize}
    を与える．
    このとき，$\mathfrak{g}$ 値1-形式の族 $A \coloneqq \Familyset[big]{A_\alpha \in \Omega^1 (U_\alpha;\, \mathfrak{g})}{\alpha \in \Lambda}$ が\textbf{ゲージ場} (gauge field) であるとは，$\forall \alpha,\, \beta \in \Lambda$ について
    \begin{align}
        A_\beta|_x = \Adj \bigl( t_{\alpha\beta}(x)^{-1} \bigr) \bigl( A_\alpha|_x \bigr)  + (t^*_{\alpha\beta} \theta)_x, \quad \forall x \in U_\alpha \cap U_\beta
    \end{align}
    が成り立つことを言う．
\end{mydef}

定義\ref{def:gauge-field}のデータが与えられたとき，まず命題\ref{prop.cocycle}によって\hyperref[def.PFD]{主束} $G \hookrightarrow P \xrightarrow{\pi} M$ が構成され，さらに定理\ref{thm:local-connection}によって $P$ の\hyperref[def:connection]{接続形式} $\omega \in \Omega^1 (P;\, \mathfrak{g})$ が構成されるのである．
...
\begin{myprop}[label=prop:horizontal-lift-vecf]{ベクトル場の水平持ち上げは右不変}
    $\tilde{X}$ は右不変である．i.e. $\forall g \in G$ に対して自分自身と\hyperref[def:F-related]{$R_g$-related}である．
\end{myprop}
...
この小節では，前小節で定義した主束上の曲率形式を局所表示し，それが物理側で\textbf{場の強さ}と呼ばれるものと同一視できることを確認する．
...
\begin{myprop}[label=prop:det-coeff-invariant]{$f_k(t^i{}_j)$ は不変多項式}
    $\mathbb{K}$ を標数 $0$ の体とする．このとき $f_k (t^i{}_j) \in \mathbb{K}[t^i{}_j]$ は\hyperref[def:invariant]{$\Adj(\LGL(n,\, \mathbb{K}))$-不変多項式}である．
\end{myprop}
...
% そのためには，$\mathbb{Z}$ を \hyperref[ax:G-mod]{ $\mathbb{Z}[G]$ 加群}と見做したときの射影的分解をとる必要がある．

% $P_n \coloneqq \mathbb{Z}^{\oplus G^{n+1}}$ とおく．$(g_1,\, \dots,\, g_{n+1}) \in G^{n+1}$ 成分のみ $1$ で他が $0$ であるような $P_n$ の元を $[g_0,\, \dots,\, g_n]$ と書く．$P_n$ への $G$ の左作用を $g \btr [g_0,\, \dots,\, g_{n}] \coloneqq [gg_0,\, \dots,\, gg_n]$ と定義する．
% $P_n$ は
