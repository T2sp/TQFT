...
\begin{proof}
    \begin{enumerate}
        ...
        \begin{description}
            ...
            \end{align}
            は短完全列になる．
            さらにこれは補題\ref{lem:connection}の証明で与えた線型写像 $\bigl( T_{1_G} (R^{(u)}) \bigr)^{-1} \colon \mathfrak{g} \lto \Ker (T_{u} \pi) \subset T_{u}P$ によって分裂するので
            \begin{align}
...
\exref{def:associated-vect}を思い出すと，主束に与えられた\hyperref[def:connection]{接続形式}が自然に同伴ベクトル束上の接続と結び付くような気がしてくる．
実際それは正しい．
