...
\begin{myprop}[label=prop.cocycle]{ファイバー束の構成}
	\begin{itemize}
        ...
        \item \textbf{コサイクル条件}\eqref{eq.cocycle}を充たす\cinfty 関数の族 $\{t_{\alpha\beta} \colon U_\beta \cap U_\alpha \to \Diff F\}$
    \end{itemize}
    を与える．
...
\begin{proof}
	...
	$\forall u \in P,\, \forall g \in G$ をとる．$\pi(u) \in U_\alpha$ となる $\alpha \in \Lambda$ を選び，対応する\hyperref[def.fiber-1]{局所自明化} $\varphi_\alpha$ による $u$ の像を $\varphi_\alpha (u) \eqqcolon (p,\, h) \in U_\alpha \times G$ とおく．
