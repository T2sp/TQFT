...
$\forall g \in G$ と $\forall X,\, Y \in \mathfrak{X}^L(G)$ をとる．このとき $(L_g)_* X = X,\; (L_g)_* Y = Y$ なので，命題\ref{prop:Lie-bracket-natural}の後半から
\begin{align}
    (L_g)_* \comm{X}{Y} = \comm{(L_g)_* X}{(L_g)_* Y} = \comm{X}{Y}
\end{align}
が言える．i.e. 
% \textbf{左不変}\hyperref[def:vecf]{ベクトル場}とは，$\mathbb{R}$-ベクトル空間
\underline{$\mathfrak{X}^L (G)$ は\hyperref[def:Lie-bracket]{Lieブラケット}について閉じるので，体 $\mathbb{R}$ 上のLie代数になる}．

\begin{myprop}[label=prop:LieAlg]{}
    $G$ をLie群とする．このとき評価写像
    \begin{align}
        \evalunit \colon \mathfrak{X}^L(M) \lto T_{1_G} G,\; X \lmto X_{1_G}
    \end{align}
    はベクトル空間の同型写像である．
    % よって $\dim \mathfrak{X}^L(G) = \dim G$ である．
\end{myprop}
...
\begin{proof}
    ...
    \begin{description}
        ...
        一方 $X \in \mathfrak{X}^L(G)$ でもあるので，$\forall g \in G$ に対して $X_g = X_{L_g(1_G)} = (T_{1_G} L_g)(X_{1_G}) = 0$ が言える\footnote{2つ目の等号で\hyperref[def:F-related]{$L_g$-related}の定義を使った．}．
        ...
        \end{align}
        が言える．i.e. $\Lie{v} \in \mathfrak{X}^L(G)$ である．
...
\begin{myprop}[label=prop:one-parameter-basic]{1パラメータ部分群の特徴付け}
    Lie群 $G$ を与える．
    \begin{enumerate}
        \item $G$ の任意の\hyperref[def:one-parameter-subgroup]{1パラメータ部分群} $\gamma \colon \mathbb{R} \lto G$ に対して，$\gamma$ を初期条件 $\gamma(0) = 1_G$ を充たす\hyperref[def:local-flow]{極大積分曲線}として持つ\hyperref[def:left-invariant]{左不変ベクトル場} $X \in \mathfrak{X}^L(G)$ が一意的に存在する．
        \item $\forall X \in \mathfrak{X}^L(G)$ に対して，初期条件 $\gamma(0) = 1_G$ を充たす唯一の $X$ の極大積分曲線 $\gamma \colon \mathbb{R} \lto G$ は $G$ の1パラメータ部分群である．
    \end{enumerate}
    \tcblower
    上述の対応によって $X \in \mathfrak{X}^L(G)$ から一意的に定まる1パラメータ部分群のことを\textbf{$\bm{X}$ が生成する1パラメータ部分群}と呼ぶ．
\end{myprop}

\begin{marker}
    命題\ref{prop:LieAlg}の同型と併せると
    \begin{align}
        \bigl\{\, G\; \text{の1パラメータ部分群}  \,\bigr\} \overset{{}_*}{\longleftrightarrow} \mathfrak{X}^L(G) \overset{\evalunit}{\longleftrightarrow} T_{1_G} G
    \end{align}
...
\begin{proof}
    \begin{enumerate}
        ...
        $\dv{}{t} \in \mathfrak{X}^L(\mathbb{R})$ なので，命題\ref{thm:induced-LieAlg-hom}より，$X \coloneqq \gamma_* \bigl(\dv{}{t}\bigr) \in \mathfrak{X}^L(G)$ は $\dv{}{t}$ と $\gamma$-related な唯一の左不変ベクトル場である．
        ...
        \item 定理\ref{thm:left-invariant-complete}より $\forall X \in \mathfrak{X}^L(G)$ は\hyperref[def:complete-vecf]{完備}なので，
        ...
        　$\forall g \in G$ をとる．\hyperref[def:left-invariant]{左不変ベクトル場の定義}より $X \in \mathfrak{X}^L(G)$ は $X$ 自身と \hyperref[def:F-related]{$L_g$-related}なので，命題\ref{prop:natural-integral-curve}から $L_g \circ \gamma \colon \mathbb{R} \lto G$ もまた $X$ の積分曲線である．
...
\begin{mydef}[label=def:exp]{指数写像}
    ...
    \end{align}
    と定義する．ただし，$\gamma_{(X)} \colon \mathbb{R} \lto G$ は $\Lie{X} \in \mathfrak{X}^L(G)$ が \hyperref[prop:one-parameter-basic]{生成する1パラメータ部分群}である．
\end{mydef}
