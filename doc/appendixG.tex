...
特に，定理\ref{thm:tangentclassifier}により，\hyperref[def:tangent-classifier]{tangent classifier} $\tau_X$ は\hyperref[def:c-smooth]{conically smoothな層状化空間} $X \in \SNGLR_0$ の上に「strataum-wise」に（通常の意味の）接束をアサインしていることが分かる．
...
\begin{mydef}[label=def:infty-operad,breakable]{{$(\infty,\, 1)$}-オペラッド}
    ...
    \begin{itemize}
        ...
        \begin{center}
            ...
            {\inftyMap{\Cat{O}^\otimes}(x,\, y)} \arrow[r, "p"']             & {\inftyMap{\Ner(\FIN_*)}\bigl(p_{[0]}(x),\, p_{[0]}(y)\bigr) = \inftyMap{\Ner(\FIN_*)} (\expval{m},\, \expval{n})}
            \end{tikzcd}
        \end{center}
        と定義した
        \footnote{
...
\begin{proof}
    ...
    \begin{description}
        ...
        \end{align}
        が up to homotopy で定まる．\hyperref[def:infty-operad]{\textsf{\textbf{(Op-2)}}}より $\prod_{i=1}^n [\bar{\rho}^i_y]_*$ は\hyperref[def:htpy-equiv]{ホモトピー同値}である．
        さらに，\hyperref[def:infty-operad]{\textsf{\textbf{(Op-2)}}}より $\bar{\rho}_x^i$ が $p$-coCartesianなので，$[\bar{\rho}^i_x]^*$ は\hyperref[def:htpy-equiv]{ホモトピー同値}である\cite[\href{https://kerodon.net/tag/01U8}{Tag 01U8}]{kerodon}．
    ...
    \begin{description}
        ...
        　$\forall n \ge 0$ および $\forall x,\, y \in (\Cat{O}^\otimes_{\expval{n}})_0$ を固定する．このとき $\Id_{\expval{n}} \in \Hom{\FIN_*} \bigl( \expval{n},\, \expval{n} \bigr)$ に対して\textsf{\textbf{(OP-2)}}を用いることで，
        ホモトピー同値
        \begin{align}
            \inftyMap{\Cat{O}^\otimes_{\expval{n}}}(x,\, y) \simeq \inftyMap{\Cat{O}^\otimes} (x,\, y)_{\Id_{\expval{n}}} \lto \prod_{i=1}^n \inftyMap{\Cat{O}^\otimes} \bigl(x,\, y_i\bigr)_{\rho^i}
        \end{align}
        が得られる．さらに，\hyperref[def:Cart-coCart]{$p$-coCartesianな射の定義}からホモトピー同値
        \begin{align}
            \prod_{i=1}^n \inftyMap{\Cat{O}^\otimes_{\expval{1}}} \bigl(\rho^i_!{}_{[0]}(x),\, \rho^i_!{}_{[0]}(y)\bigr) \simeq \prod_{i=1}^n \inftyMap{\Cat{O}^\otimes} \bigl(x_i,\, y_i\bigr)_{\Id_{\expval{1}}} \lto \prod_{i=1}^n \inftyMap{\Cat{O}^\otimes} \bigl(x,\, y_i\bigr)_{\rho^i}
        \end{align}
        が得られる\footnote{\textsf{\textbf{(OP-2)}}における $y_i \in (\Cat{O}^\otimes_{\expval{1}})_0$ とは，ちょうど $\rho^i_!{}_{[0]}(y)$ のことである．}
