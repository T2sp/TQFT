...
\begin{mydef}[label=def:XU]{$[U]$-層}
    $\forall (X \to P) \in \Obj{\Snglr}$ および\hyperref[def:Bsc-induction]{basic} $\forall U = \mathbb{R}^n \times \Cone{Z} \in \Obj{\Bsc}$ を与える．
    このとき位相空間 $\bm{X_{[U]}} \in \Obj{\TOP}$ を，以下の $(1,\, 1)$-圏 $\TOP$ における\hyperref[def:pullback-pushout]{引き戻し}により定義する：
    \begin{center}
...
\begin{mytheo}[label=thm:tangentclassifier]{tangent classifierの局所的な構造}
    ...
    \end{align}
    が成り立つ．ただし，左辺では命題\ref{prop:Hom-htpy-equiv}を用いて得られるKan複体の\footnote{もしくは $\Kan$-豊穣圏の}ホモトピー同値 $\BSC_{[U]} \simeq \deloop{\Aut(U)} \simeq \deloop{\LO(U)}$ から誘導されるKan複体の謝 $\BSC_{[U]} \lto \deloop{\LO(n)}$ を用いた．
\end{mytheo}
