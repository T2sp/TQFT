...
次に，\hyperref[redef:braided-monoidal]{対象モノイダル圏}の $(\infty,\, 1)$-圏論における対応物を考える．
...
\begin{mydef}[label=def:sym-monoidal-infty]{対象モノイダル {$(\infty,\, 1)$-圏}}
    \hyperref[def:O-monoidal-infty]{$\Cat{O}$-モノイダル $(\infty,\, 1)$-圏} $\Cat{C}^\otimes \xrightarrow{p} \Cat{O}^\otimes$ において $\Cat{O}^\otimes = \Ner(\FIN_*)$ ならば，$\Cat{C}^\otimes$ は\textbf{対象モノイダル $\bm{(\infty,\, 1)}$-圏} (symmetric monoidal $(\infty,\, 1)$-category) であると言う．

    i.e. 対象モノイダル $(\infty,\, 1)$-圏とは，\hyperref[def:infty-operad]{$(\infty,\, 1)$-オペラッド} $\Cat{C}^\otimes \lto \Ner(\FIN_*)$ であって\hyperref[def:Cart-coCart]{coCartesianファイブレーション}でもあるもののこと~\cite[Example 2.1.2.18.]{Lurie2017HA}
\end{mydef}
...
\begin{mydef}[label=def:sym-monoidal-functor]{対象モノイダル {$(\infty,\, 1)$}-関手}
    \hyperref[def:sym-monoidal-infty]{対象モノイダル $(\infty,\, 1)$-圏} $\Cat{C}^\otimes \lto \Ner(\FIN_*)$,\, $\Cat{D}^\otimes \lto \Ner(\FIN_*)$ の間の
    \textbf{対象モノイダル $(\infty,\, 1)$-関手} (symmetric monoidal $(\infty,\, 1)$-functor) とは，$f \in \Alg{\Cat{C}}{\Cat{D}}$ であって全ての\hyperref[def:p-Cart]{coCartesianな1-射}を保存するもののこと．
    対象モノイダル $(\infty,\, 1)$-関手が成す\hyperref[def:fullsub-infty]{充満部分 $(\infty,\, 1)$-圏}を
    \begin{align}
