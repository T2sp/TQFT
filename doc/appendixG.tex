...
\begin{myexample}[label=ex:ordinary-GL]{通常の一般線形群}
    ...
    \end{align}
    が成り立つことを意味する．i.e. $\forall x \in \Delta^m_e$ に対して，$\tilde{T}_x \coloneqq \mathrm{proj}_{\mathbb{R}^n} \circ T|_{\{x\} \times U} \colon \mathbb{R}^n \lto \mathbb{R}^n$ とおくと
    \begin{align}
        \tilde{T}_x \bigl( t(x) u + p(x) \bigr) = t(x)\, \tilde{T}_x u + \tilde{T}_x p(x)
    \end{align}
    が成り立つため，$\tilde{T}_x$ は線型写像である．
    \hyperref[def:Snglr]{$(1,\, 1)$-圏 $\Snglr$ の定義}より $T$ は開埋め込みであり，単射である．故に，有限次元ベクトル空間の間の単射線型写像 $\tilde{T}_x$ は全単射でもあり，$\tilde{T}_x \in \LGL(n,\, \mathbb{R})$ だと分かった．
...
\begin{proof}
    ...
    \begin{description}
        ...
        \begin{align}
            &T \Bigl( x,\, \bigl( t(x)v + p(x),\, [0,\, *] \bigr) \Bigr)  \\
            &= T \circ \gamma_{[m]}{}_{t,\, p} \bigl( x,\, (v,\, [0,\, *]) \bigr) \\
            &= \gamma_{[m]}{}_{t,\, (Tp)_{\mathbb{R}^n}} \circ T\bigl( x,\, v,\, [0,\, *] \bigr) \\
            &= \biggl( x,\, \Bigl( t(x)\, \mathrm{proj}_{\mathbb{R}^n} \circ T \bigl( x,\, v,\, [0,\, *] \bigr) +\mathrm{proj}_{\mathbb{R}^n} \circ T \bigl( x,\, p(x),\, [0,\, *] \bigr),\, [0,\, *]  \Bigr)  \biggr) 
        \end{align}
...
\begin{mylem}[label=lem:strat-basic,breakable]{半順序集合 $[\BSC]$ による層状化}
    ...
    \begin{align}
        \mathcal{U}_x \coloneqq \bigl\{\, U \in \Obj{\Bsc} \bigm| \exists \iota \in \Hom{\Bsc}(U,\, X),\; x \in \iota(U) \,\bigr\} 
    \end{align}
...
\begin{proof}
    \begin{description}
        ...
        $\depth(X \to P)$ に関する数学的帰納法により示す．まず $\depth(X) = -1$ のとき，$X = \emptyset$ であり自明である．

        　$\depth(X \to P) \le d$ を充たす全ての $(X \xrightarrow{s} P) \in \Obj{\Snglr}$ についての証明が完了しているとする．
        まず，$\forall (U \xrightarrow{s} P) = (\mathbb{R}^n \xrightarrow{0} [0]) \times \Cone{Z \xrightarrow{t} Q} \in \Obj{\Bsc_{d+1,\, \le \infty}}$ について写像 $b_U \colon U \lto [\BSC]$ が連続であることを示そう．
        実際，$\forall x \in \mathbb{R}^n \times \{\mathrm{pt}\} \subset \mathbb{R}^n \times \Cone{Z}$ に対しては
        ...
        \begin{align}
            ...
            &= \dim_x \bigl( (\mathbb{R}^n \xrightarrow{0} [0]) \times \Cone{Z \xrightarrow{t} Q} \bigr)  - \dim_x (U_{s(x)}) \\
            &= n + \dim_{\mathrm{pt}} \Cone{Z \xrightarrow{t} Q} - n \\
            &= \depth_{\mathrm{pt}} \Cone{Z \xrightarrow{t} Q}\\
            &= \depth(U)
        \end{align}
        が成り立ち，$b_U(x) = [U]$ であることが分かった．
        ここで，\hyperref[def:topo-poset]{位相空間 $[\BSC]$} の開集合 $[U] \in \forall W \underset{\text{open}}{\subset} [\Bsc]$ をとる．
    \end{description}
