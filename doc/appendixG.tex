...
\begin{marker}
    以下では，混乱が生じにくい場合は層状化空間 $(X \xrightarrow{s} P)$ の $s$ と $P$ を省略する．
\end{marker}

いま，\hyperref[def:simplicial-top]{特異単体}の変種として，滑らかな（境界付き）多様体 $X$ の\textbf{滑らかな特異単体} (smooth singular simplicial set) と呼ばれる単体的集合を
\begin{align}
    \Sing^{\text{sm}}(X) \colon \OP{\Delta} &\lto \SETS, \\
    [n] &\lmto \Hom{\TOP}(\textcolor{red}{\Delta^n_e},\, X)_{\text{smooth}}, \\
...
\begin{align}
    ...
    (t,\, p) &\lmto \Bigl( \bigl( v,\, [s,\, z] \bigr) \mapsto \bigl( tv+p,\, [ts,\, z] \bigr) \Bigr) \eqqcolon \tilde{\gamma}_{[m]}{}_{t,\, p}
\end{align}
...
\begin{align}
    \Aut(Z)_m \coloneqq \bigl\{\, f \in \Hom{\SNGLR} (Z,\,Z)_m \bigm| \mathrm{proj}_{\Delta^m_e} \circ f = \mathrm{proj}_{\Delta^m_e},\; \exists f^{-1} \in \Hom{\Snglr} (\Delta^m_e \times Z,\, \Delta^m_e \times Z) \,\bigr\} 
\end{align}
なお，$\forall m \ge 0$ に対し集合 $\Aut(Z)_m$ は $(1,\, 1)$-圏 $\Snglr$ における1-射の合成に関して群になるため，Kan複体 $\Aut(Z)$ は\textbf{Kan群} (group Kan complex)\footnote{Kan複体であって，$\OP{\Delta} \lto \GRP$ でもあるもの．従って，homotopy hypothesisより位相群と見做すことができる．}と見做すことができる．

\begin{mydef}[label=def:stratified-GL]{層状化された一般線形群}
    勝手な\hyperref[def:Snglr]{basic} $U \coloneqq \bigl( (\mathbb{R}^n \to [0]) \times \Cone{Z \to P},\, \mathcal{A}_Z \bigr) \in \Obj{\Bsc}$ を与える．
