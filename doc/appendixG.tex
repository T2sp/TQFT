...
\begin{mydef}[label=def:O-monoidal-infty]{{$\Cat{O}$-モノイダル $(\infty,\, 1)$-圏}}
    $q \colon \Cat{C}^\otimes \lto \Ner(\FIN_*),\; r \colon \Cat{O}^\otimes \lto \Ner(\FIN_*)$ を\hyperref[def:infty-operad]{$(\infty,\, 1)$-オペラッド}とする．
    \hyperref[def:infty-operad-map]{$(\infty,\, 1)$-オペラッドの射} $p \colon \Cat{C}^\otimes \lto \Cat{O}^\otimes$ が
    ...
    \begin{description}
        ...
        $p \colon \Cat{C}^\otimes \lto \Cat{O}^\otimes$ を\hyperref[def:infinity-1]{$(\infty,\, 1)$-圏の関手}と見做したとき，\hyperref[def:Cart-coCart]{coCartesianファイブレーション}である．
    ...
    \end{description}
    特にこのとき，$p$ は $\Cat{C}^\otimes$ を $\bm{\Cat{O}}$\textbf{-モノイダル $(\infty,\, 1)$-圏}にしている ($p$ exhibits $\Cat{C}^\otimes$ as $\Cat{O}$-monoidal $(\infty,\, 1)$-category) と言う~\cite[Definition 2.1.2.13.]{Lurie2017HA}．
\end{mydef}
