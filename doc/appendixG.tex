...
\begin{proof}
    ...
    \begin{description}
        ...
        $\forall x \in (\Cat{O}^\otimes_{\expval{n}})_0$ は，定義から $x \in \Cat{O}^\otimes_0$ かつ $p_{[0]}(x) = \expval{n}$ を充たす．
        よって\hyperref[def:infty-operad]{\textsf{\textbf{(Op-1)}}}から，inertな射の族 $\Familyset[\big]{\rho^i \in  \Hom{\FIN_*}\bigl( \expval{n},\, \expval{1} \bigr)}{1 \le i \le n}$ の\hyperref[def:p-Cart]{$p$-coCartesian lift} $\Familyset[\big]{x \xrightarrow{\bar{\rho}^i} x_i \in (\Cat{O}^\otimes)_1}{1 \le i \le n}$ が存在する．これにより
        \begin{align}
            (\rho^1_{!},\, \dots,\, \rho^n_{!})_{[0]} \colon x \lmto (x_1,\, \dots,\, x_n)
        \end{align}
        と定義する\footnote{$p_{[0]} (x_i) = p_{[0]} (\partial^1_0 \bar{\rho}^i) = \partial^1_0 p_{[1]}(\bar{\rho}^i) = \partial^1_0 \rho^i = \expval{1}$ より $x_i \in (\Cat{O}^\otimes_{\expval{1}})_0$ である．}．

        \item[\textbf{$m \ge 1$-射}]　
        
        $\forall x,\, y \in (\Cat{O}^\otimes_{\expval{n}})_0$ を固定し，\hyperref[def:infty-operad]{\textsf{\textbf{(Op-1)}}}による\hyperref[def:p-Cart]{$p$-coCartesian lift} $\Familyset[\big]{(x \xrightarrow{\bar{\rho}_x^i} x_i) \in (\Cat{O}^\otimes)_1}{1 \le i \le n},\; \Familyset[\big]{(y \xrightarrow{\bar{\rho}_y^i} y_i) \in (\Cat{O}^\otimes)_1}{1 \le i \le n}$ をとっておく．
        ...
        \hyperref[def:infty-operad]{\textsf{\textbf{(Op-1)}}}より $\bar{\rho}_x^i$ が $p$-coCartesianなので，$[\bar{\rho}^i_x]^*$ は\hyperref[def:htpy-equiv]{ホモトピー同値}であり\cite[\href{https://kerodon.net/tag/01U8}{Tag 01U8}]{kerodon}，
        uo to homotopy で向きを逆にできる．このようにして $\forall m \ge 1$ 射の対応が定まる．
...
\begin{description}
    ...
    ある $c \in \Obj{\Cat{O}^\otimes_{\expval{n}}}_0$ および \hyperref[def:p-Cart]{$p$-coCartesian}な射 $\widehat{\rho_i} \in \Hom{\Cat{O}^\otimes} (c,\, c_i)_{\rho^i}$ が存在する．
\end{description}
