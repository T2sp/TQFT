...
\begin{align}
    ...
    \Hom{\BSC} (U,\, U)_m &= \bigl\{\, f \in \Hom{\Strat}(\Delta^m_e \times U,\, \Delta^m_e \times U) \bigm| \mathrm{proj}_{\Delta^m_e} \circ f = \mathrm{proj}_{\Delta^m_e} \,\bigr\}
\end{align}
であったから，
\begin{align}
    \gamma_{[m]} \colon \Sing^{\text{sm}} (\mathbb{R}_{\ge 0} \times \mathbb{R}^n)_m &\lto \Hom{\BSC} (U,\, U)_m, \\
    \Bigl( x \mapsto \bigl( t(x),\, p(x) \bigr)  \Bigr)  &\lmto \Bigl( \bigl(x,\, u\bigr) \mapsto \bigl(x,\, \tilde{\gamma}_{t(x),\, p(x)}(u)\bigr) \Bigr) \eqqcolon \gamma_{t,\, p}
\end{align}
と定義すればよい．
...
\begin{proof}
    \begin{description}
        ...
        射は全て\hyperref[def:c-smooth-map]{conically smooth}なので，$\forall f \in \Hom{\BSC}^0(U,\, U)_m$ の\hyperref[def:c-smooth-along]{微分} $D_0 f \coloneqq Df|_{\mathbb{R}^n \times \{0\} \times \Cone{Z}} \in \Hom{\Snglr}(U,\, U)$ が存在する．この $D_0$ がホモトピー逆を与える．
