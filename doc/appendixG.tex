...
\begin{mydef}[label=def:Bsc-induction,breakable]{圏 $\Bsc_{\le k+1,\, \le \infty}$}
    ...
    \begin{description}
        ...
        \footnote{\hyperref[def:dim-depth]{depthの定義}から $\depth (Z \to P) \le \dim (Z \to P)$ である．故に\exref{ex:depth-Cone}から，$\depth (Z \to P) \le \dim (Z \to P) = \depth U^n_Z - 1 \le k$ であること，i.e. $(Z \to P) \in \Obj{\SnglrC_{\le k,\, \le \infty}}$ が分かる．} $U^n_Z \in \Obj{\SnglrC_{\le k+1,\, \le \infty}}$ および，極大アトラス $\mathcal{A}_Z \in \Obj{\Sm_{\textcolor{red}{\le k},\, \le \infty} (Z \to P)}$ の組み
        \begin{align}
            (U^n_Z,\, \mathcal{A}_Z)
        \end{align}
        を対象とする．
...
\begin{align}
    ...
    (Z \xrightarrow{\alpha} W) &\lmto \Biggl(f \mapsto \biggl(\Bigl(w,\, x\Bigr) \mapsto \Bigl(w,\, \mathrm{proj}_Y \circ f \bigl(\alpha(w),\, x\bigr)\Bigr)\biggr)\Biggr)
\end{align}
で定義する．
...
$[-1] \coloneqq \emptyset$ とおく．このとき $d^n_j \in \Hom{\OP{\Delta}}([n],\, [n-1])$ に対して
\begin{align}
    (d^n_j)^{-1} ([i]) &= 
    \begin{cases}
        [i], &-1 \le i < j \\
    ...
    \begin{itemize}
        ...
        \begin{description}
            ...
            \begin{center}
                ...
                \begin{tikzcd}
                \mathrm{N}\bigl(\{n-1 \le n\}\bigr) \cong \Delta^1 \arrow[d, hook] \arrow[rd, "\bar{f}"] &                        \\
        ...
        \begin{description}
            ...
            \begin{center}
                % https://tikzcd.yichuanshen.de/#-3uAGYgUhMg1Bu8VhABIwGBZ6AI0HwsCBqAw6JJHAAFbo1AoCWR8HA3EDgwE7YFQnZ0E5nC7XMF3Z7QxCw15NZLU2mIRlPEG4rBgIaTOB8eY4vlDTgwAAeWDgODgFkkn2AnHRmKwFQoYiAA
                \begin{tikzcd}
                \mathrm{N}\bigl(\{0 \le 1\}\bigr) \cong \Delta^1 \arrow[d, hook] \arrow[rd, "\bar{f}"] &                        \\
...
\begin{center}
    ...
    \begin{tikzcd}
    \mathrm{N}\bigl(\{0 \le 1\}\bigr) \cong \Delta^1 \arrow[d, hook] \arrow[rd, "\bar{f}"] &                        \\
    \Lambda^n_0 \arrow[r,blue,"\forall \varphi_0"]                                                  & \Cat{E}
    \end{tikzcd}
\end{center}
...
\begin{center}
    \begin{tikzpicture} 
        ...
        ;
        \draw[->-=.5] (v0) --node[midway,above left]{$p_{[1]}(\bar{f}) = f$} (v1);
