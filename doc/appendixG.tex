...
\begin{mytheo}[label=thm:tangentclassifier]{tangent classifierの局所的な構造}
    ...
    \end{align}
    が成り立つ．ただし，左辺では命題\ref{prop:Hom-htpy-equiv}を用いて得られるKan複体の\footnote{もしくは $\Kan$-豊穣圏の}ホモトピー同値 $\BSC_{[U]} \simeq \deloop{\Aut(U)} \simeq \deloop{\LO(U)}$ から誘導されるKan複体の謝 $\BSC_{[U]} \lto \deloop{\LO(\mathbb{R}^n)}$ を用いた．
\end{mytheo}
...
\begin{mydef}[label=def:infty-operad,breakable]{{$(\infty,\, 1)$}-オペラッド}
    ...
    \begin{itemize}
        ...
        $\bar{f} \in \inftyMap{\Cat{O}^\otimes}(c,\, c')_{0}$ であって $p_{[1]}(\bar{f}) = f$ を充たすもの全体から定まる，$\inftyMap{\Cat{O}^\otimes} (c,\, c') \in \SPACES_0$
        の
