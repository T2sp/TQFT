...
\begin{mydef}[label=def:infty-operad-map]{{$(\infty,\, 1)$}-オペラッドの射}
    ...
    \begin{description}
        ...
        \item[\textbf{(Opmap-2)}] 1-射の間の写像 $f_{[1]} \colon (\Cat{O}^{\otimes})_1 \lto (\Cat{O}'{}^{\otimes})_1$ により，任意の\hyperref[def:inert-active]{inertな射}の \hyperref[def:infty-operad]{\textsf{\textbf{(Op-1)}}} による\hyperref[def:Cart-coCart]{p-coCartesian}な持ち上げが保たれる．
    \end{description}
\end{mydef}
...
\begin{myprop}[label=prop:c-operad]{色付きオペラッドとcategory of operators}
    ...
    \begin{align}
        p \colon \Cat{O}^\otimes \lto \FIN_*
    \end{align}
    において，$\Cat{O}^\otimes$ がある\hyperref[def:c-operad]{色付きオペラッド}の\hyperref[def:cat-of-operators]{category of operators}と圏同値になる必要十分条件は，
