...
これまでと同様に，\hyperref[def:stratified-space]{層状化空間}$\Cone{Z \xrightarrow{s} P} = \bigl(\Cone{Z} \xrightarrow{\Cone{s}} \Cone{P}\bigr)$ の\hyperref[def:str-cone]{コーンポイント}のことを $\mathrm{pt} \in \Cone{Z}$ と書き，$\mathbb{R}^n \times \{\mathrm{pt}\} \subset \mathbb{R}^n \times \Cone{Z}$ のことを $\mathbb{R}^n$ と略記することにする．
さらに，点 $(0,\, \mathrm{pt}) \in \mathbb{R}^n \times \Cone{Z}$ のことを $0 \in \mathbb{R}^n \times \Cone{Z}$ と略記し，$U$ の\textbf{原点} (origin) と呼ぶことにする．
...
\begin{align}
    \Sing^{\text{sm}} (\mathbb{R}_{\ge 0} \times \mathbb{R}^n)_m &= \Hom{\TOP} (\Delta_e^m,\, \mathbb{R}_{\ge 0} \times \mathbb{R}^n)_{\text{smooth}}, \\
...
\begin{align}
    \Aut(Z)_m \coloneqq \bigl\{\, f \in \Hom{\SNGLR} (Z,\,Z)_m \bigm| \exists f^{-1} \in \Hom{\Snglr} (\Delta^m_e \times Z,\, \Delta^m_e \times Z) \,\bigr\} 
\end{align}
...
\begin{mydef}[label=def:stratified-GL]{層状化された一般線形群}
    ...
    \begin{align}
        \LGL(U)_m \coloneqq \bigl\{\, T \in \Hom{\SNGLR} (U,\, U)_m \bigm| \forall (t,\, p) \in \Sing^{\text{sm}}(\mathbb{R}_{\ge 0} \times \mathbb{R}^n)_m,\; T \circ \gamma_{[m]}{}_{t,\, p} = \gamma_{[m]}{}_{t,\, (Tp)_{\mathbb{R}^n}} \circ T  \,\bigr\} 
    \end{align}
    ただし，$(Tp)_{\mathbb{R}^n} \coloneqq \mathrm{proj}_{\mathbb{R}^n} \circ T|_{\Delta^m_e \times \mathbb{R}^n \times \{0\}} \circ p \colon \Delta^m_e \lto \mathbb{R}^n$ と略記した．
...
\begin{align}
    \Hom{\BSC}^0 (U,\, U)_m &\coloneqq \bigl\{\, f \in \Hom{\SNGLR} (U,\, U)_m \bigm| \forall x \in \Delta^m_e,\; f(x,\, 0) = (x,\, 0) \,\bigr\}, \\
    \Aut^0 (U)_m &\coloneqq \bigl\{\, f \in \Hom{\BSC}^0 (U,\, U)_m \bigm| \forall x \in \Delta^m_e,\; \exists (f|_{\{x\} \times U})^{-1} \in \Hom{\Snglr}(U,\, U) \,\bigr\}
\end{align}
...
\begin{proof}
    ...
    $\forall x \in \Delta^m_e$ および $\forall t \in \mathbb{R}_{\ge 0}$ に対して $\gamma_{[m]}{}_{t,\, 0} (x,\, 0) = (x,\, 0)$ が成り立つので，
    \begin{align}
        T(x,\, 0) = T \circ \gamma_{[m]}{}_{0,\, 0} (x,\, 0) = \gamma_{[m]}{}_{0,\, 0} \bigl( T(x,\, 0) \bigr) = (x,\, 0)
    \end{align}
    が言える．
    また，\hyperref[def:Snglr]{$(1,\, 1)$-圏 $\Snglr$ の定義}より $T \in \Hom{\Snglr}(\Delta^m_e \times U \lto \Delta^m_e \times U)$ は開埋め込みであるから単射である． 
    故に $T$ が全射であることを示せば良い．
\end{proof}
...
\begin{myprop}[label=prop:Hom-htpy-equiv]{}
    Kan複体の包含写像
    \begin{align}
...
\begin{mylem}[label=lem:basic-conservative]{}
    ...
    \begin{enumerate}
        ...
        \item $f \in \Hom{\Bsc}(U,\, V)$ であって，$f(U) \cap \bigl(\mathbb{R}^m \times \{*\}\bigr) \neq \empty$ を充たすものが存在する．
        \item $(1,\, 1)$-圏 $\Bsc$ における $U$ から $V$ への同型射全体を対象に持つ，\hyperref[lem:Kan-enriched]{Kan複体 $\Hom{\BSC}(U,\, V)$} の\hyperref[def:fullsub-infty]{充満部分Kan複体} $\mathcal{I}\mathrm{so}_{\BSC} (U,\, V) \hookrightarrow \Hom{\BSC}(U,\, V)$ について，その包含写像はKan複体の弱ホモトピー同値である．
...
\begin{proof}
    \begin{description}
        ...
        命題\ref{prop:Hom-htpy-equiv}より従う．
