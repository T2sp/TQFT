...
\begin{mydef}[label=def:infty-operad,breakable]{{$(\infty,\, 1)$}-オペラッド}
    ...
    ただし，以下の記法を採用した：
    \begin{itemize}
        \item 点 $\expval{n} \in \Ner (\FIN_*)_0$ における\hyperref[def:infinity-1]{$(\infty,\, 1)$-圏の関手} $p \colon \Cat{O}^\otimes \lto \Ner (\FIN_*)$ の\textbf{ファイバー} $\Cat{O}^{\otimes}_{\expval{n}}$ を，
...
\begin{proof}
    ...
    \begin{description}
        ...
        $\forall x,\, y \in (\Cat{O}^\otimes_{\expval{n}})_0$ を固定しよう．
