...
\begin{mydef}[label=def:infty-operad,breakable]{{$(\infty,\, 1)$}-オペラッド}
    ...
    ただし，点 $\expval{n} \in \Ner (\FIN_*)_0$ における\hyperref[def:infinity-1]{$(\infty,\, 1)$-圏の関手} $p \colon \Cat{O}^\otimes \lto \Ner (\FIN_*)$ の\textbf{ファイバー}\footnote{これ自体が $(\infty,\, 1)$-圏である．} $\Cat{O}^{\otimes}_{\expval{n}}$ を，\hyperref[def:pullback-infty]{$(\infty,\, 1)$-圏のpullback}
    \begin{center}
