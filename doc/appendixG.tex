...
\begin{mydef}[label=def:lim-colim-infty]{{$(\infty,\, 1)$}-圏のlimit/colimit}
    ...
    \begin{itemize}
        ...
        \item $D$ の\textbf{colimit}とは，\hyperref[def:overcat-infty]{コスライス $(\infty,\, 1)$-圏} $\Cat{C}_{D/}$ における\hyperref[def:initial-final-infty]{始対象}のこと．$\bm{\colim D} \in \Cat{C}_0$ と書く．
    \end{itemize}
\end{mydef}
...
\begin{myexample}[label=def:pullback-infty]{pullback}
    ...
    \end{align}
    と書く．\exref{ex:Map1}および補題\ref{lem:n-fold-Map}より，$D \in \Hom{\sSet}(\Delta^1 \times \Delta^1,\, \Cat{C})$ は
    \begin{align}
        D = \quad
        \begin{tikzpicture}[baseline={([yshift=-.5ex]current bounding box.center)}]
...
\begin{myprop}[label=prop:Op-infty-limit]{}
    ...
    i.e. 任意の小さな \hyperref[def:infinity-1]{$(\infty,\, 1)$-圏の図式} $D \colon \Cat{K} \lto \OPinfty$ に対して，$\Cat{K} \xrightarrow{D} \OPinfty \xrightarrow{\text{forget}} \CATinfty{}_{/\Ner(\FIN_*)}$ が\hyperref[def:lim-colim-infty]{極限} $\bigl(p \colon \Cat{O} \lto \Ner(\FIN_*)\bigr) \in (\CATinfty{}_{/\Ner(\FIN_*)})_0$ を持つならば，$p$ は\hyperref[def:infty-operad]{$(\infty,\, 1)$-オペラッド}の構造を持ち，かつそれが $\lim_{\Cat{K}} D = \bigl(p \colon \Cat{O} \lto \Ner(\FIN_*)\bigr)$ を充たす．
 \end{myprop}
...
\begin{mydef}[label=def:tensor-sifted]{{$\otimes$-sifted cocomplete}}
    \begin{itemize}
        \item \hyperref[def:sym-monoidal-infty]{対称モノイダル $(\infty,\, 1)$-圏} $p \colon \Cat{V}^\otimes \Ner(\FIN_*)$
