...
\begin{mydef}[label=def:infty-operad,breakable]{{$(\infty,\, 1)$}-オペラッド}
    ...
    \begin{itemize}
        ...
        の\footnote{\hyperref[def:infty-1]{Kan複体} $\Hom{\Cat{O}^\otimes} (c,\, c') \in \SPACES_0$ は，\hyperref[def:infty-Hom]{$(\infty,\, 1)$-圏におけるHom関手} $\inftyMap{\Cat{O}^\otimes} \colon \OP{(\Cat{O}^\otimes)} \times \Cat{O}^\otimes \lto \SPACES$ による対象 $(c,\, c') \in (\OP{(\Cat{O}^\otimes)} \times \Cat{O}^\otimes)_0$ の像である．}
...
\begin{proof}
    \begin{description}
        ...
        　$\forall n \ge 0$ および $\forall c,\, c' \in (\Cat{O}^\otimes_{\expval{n}})_0$ を固定する．このとき $\Id_{\expval{n}} \in \Hom{\FIN_*} \bigl( \expval{n},\, \expval{n} \bigr)$ に対して\textsf{\textbf{(OP-2)}}を用いることで\footnote{\textsf{\textbf{(OP-2)}}における $c'_i \in (\Cat{O}^\otimes_{\expval{1}})_0$ とは，ちょうど $\rho^i_!{}_{[0]}(c')$ のことである．}，
        ホモトピー同値
        \begin{align}
            \inftyMap{\Cat{O}^\otimes} (c,\, c')_{\Id_{\expval{n}}} \lto \prod_{i=1}^n \inftyMap{\Cat{O}^\otimes} \bigl(c,\, \rho^i_!{}_{[0]}(c')\bigr)_{\rho^i}
        \end{align}
        が得られる．さらに，\hyperref[def:Cart-coCart]{$p$-coCartesianな射の定義}からホモトピー同値
        \begin{align}
            \prod_{i=1}^n \inftyMap{\Cat{O}^\otimes} \bigl(\rho^i_!{}_{[0]}(c),\, \rho^i_!{}_{[0]}(c')\bigr)_{\Id_{\expval{1}}} \lto \prod_{i=1}^n \inftyMap{\Cat{O}^\otimes} \bigl(c,\, \rho^i_!{}_{[0]}(c')\bigr)_{\rho^i}
        \end{align}
        が得られるため，\eqref{eq:Segalmap}が\hyperref[def:FF-ES-infty]{忠実充満}だと分かった．
