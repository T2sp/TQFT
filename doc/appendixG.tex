...
\begin{center}
    ...
    \OP{\Cat{C}} \times \Cat{C} \arrow[r, red, "\Hom{\Cat{C}} \coloneqq \St(\mathrm{pr})"']    & \SPACES                                
    \end{tikzcd}
\end{center}
...
\begin{mydef}[label=def:Cart-coCart,breakable]{デカルトファイブレーション}
    ...
    \begin{itemize}
        ...
        %         % https://tikzcd.yichuanshen.de/#-3uAGYgUhMg1Bu8VhABIwGBZ6AI0HwsCBqAw6JJHAAFbo1AoCWR8HA3EDgwE7YFQnZ0E5nC7XMF3Z7QxCw15NZLU2mIRlPEG4rBgIaTOB8eY4vlDTgwAAeWDgODgFkkn2AnHRmKwFQoYiAA
        %         \begin{tikzcd}
        %         \Ner \bigl(\{n-1 \le n\}\bigr) \cong \Delta^1 \arrow[d, hook] \arrow[rd, "\bar{f}"] &                        \\
        %         \Lambda^n_n \arrow[r,blue, "\forall \varphi_0"] \arrow[d, hook]                                                    & \Cat{E} \arrow[d, "p"] \\
        %         \Delta^n \arrow[r, blue,"\forall \varphi"'] \arrow[ru, red, "\exists \bar{\varphi}", dashed]       & \Cat{B}               
        %         \end{tikzcd}
        %     \end{center}
        %     ただし，$\sSet$ の射 $\bar{f} \colon \Ner \bigl(\{n-1 \le n\}\bigr) \lto \Cat{E}$ とは
        %     \begin{align}
        %         \bar{f}_{[1]}
        %             \left( 
        %                \begin{tikzpicture}[baseline={([yshift=-.5ex]current bounding box.center)}]
        ...
        \begin{description}
            ...
            %     % https://tikzcd.yichuanshen.de/#-3uAGYgUhMg1Bu8VhABIwGBZ6AI0HwsCBqAw6JJHAAFbo1AoCWR8HA3EDgwE7YFQnZ0E5nC7XMF3Z7QxCw15NZLU2mIRlPEG4rBgIaTOB8eY4vlDTgwAAeWDgODgFkkn2AnHRmKwFQoYiAA
            %     \begin{tikzcd}
            %     \Ner \bigl(\{0 \le 1\}\bigr) \cong \Delta^1 \arrow[d, hook] \arrow[rd, "\bar{f}"] &                        \\
            %     \Lambda^n_0 \arrow[r,blue,"\forall \varphi_0"] \arrow[d, hook]                                                    & \Cat{E} \arrow[d, "p"] \\
            %     \Delta^n \arrow[r, blue,"\forall \varphi"'] \arrow[ru, red, "\exists \bar{\varphi}", dashed]       & \Cat{B}               
            %     \end{tikzcd}
            % \end{center}
            % ただし，$\sSet$ の射 $\bar{f} \colon \Ner \bigl(\{0 \le 1\}\bigr) \lto \Cat{E}$ とは
            % \begin{align}
            %     \bar{f}_{[1]}
            %         \left( 
            %            \begin{tikzpicture}[baseline={([yshift=-.5ex]current bounding box.center)}]
...
\begin{mydef}[label=def:infty-operad,breakable]{{$(\infty,\, 1)$}-オペラッド}
    ...
    \hyperref[def:infinity-1]{$(\infty,\, 1)$-圏の関手} $p \colon \Cat{O}^{\otimes} \lto \Ner (\FIN_*)$ であって以下の条件を充たすもののこと：
    \begin{description}
        \item[\textbf{(Op-1)}]　
        ...
        innertな射 $\rho^i \in  \Hom{\FIN_*}\bigl( \expval{n},\, \expval{1} \bigr)$ に \textsf{\textbf{(Op-1)}} を適用して得られる\hyperref[def:Cart-coCart]{$p$-coCartesian}な1-射の族 $\Familyset[\big]{c' \xrightarrow{\bar{\rho}^i} c'_i \in (\Cat{O}^\otimes)_1}{1 \le i \le n}$ が誘導する\hyperref[def:infinity-1]{Kan複体の関手}
        \begin{align}
        ...
        ある $c \in (\Cat{O}^\otimes_{\expval{n}})_0$ および \hyperref[def:Cart-coCart]{$p$-coCartesian}な1-射 $\widehat{\rho_i} \in \bigl(\Hom{\Cat{O}^\otimes} (c,\, c_i)_{\rho^i}\bigr)_0$ が存在する．
    \end{description}
    ...
    \begin{itemize}
        ...
        $\bar{f} \in \Hom{\Cat{O}^\otimes}(c,\, c')_{0}$ であって $p_{[1]}(\bar{f}) = f$ を充たすもの全体が生成する，\hyperref[def:infty-1]{Kan複体} $\Hom{\Cat{O}^\otimes} (c,\, c') \in \SPACES_0$\footnote{$\Hom{\Cat{O}^\otimes} (c,\, c') \in \SPACES_0$ は，\hyperref[def:infty-Hom]{$(\infty,\, 1)$-圏におけるHom関手} $\Hom{\Cat{O}^\otimes} \colon \OP{(\Cat{O}^\otimes)} \times \Cat{O}^\otimes \lto \SPACES$ による対象 $(c,\, c') \in (\OP{(\Cat{O}^\otimes)} \times \Cat{O}^\otimes)_0$ の像である．}
        の最大の部分Kan複体を
        \begin{align}
            \bm{\Hom{\Cat{O}^\otimes} (c,\, c')_f} \hookrightarrow \Hom{\Cat{O}^\otimes} (c,\, c')
        \end{align}
        と書いた．
    \end{itemize}
\end{mydef}
...
\begin{mydef}[label=def:p-coCart-ord,breakable]{{$(1,\, 1)$}-圏におけるcoCartesian fibration}
    ...
    \begin{itemize}
        \item $\Cat{E}$ の射 $\bar{f} \in \Hom{\Cat{E}} (\bar{x},\, \bar{y})$ が以下の条件を充たすとき，$f$ は\textbf{$\bm{p}$-coCartesian}であると言う：
        \begin{description}
...
\begin{description}
    ...
    任意の\hyperref[def:innert-active]{innertな}射 $f \in \Hom{\FIN_*} \bigl( \expval{m},\, \expval{n} \bigr)$ および $\forall c \in \Obj{\Cat{O^\otimes}_{\expval{m}}}$ に対して，
    ...
    innertな射 $\rho^i \in  \Hom{\FIN_*}\bigl( \expval{n},\, \expval{1} \bigr)$ に \textsf{\textbf{(Op-ord-1)}} を適用して得られる\hyperref[def:p-coCart-ord]{$p$-coCartesian}な射の族 $\Familyset[\big]{\bar{\rho}^i \in \Hom{\Cat{O}^\otimes} (c',\, c'_i)}{1 \le i \le n}$ が誘導する写像
    \begin{align}
        \Hom{\Cat{O}^\otimes} (c,\, c')_f &\lto \prod_{i=1}^n \Hom{\Cat{O}^\otimes} (c,\, c'_i)_{\rho^i \circ f}, \\
        \varphi &\lmto (\bar{\rho}^1 \circ \varphi,\, \dots,\, \bar{\rho}^n \circ \varphi)
    \end{align}
    は，$(1,\, 1)$-圏 $\SETS$ における\hyperref[def:iso]{同型射}である．

    \item[\textbf{(OP-ord-3)}]　 
    
    $\forall c_1,\, \dots,\, c_n \in \Obj{\Cat{O}^\otimes}$ であって $p(c_i) = \expval{1}$ を充たすものに対して，
...
\begin{myprop}[label=prop:operad-ord]{色付きオペラッドの再構成}
    条件\textsf{\textbf{(Op-ord-1)}}-\textsf{\textbf{(Op-ord-3)}}を充たす $(1,\, 1)$-圏の関手\eqref{eq:operad-ord}から次のように構成されたデータの組み $\Cat{O}$ は\textbf{色付きオペラッド} (colored operad) を成す：
    \begin{itemize}
        \item 対象の集まり $\Obj{\Cat{O}} \coloneqq \Obj{\Cat{O}^\otimes}$
        \item $\forall x_1,\, \dots,\, x_n,\, y \in \Obj{\Cat{O}}$ に対して，以下の3つ組 (multimorphism) 全体が成す集合 $\Mul{\Cat{O}} \bigl( (x_1,\, \dots,\, x_n),\, y \bigr)$：
        \begin{enumerate}
            \item $x_1,\, \dots,\, x_n$ に対して\textsf{\textbf{(Op-ord-3)}}を適用することにより定まる $X \in \Obj{\Cat{O}^\otimes_{\expval{n}}}$ 
            \item $x_1,\, \dots,\, x_n$ に対して\textsf{\textbf{(Op-ord-3)}}を適用することにより定まる\hyperref[def:Cart-coCart]{$p$-coCartesian}な射の族 $\Familyset[\big]{\widehat{\rho_i} \in \Hom{\Cat{O}^\otimes}(X,\, x_i)_{\rho^i}}{1 \le i \le n}$
            \item \hyperref[def:innert-active]{active}な射
            \footnote{$\alpha_n$ は
                \begin{align}
                    \alpha_n(i) \coloneqq \begin{cases}
                        1, &i \neq *, \\
                        *, &i = *
                    \end{cases}
                \end{align}
        ...
        \end{enumerate}
        \item $n$ 個の多射 $F^1 \in \Mul{\Cat{O}} \bigl( (x^1_1,\, \dots,\, x^1_{m_1}),\, y_1 \bigr),\, \dots,\, F^n \in \Mul{\Cat{O}} \bigl( (x^n_1,\, \dots,\, x^n_{m_n}),\, y_n \bigr)$ と1つの多射 $G \in \Mul{\Cat{O}} \bigl( (y_1,\, \dots,\, y_n),\, z\bigr)$ の合成
        \begin{align}
            G \circ (F^1;\, \dots \, ;\, F^n) \in \Mul{\Cat{O}} \bigl( (x^1_1,\, \dots,\, x^1_{m_1};\, \dots\, ;\, x^n_1,\, \dots,\, x^n_{m_n}),\, z \bigr) 
        \end{align}
        を次のように定義する：
        \begin{enumerate}
            \item $x^1_1,\, \dots,\, x^n_{m_n} \in \Obj{\Cat{O}}$ に対して\textsf{\textbf{(Op-ord-3)}}を適用することにより定まる $X \in \Obj{\Cat{O}^\otimes_{\expval{m_1 + \cdots + m_n}}}$ 
            \item $x^1_1,\, \dots,\, x^n_{m_n} \in \Obj{\Cat{O}}$ に対して\textsf{\textbf{(Op-ord-3)}}を適用することにより定まる \hyperref[def:Cart-coCart]{$p$-coCartesian}な射の族 $\Familyset[\big]{\widehat{\rho_i} \in \Hom{\Cat{O}^\otimes}(X,\, x^j_i)_{\rho^i}}{\substack{1 \le i \le m_1 + \cdots + m_n, \\ 1 \le j \le n}}$
            \item 
        \end{enumerate}
