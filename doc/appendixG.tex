...
\begin{myexample}[label=ex:ordinary-GL]{通常の一般線形群}
    ...
    \hyperref[def:Snglr]{$(1,\, 1)$-圏 $\Snglr$ の定義}より $T$ は開埋め込みであり，単射である．故に，次元の等しい有限次元ベクトル空間の間の単射線型写像 $T_x$ は全単射でもあり，$T_x \in \LGL(n,\, \mathbb{R})$ だと分かった．
...
\begin{proof}
    ...
    \begin{description}
        ...
        $(\infty,\, 1)$-圏 $\BSC$ の定義において登場する射は全て\hyperref[def:c-smooth-map]{conically smooth}なので，$\forall f \in \Hom{\BSC}^0(U,\, U)_m$ の\hyperref[def:c-smooth-along]{微分}
        \begin{align}
            Df \colon T(\Delta^m_e \times \mathbb{R}^n) \times \Cone{Z} \lto T(\Delta^m_e \times \mathbb{R}^n) \times \Cone{Z}
        \end{align}
        が存在する\footnote{$\Delta^m_e \approx \mathbb{R}^m$ である．}．
        このとき，$T(\Delta^m_e \times \mathbb{R}^n) \approx (\Delta^m_e \times \mathbb{R}^n) \times (\Delta^m_e \times \mathbb{R}^n)$ であり，底空間の勝手な元 $(x,\, p) \in \Delta^m_e \times \mathbb{R}^n$ に対して
        \begin{align}
            D_{(x,\, p)}f \coloneqq Df|_{\Delta^m_e \times \mathbb{R}^n \times \{(x,\, p)\} \times \Cone{Z}} \in \Hom{\BSC}(U,\, U)_m \subset \Hom{\Snglr} (\Delta^m_e \times U,\, \Delta^m_e \times U)
        \end{align}
        とおこう．特に，$D_{(x,\, 0)} f \in \Hom{\BSC}^0(U,\, U)_m$ であることに注意する．

        　$\forall x \in \Delta^m_e$ を1つ固定する．まずは $\forall f \in \Hom{\BSC}^0(U,\, U)_m$ に対して $D_{(x,\, 0)} f \in \LGL(U)_m$ であること，i.e. Kan複体の射
        \begin{align}
            D_{0} \colon \Hom{\BSC}^0(U,\, U) \lto \LGL(U)
        \end{align}
        が定まることを示そう．
        実際，$\forall (t,\, p) \in \Sing^{\text{sm}} (\mathbb{R}_{\ge 0} \times \mathbb{R}^n)_m$ に対して\footnote{ややこしいので $\gamma_{[m]}$ を $\gamma$ と略記している．}
        \begin{align}
            D_{(x,\, 0)} f \circ \gamma_{[m]}{}_{t,\, p}
            &= \Bigl( \lim_{s \to +0} \gamma_{\frac{1}{s},\, 0} \circ f \circ \gamma_{s,\, 0}  \Bigr) \circ \gamma_{t,\, p} \\
            &= \lim_{s \to +0} \gamma_{\frac{1}{s},\, 0} \circ f \circ \gamma_{ts,\,sp} \\
            &= \cdots \\
            &= \gamma_{[m]}{}_{t,\, (D_{(x,\, 0)} f(p))_{\mathbb{R}^n}} \circ D_{(x,\, 0)} f
        \end{align}
        と計算できる．
