...
\begin{myexample}[label=def:CG]{圏 $\Cat{C}_G$・$\VecG{G}$}
...
\begin{myexample}[label=def:CGa]{圏 $\Cat{C}_G^\alpha$・$\mathrm{Vec}^\alpha_G$}
...
\begin{myexample}[label=def:CG-functor]{圏 $\VecG{G}^\alpha$ のテンソル関手}
    ...
    \begin{align}
        ...
        \alpha_1 = f^* \omega_2 \cdot \delta \mu
    \end{align}
...
\begin{myexample}[label=def:CG-monoidalnat]{圏 $\VecG{G}^\alpha$ のモノイダル自然変換}
    ...
    \hyperref[def:monidal-nat]{\textsf{\textbf{(テンソル積の保存)}}}の条件から，$\forall g_1,\, g_2 \in G_1$ に対して
    \begin{align}
