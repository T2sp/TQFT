    ...
    \begin{mydef}[label=def:semisimple-cat]{単純・半単純}
        \begin{itemize}
            ...
            ちょうど2種類のモノ射 $\textcolor{blue}{i} \colon \textcolor{blue}{U} \hookrightarrow X$ だけが存在し，それぞれ $0$ であるか同型射であることを言う\footnote{従って零対象 $0 \in \Obj{\Cat{C}}$ は単純ではない．}．
    ...
    \begin{mydef}[label=def:finite-abcat]{有限性}
        ...
        \begin{itemize}
            ...
            \end{align}
            であって $X_i/X_{i-1} \coloneqq \Coker (X_{i-1} \hookrightarrow X_i) \quad (1 \le \forall i \le n)$ が\hyperref[def:semisimple-cat]{単純対象}であるようなもの（\textbf{Jordan-H\"{o}lder列}と言う）が存在することを言う．このときの $n$ を $X$ の\textbf{長さ} (length) と呼ぶ
            \footnote{Jordan-H\"{o}lderの定理~\cite[THEOREM 1.5.4, p.5]{etingof2015tensor}から，$X$ の任意のJordan-H\"{o}lder列は存在すれば同一の長さを持つ．}．
        \end{itemize}
        ...
        \begin{itemize}
            ...
            \begin{description}
                ...
                \item[\textbf{(Fin-3)}] $\Obj{\Cat{C}}$ の任意の\hyperref[def:semisimple-cat]{単純対象}が射影的被覆
                \footnote{$P \in \Obj{\Cat{C}}$ が\textbf{射影的} (projective) であるとは，関手 $\Hom{\Cat{C}}(P,\, \mhyphen) \colon \Cat{C} \lto \VEC{\mathbb{K}}$ が完全関手であることを言う．$X \in \Obj{\Cat{C}}$ の\textbf{射影的被覆} (projective cover) とは，射影的対象 $P_X \in \Obj{\Cat{C}}$ とエピ射 $p_X \colon P_X \lto X$ の組み $(P_X,\, p_X)$ であって，任意の射影的対象 $\textcolor{blue}{P} \in \Obj{\Cat{C}}$ およびエピ射 $\textcolor{blue}{p} \colon \textcolor{blue}{P} \lto X$ に対してあるエピ射 $\textcolor{red}{h} \colon \textcolor{blue}{P} \lto P_X$ が存在して $p_X \circ \textcolor{red}{h} = p$ を充たすようなもののこと．}
                を持つ．i.e. $\Cat{C}$ は\textbf{十分射影的対象を持つ} (has enough projectives)
    ...
    \begin{mydef}[label=def:starcat]{$C^*$-圏}
        ...
        以下の条件を充たす\hyperref[redef:unitary]{ダガー圏} $(\Cat{C},\, \dagger)$ は $\bm{C^*}$\textbf{-圏}と呼ばれる~\cite[p.5]{Reutter2020fusion}：
        \begin{description}
...
$f \in \End_{\Cat{C}} (x)$ に対してトレースを定義するためには旋回構造が必要である．
...
\begin{proof}
    \begin{enumerate}
        ...
        \end{align}
        \item $\forall x \in \Simp (\Cat{C})$ に対して示せば十分である．実際，
        \begin{align}
            v_x
            \coloneqq 
            \begin{tikzpicture}[baseline={([yshift=-.5ex]current bounding box.center)}]
                \path 
                    ...
                    +(2,0) coordinate (z_2)
                ;
                \draw[-<-=.5] (z_1) -- node[pos=.5,right,looseness=1.5] {$x^{**}$} (-90:1);
                \draw[-<-=.5] (x_1) to[out=-90,in=-90] (z_1);
                \draw[->-=.5] (x_1) -- (x_2);
                \draw[-<-=.7] (y_1) .. controls +(90:.5) and +(-90:.5) .. (z_2);
                \path[spath/save=Br] (z_1) .. controls +(90:.5) and +(-90:.5) .. (y_2);
                \Over{Br}{.1}{.9}{black}{.4pt}
                \draw[->-=.7,spath/use=Br];
                \draw[->-=.5] (x_2)  -- node[midway,left] {$x$} +(90:1);
                \draw[->-=.5] (y_2) to[out=90,in=90,looseness=1.5] (z_2);
            \end{tikzpicture}
        \end{align}
        が定数倍を除いて $u_x$ の逆を与える．
    \end{enumerate}
...
\begin{myprop}[label=preprop:MTC]{MTCの性質}
...
\begin{myprop}[label=prop:DeligneProduct]{Deligneのテンソル積の基本性質}
    \begin{enumerate}
        ...
        \item $\Cat{C},\, \Cat{D}$ が\hyperref[def:ringcat]{(多重) 環圏}/\hyperref[def:tensorfusion-cat]{(多重) テンソル圏}ならば $\Cat{C} \boxtimes \Cat{D}$ は\hyperref[def:ringcat]{(多重) 環圏}/\hyperref[def:tensorfusion-cat]{(多重) テンソル圏}である．
            特に，$\Cat{C} \boxtimes \Cat{D}$ の\hyperref[redef:monoidal-category]{モノイダル圏}としての構造は次のようにして定まる：
            \begin{align}
                (x_1 \boxtimes y_1) \otimes^{\Cat{C} \boxtimes \Cat{D}} (x_2 \boxtimes y_2) &\coloneqq (x_1 \otimes^{\Cat{C}} x_2) \boxtimes (y_1 \otimes^{\Cat{D}} y_2), \\
                1_{\Cat{C} \boxtimes \Cat{D}} &\coloneqq 1_{\Cat{C}} \boxtimes 1_{\Cat{D}}, \\
                a^{\Cat{C} \boxtimes \Cat{D}}_{x_1 \boxtimes y_1,\, x_2 \boxtimes y_2,\, x_3 \boxtimes y_3} &\coloneqq a^{\Cat{C}}_{x_1,\, x_2,\, x_3} \boxtimes a^{\Cat{D}}_{y_1,\, y_2,\, y_3}, \\
                l^{\Cat{C} \boxtimes \Cat{D}}_{x \boxtimes y} &\coloneqq l^{\Cat{C}}_x \boxtimes l^{\Cat{D}}_y \\
                r^{\Cat{C} \boxtimes \Cat{D}}_{x \boxtimes y} &\coloneqq r^{\Cat{C}}_x \boxtimes r^{\Cat{D}}_y
            \end{align}
        \item $\Cat{C},\, \Cat{D}$ が\hyperref[def:spherical]{球状}\hyperref[def:tensorfusion-cat]{テンソル圏}ならば $\Cat{C} \boxtimes \Cat{D}$ は\hyperref[def:spherical]{球状}\hyperref[def:tensorfusion-cat]{テンソル圏}である．
            特に，$\Cat{C} \boxtimes \Cat{D}$ の\hyperref[def:spherical]{球状構造} $p^{\Cat{C} \boxtimes \Cat{D}}$ は次のようにして定まる：
            \begin{align}
                p^{\Cat{C} \boxtimes \Cat{D}}_{x \boxtimes y} \coloneqq p^{\Cat{C}}_x \boxtimes p^{\Cat{D}}_y
            \end{align}
        \item $\Cat{C},\, \Cat{D}$ が\hyperref[redef:braided-monoidal]{組紐付き}\hyperref[def:ringcat]{(多重) 環圏}/\hyperref[def:tensorfusion-cat]{(多重) テンソル圏}ならば $\Cat{C} \boxtimes \Cat{D}$ は\hyperref[redef:braided-monoidal]{組紐付き}\hyperref[def:ringcat]{(多重) 環圏}/\hyperref[def:tensorfusion-cat]{(多重) テンソル圏}である．
            特に，$\Cat{C} \boxtimes \Cat{D}$ の\hyperref[redef:braided-monoidal]{組紐} $b^{\Cat{C} \boxtimes \Cat{D}}$ は次のようにして定まる：
            \begin{align}
                b^{\Cat{C} \boxtimes \Cat{D}}_{x_1 \boxtimes y_1,\, x_2 \boxtimes y_2} 
                &\coloneqq b^{\Cat{C}}_{x_1,\, x_2} \boxtimes b^{\Cat{D}}_{y_1,\, y_2}
            \end{align}
        \item $\Cat{C},\, \Cat{D}$ が\hyperref[def:ribbon]{リボン}\hyperref[def:ringcat]{(多重) 環圏}/\hyperref[def:tensorfusion-cat]{(多重) テンソル圏}ならば $\Cat{C} \boxtimes \Cat{D}$ は\hyperref[def:ribbon]{リボン}\hyperref[def:ringcat]{(多重) 環圏}/\hyperref[def:tensorfusion-cat]{(多重) テンソル圏}である．
            特に，$\Cat{C} \boxtimes \Cat{D}$ の\hyperref[def:ribbon]{リボン構造} $\theta^{\Cat{C} \boxtimes \Cat{D}}$ は次のようにして定まる：
            \begin{align}
                \theta^{\Cat{C} \boxtimes \Cat{D}}_{x \boxtimes y} \coloneqq \theta^{\Cat{C}}_x \boxtimes \theta^{\Cat{D}}_y 
            \end{align}
    \end{enumerate}
\end{myprop}
...
\begin{mydef}[label=def:tensor-overalg]{代数上のテンソル積}
    \begin{itemize}
        ...
        \item $\Cat{C}$ における\hyperref[def:moduleobj]{左 $x$-加群対象} $(n,\, \lambda) \in \Obj{\MODCR{\Cat{C}}{x}}$
    \end{itemize}
...
\begin{mydef}[label=def:internal-hom,breakable]{加群圏におけるinternal hom}
    ...
    \begin{itemize}
        \item 関手
        \begin{align}
            \multimap \colon \OP{\Cat{M}} \times \Cat{M} \lto \Cat{C}
        \end{align}
        \item 
        \textbf{currying}と呼ばれる\hyperref[def:nat]{自然同型}
        \begin{align}
            \Familyset[\big]{c_{x,\, m_1,\, m_2} \colon \Hom{\Cat{M}}(x \btr m_1,\, m_2) \xrightarrow{\cong} \Hom{\Cat{C}}(x,\, m_1 \multimap m_2)}{x \in \Obj{\Cat{C}},\, Y,\, Z \in \Obj{\mathcal{C}}}
        \end{align}
    \end{itemize}    
\end{mydef}

\subsection{加群圏の完全可約性}

\begin{marker}
    混乱防止のため，以降では次の略記を行うことがある：
    \begin{itemize}
        \item \hyperref[def:modulecat]{左 $\Cat{C}$-加群圏} $(\Cat{M},\, \btr,\, \alpha,\, \lambda)$ のことを $\LModCat{\Cat{M}}{\Cat{C}}$ と書く．
        \item \hyperref[def:modulecat]{右 $\Cat{C}$-加群圏} $(\Cat{M},\, \btl,\, \beta,\, \rho)$ のことを $\RModCat{\Cat{M}}{\Cat{C}}$ と書く．
        \item \hyperref[def:bimodulecat]{$(\Cat{C}_1,\, \Cat{C}_2)$-両側加群圏} $(\Cat{M},\, \btr,\, \btl,\, \alpha,\, \beta,\, \lambda,\, \rho,\, b)$ のことを $\BiModCat{\Cat{M}}{\Cat{C}_1}{\Cat{C}_2}$ と書く．
    \end{itemize}
\end{marker}

\begin{mydef}[label=def:modulecat-exact,breakable]{完全な加群圏}
    \begin{itemize}
        \item 十分射影的対象を持つ
            \footnote{\hyperref[def:additive-cat]{$\mathbb{K}$-線形なアーベル圏} $\Cat{C}$ が\textbf{十分射影的対象を持つ} (has enough projectives) とは，$\forall x \in \Simp(\Cat{C})$ が\hyperref[def:finite-abcat]{射影的被覆}を持つこと．}
            \hyperref[def:tensorfusion-cat]{多重テンソル圏} $\Cat{C}$
        \item \hyperref[def:finite-abcat]{局所有限}な\hyperref[def:modulecat]{左 $\Cat{C}$-加群圏} $\LModCat{\Cat{M}}{\Cat{C}}$
        \item \hyperref[def:finite-abcat]{局所有限}な\hyperref[def:modulecat]{右 $\Cat{C}$-加群圏} $\RModCat{\Cat{M}}{\Cat{C}}$
    \end{itemize}
    を与える．このとき，
    \begin{itemize}
        \item $\LModCat{\Cat{M}}{\Cat{C}}$ が\textbf{完全} (exact) であるとは，任意の\hyperref[def:finite-abcat]{射影的対象} $p \in \Obj{\Cat{C}}$ および $\forall m \in \Obj{\LModCat{\Cat{M}}{\Cat{C}}}$ に対して，$p \btr m \in \Obj{\LModCat{\Cat{M}}{\Cat{C}}}$ が射影的対象であることを言う．
        \item $\RModCat{\Cat{M}}{\Cat{C}}$ が\textbf{完全} (exact) であるとは，任意の\hyperref[def:finite-abcat]{射影的対象} $p \in \Obj{\Cat{C}}$ および $\forall m \in \Obj{\RModCat{\Cat{M}}{\Cat{C}}}$ に対して，$m \btl p \in \Obj{\LModCat{\Cat{M}}{\Cat{C}}}$ が射影的対象であることを言う．
    \end{itemize}
    
    \tcblower

    \begin{itemize}
        \item 十分射影的対象を持つ\hyperref[def:tensorfusion-cat]{多重テンソル圏} $\Cat{C}_1,\, \Cat{C}_2$
        \item \hyperref[def:finite-abcat]{局所有限}な\hyperref[def:bimodulecat]{$(\Cat{C}_1,\, \Cat{C}_2)$-両側加群圏} $\BiModCat{\Cat{M}}{\Cat{C}_1}{\Cat{C}_2}$
    \end{itemize}
    を与える．このとき，$\BiModCat{\Cat{M}}{\Cat{C}_1}{\Cat{C}_2}$ が\textbf{完全} (exact) であるとは，任意の\hyperref[def:finite-abcat]{射影的対象} $p_i \in \Obj{\Cat{C}_i}$ および $\forall m \in \Obj{\BiModCat{\Cat{M}}{\Cat{C}_1}{\Cat{C}_2}}$ に対して，$p_1 \btr m \btl p_2 \in \Obj{\BiModCat{\Cat{M}}{\Cat{C}_1}{\Cat{C}_2}}$ が射影的対象であることを言う．
\end{mydef}

\begin{mylem}[label=lem:mod-exact]{半単純な加群圏は完全}
    十分射影的対象を持つ\hyperref[def:tensorfusion-cat]{多重テンソル圏} $\Cat{C}_1,\, \Cat{C}_2$ を与える．
    このとき，局所有限かつ半単純な\hyperref[def:modulecat]{左，右}，\hyperref[def:bimodulecat]{両側}加群圏 $\LModCat{\Cat{M}}{\Cat{C}_1},\, \RModCat{\Cat{M}}{\Cat{C}_2},\, \BiModCat{\Cat{M}}{\Cat{C}_1}{\Cat{C}_2}$ は全て\hyperref[def:modulecat-exact]{完全}である．
\end{mylem}

\begin{proof}
    局所有限かつ半単純な\hyperref[def:additive-cat]{$\mathbb{K}$-線形なアーベル圏}の任意の対象は\hyperref[def:finite-abcat]{射影的}である．
\end{proof}

\begin{mydef}[label=def:modulecat-indecomposable]{既約な加群圏}
    \begin{itemize}
        \item 十分射影的対象を持つ\hyperref[def:tensorfusion-cat]{多重テンソル圏} $\Cat{C}_1,\, \Cat{C}_2$
        \item \hyperref[def:finite-abcat]{局所有限}な\hyperref[def:modulecat]{左，右}，\hyperref[def:bimodulecat]{両側}加群圏 $\LModCat{\Cat{M}}{\Cat{C}_1},\, \RModCat{\Cat{M}}{\Cat{C}_2},\, \BiModCat{\Cat{M}}{\Cat{C}_1}{\Cat{C}_2}$
    \end{itemize}
    を与える．このとき，
    \begin{itemize}
        \item  $\LModCat{\Cat{M}}{\Cat{C}_1}$ が\textbf{既約} (indecomposable) であるとは，それが非自明な\hyperref[def:modulecat]{左 $\Cat{C}_1$-加群圏}の直和に\hyperref[def:equiv-1Cat]{圏同値}\underline{でない}ことを言う．
        \item  $\RModCat{\Cat{M}}{\Cat{C}_2}$ が\textbf{既約} (indecomposable) であるとは，それが非自明な\hyperref[def:modulecat]{右 $\Cat{C}_2$-加群圏}の直和に\hyperref[def:equiv-1Cat]{圏同値}\underline{でない}ことを言う．
        \item  $\BiModCat{\Cat{M}}{\Cat{C}_1}{\Cat{C}_2}$ が\textbf{既約} (indecomposable) であるとは，それが非自明な\hyperref[def:bimodulecat]{$(\Cat{C}_1,\, \Cat{C}_2)$-両側加群圏}の直和に\hyperref[def:equiv-1Cat]{圏同値}\underline{でない}ことを言う．
    \end{itemize}
\end{mydef}

\begin{mytheo}[label=thm:modulecat-complete-reducible]{完全な加群圏の完全可約性}
    十分射影的対象を持つ\hyperref[def:tensorfusion-cat]{多重テンソル圏} $\Cat{C}_1,\, \Cat{C}_2$ を与える．
    このとき，任意の\hyperref[def:modulecat-exact]{完全}な\hyperref[def:modulecat]{左，右}，\hyperref[def:bimodulecat]{両側}加群圏 $\LModCat{\Cat{M}}{\Cat{C}_1},\, \RModCat{\Cat{M}}{\Cat{C}_2},\, \BiModCat{\Cat{M}}{\Cat{C}_1}{\Cat{C}_2}$ は，
    それぞれ\hyperref[def:moduleat-indecomposable]{既約}な左，右，両側加群圏の直和で書ける．
\end{mytheo}

\begin{proof}
    ~\cite[PROPOSITION 7.6.7.]{etingof2015tensor}
\end{proof}

\subsection{加群関手の成すテンソル圏}

\begin{mydef}[label=def:module-Fun,breakable]{加群関手の圏}
    \begin{itemize}
        \item \hyperref[def:tensorfusion-cat]{多重テンソル圏} $\Cat{C}_1,\, \Cat{C}_2$
        \item \hyperref[def:modulecat-exact]{完全}な\hyperref[def:modulecat]{左，右}，\hyperref[def:bimodulecat]{両側}加群圏 $\LModCat{\Cat{M}_i}{\Cat{C}_1},\, \RModCat{\Cat{M}_i}{\Cat{C}_2},\, \BiModCat{\Cat{M}_i}{\Cat{C}_1}{\Cat{C}_2}\; (i=1,\, 2)$
    \end{itemize}
    を与える．このとき，
    \begin{enumerate}
        \item $(1,\, 1)$-圏 $\LModFun{\Cat{C}_1} (\LModCat{\Cat{M}_1}{\Cat{C}_1},\, \LModCat{\Cat{M}_2}{\Cat{C}_1})$ を次のように定義する：
        \begin{itemize}
            \item \hyperref[def:module-functor]{左 $\Cat{C}_1$-加群関手} $\LModCat{\Cat{M}_1}{\Cat{C}_1} \lto \LModCat{\Cat{M}_2}{\Cat{C}_1}$ を対象とする．
            \item \hyperref[def:module-functor]{左 $\Cat{C}_1$-加群自然変換}を射とする．
        \end{itemize}

        \item $(1,\, 1)$-圏 $\RModFun{\Cat{C}_2} (\RModCat{\Cat{M}_1}{\Cat{C}_2},\, \RModCat{\Cat{M}_2}{\Cat{C}_2})$ を次のように定義する：
        \begin{itemize}
            \item \hyperref[def:module-functor]{右 $\Cat{C}_2$-加群関手} $\RModCat{\Cat{M}_1}{\Cat{C}_2} \lto \RModCat{\Cat{M}_2}{\Cat{C}_2}$ を対象とする．
            \item \hyperref[def:module-functor]{右 $\Cat{C}_2$-加群自然変換}を射とする．
        \end{itemize}

        \item $(1,\, 1)$-圏 $\BiModFun{\Cat{C}_1}{\Cat{C}_2} (\BiModCat{\Cat{M}_1}{\Cat{C}_1}{\Cat{C}_2},\, \BiModCat{\Cat{M}_2}{\Cat{C}_1}{\Cat{C}_2})$ を次のように定義する：
        \begin{itemize}
            \item \hyperref[def:bimod-functor]{$(\Cat{C}_1,\, \Cat{C}_2)$-加群関手} $\BiModCat{\Cat{M}_1}{\Cat{C}_1}{\Cat{C}_2} \lto \BiModCat{\Cat{M}_2}{\Cat{C}_1}{\Cat{C}_2}$ を対象とする．
            \item \hyperref[def:module-functor]{$(\Cat{C}_1,\, \Cat{C}_2)$-両側加群自然変換}を射とする．
        \end{itemize}
    \end{enumerate}
\end{mydef}

\begin{marker}
    煩雑さを回避するため，以下では $\Fun$ の引数の中では\hyperref[def:modulecat]{左，右}，\hyperref[def:bimodulecat]{両側}加群圏 $\LModCat{\Cat{M}}{\Cat{C}_1},\, \RModCat{\Cat{M}}{\Cat{C}_2},\, \BiModCat{\Cat{M}}{\Cat{C}_1}{\Cat{C}_2}$ を全て $\Cat{M}$ と書く．
\end{marker}

\begin{marker}
    定義\ref{def:module-Fun}において，加群圏の\hyperref[def:modulecat-exact]{完全性}の条件を外すと，モノイダル圏 $\LModFun{\Cat{C}} (\LModCat{\Cat{M}}{\Cat{C}},\, \LModCat{\Cat{M}}{\Cat{C}})$ がrigidになってくれないなどの様々な弊害が生じる~\cite[p.154]{etingof2015tensor}．
    ゆえに，以降では特に断らない限り\hyperref[def:modulecat-exact]{完全}な加群圏のみを考える．
\end{marker}

\begin{mylem}[label=lem:exact-Fun]{}
    \begin{itemize}
        \item \hyperref[def:tensorfusion-cat]{多重テンソル圏} $\Cat{C}_1,\, \Cat{C}_2$
        \item \hyperref[def:modulecat-exact]{完全}な\hyperref[def:modulecat]{左，右}，\hyperref[def:bimodulecat]{両側}加群圏 $\LModCat{\Cat{M}_i}{\Cat{C}_1},\, \RModCat{\Cat{M}_i}{\Cat{C}_2},\, \BiModCat{\Cat{M}_i}{\Cat{C}_1}{\Cat{C}_2}\; (i=1,\, 2)$
    \end{itemize}
    を与える．このとき，3つの $(1,\, 1)$-圏
    % \hyperref[def:module-Fun]{
    \begin{align}
        &\LModFun{\Cat{C}_1} (\Cat{M}_1, \Cat{M}_2), &&\RModFun{\Cat{C}_2} (\Cat{M}_1,\, \Cat{M}_2), &&\BiModFun{\Cat{C}_1}{\Cat{C}_2} (\Cat{M}_1,\, \Cat{M}_2)
    \end{align}
    の全てについて以下が成り立つ：
    \begin{enumerate}
        \item 任意の対象が\hyperref[def:adjoint]{左/右随伴}を持つ．
        \item \hyperref[def:additive-cat]{$\mathbb{K}$-線形アーベル圏}である．
        \item $\Cat{C}_1,\, \Cat{C}_2$ が\hyperref[def:finite-abcat]{有限}ならば，\hyperref[def:finite-abcat]{有限な $\mathbb{K}$-線形アーベル圏}である．
    \end{enumerate}
\end{mylem}

\begin{proof}
    \begin{enumerate}
        \item ~\cite[LEMMA 7.11.4.]{etingof2015tensor}
        \item ~\cite[PROPOSITION 7.11.1.]{etingof2015tensor}
        \item ~\cite[PROPOSITION 7.11.6.]{etingof2015tensor}
    \end{enumerate}
\end{proof}

\begin{myprop}[label=prop:module-End]{{$\Fun(\Cat{M},\, \Cat{M})$} の多重テンソル圏としての構造}
    \begin{itemize}
        \item \hyperref[def:tensorfusion-cat]{多重テンソル圏} $\Cat{C}_1,\, \Cat{C}_2$
        \item \hyperref[def:modulecat-exact]{完全}な\hyperref[def:modulecat]{左，右}，\hyperref[def:bimodulecat]{両側}加群圏 $\LModCat{\Cat{M}}{\Cat{C}_1},\, \RModCat{\Cat{M}}{\Cat{C}_2},\, \BiModCat{\Cat{M}}{\Cat{C}_1}{\Cat{C}_2}$
    \end{itemize}
    を与える．このとき，3つの $(1,\, 1)$-圏
    \begin{align}
        &\LModFun{\Cat{C}_1} (\Cat{M}, \Cat{M}), &&\RModFun{\Cat{C}_2} (\Cat{M},\, \Cat{M}), &&\BiModFun{\Cat{C}_1}{\Cat{C}_2} (\Cat{M},\, \Cat{M})
    \end{align}
    の全てについて以下が成り立つ：
    \begin{enumerate}
        \item 関手の合成をテンソル積，恒等関手を単位対象とする\hyperref[redef:monoidal-category]{モノイダル圏}を成す．
        \item (1) のモノイダル構造に関して，\hyperref[def:adjoint]{随伴}の \hyperref[def:unit-counit-adjunction]{counit / unit} をそれぞれ\hyperref[redef:dual]{evaluation / coevaluation}とする\hyperref[redef:rigid]{rigidなモノイダル圏}を成す．
        \item $\Cat{C}_1,\, \Cat{C}_2$ が\hyperref[def:finite-abcat]{有限}ならば，\hyperref[def:finite-abcat]{有限}な\hyperref[def:tensorfusion-cat]{多重テンソル圏}である．
        \item $\Cat{C}_1,\, \Cat{C}_2$ が\hyperref[def:finite-abcat]{有限}かつ $\Cat{M}$ が\hyperref[def:modulecat-indecomposable]{既約}ならば，\hyperref[def:finite-abcat]{有限}な\hyperref[def:tensorfusion-cat]{テンソル圏}である．
    \end{enumerate}
\end{myprop}

\begin{proof}
    証明は全く同様なので，左 $\Cat{C}$-加群圏 $\LModCat{\Cat{M}}{\Cat{C}}$ の場合にのみ示す．
    \begin{enumerate}
        \item \hyperref[redef:monoidal-category]{テンソル積} $\otimes^{\LModFun{\Cat{C}}(\Cat{M},\, \Cat{M})} \colon \LModFun{\Cat{C}}(\Cat{M},\, \Cat{M})^{\times 2} \lto \LModFun{\Cat{C}}(\Cat{M},\, \Cat{M})$ を，次のように定義する：
        \begin{itemize}
            \item $\forall F,\, G \in \Obj{\Cat{\LModFun{\Cat{C}}(\Cat{M},\, \Cat{M})}}$ に対して
            \begin{align}
                F \otimes^{\LModFun{\Cat{C}}(\Cat{M},\, \Cat{M})} G \coloneqq F \circ G \in \Obj{\LModFun{\Cat{C}}(\Cat{M},\, \Cat{M})}
            \end{align}
            と定義する．
            \item $\forall \tau \in \Hom{\Cat{\LModFun{\Cat{C}}(\Cat{M},\, \Cat{M})}} (F,\, F'),\; \forall \tau' \in \Hom{\Cat{\LModFun{\Cat{C}}(\Cat{M},\, \Cat{M})}} (G,\, G')$ に対して
            \begin{align}
                \tau \otimes^{\LModFun{\Cat{C}}(\Cat{M},\, \Cat{M})} \tau' \coloneqq &\Familyset[\Big]{F\bigl(G(m)\bigr) \xrightarrow{F(\tau'_m)} F\bigl(G'(m)\bigr) \xrightarrow{\tau_{G'(m)}} F'\bigl( G'(m) \bigr)  }{m \in \Obj{\Cat{M}}} \\
                &\in \Hom{\LModFun{\Cat{C}}(\Cat{M},\, \Cat{M})} (F \circ G,\, F' \circ G')
            \end{align}
            と定義する．
        \end{itemize}
        $\otimes^{\LModFun{\Cat{C}}(\Cat{M},\, \Cat{M})}$ が関手であることを示そう．
        まず，4つの\hyperref[def:module-nat]{左 $\Cat{C}$-加群の自然変換}を
        \begin{align}
            F &\xRightarrow{\tau} F' \xRightarrow{\tau'} F'', \\
            G &\xRightarrow{\sigma} G' \xRightarrow{\sigma'} G''
        \end{align}
        のようにとる．$\tau$ は自然変換なので $\forall m \in \Obj{\Cat{M}}$ に対して $\tau_{G''(m)} \circ F(\sigma'_m) = F'(\sigma'_m) \circ \tau_{G'(m)}$ が成り立つ．
        故に
        \begin{align}
            &(\tau' \circ \tau) \otimes^{\LModFun{\Cat{C}}(\Cat{M},\, \Cat{M})} (\sigma' \circ \sigma) \\
            &= \Familyset[\big]{FG(m) \xrightarrow{F(\sigma_m)} FG'(m) \textcolor{red}{\xrightarrow{F(\sigma'_m)}} FG''(m) \textcolor{red}{\xrightarrow{\tau_{G''(m)}}} F'G''(m) \xrightarrow{\tau'_{G''(m)}} F''G''(m)}{m \in \Obj{\Cat{M}}} \\
            &= \Familyset[\big]{FG(m) \xrightarrow{F(\sigma_m)} FG'(m) \textcolor{red}{\xrightarrow{\tau_{G'(m)}}} F'G'(m) \textcolor{red}{\xrightarrow{F'(\sigma'_m)}} F'G''(m) \xrightarrow{\tau'_{G''(m)}} F''G''(m)}{m \in \Obj{\Cat{M}}} \\
            &= (\tau' \otimes^{\LModFun{\Cat{C}}(\Cat{M},\, \Cat{M})} \sigma') \circ (\tau \otimes^{\LModFun{\Cat{C}}(\Cat{M},\, \Cat{M})} \sigma)
        \end{align}
        が成り立ち，$\otimes^{\LModFun{\Cat{C}}(\Cat{M},\, \Cat{M})}$ が関手であることが示された．
        関手の合成の結合則から，3つ組 $\bigl(\LModFun{\Cat{C}}(\Cat{M},\, \Cat{M}),\, \otimes^{\LModFun{\Cat{C}}(\Cat{M},\, \Cat{M})},\, \Id_{\Cat{M}} \bigr)$ が\hyperref[redef:monoidal-category]{厳密なモノイダル圏}を成すことが分かる．
        以下では $\otimes^{\LModFun{\Cat{C}}(\Cat{M},\, \Cat{M})}$ を $\otimes$ と略記する．

        \hrulefill

        　ここで新しい記号を導入しておくと便利である．

        \begin{mydef}[label=def:whiskering]{whiskering}
            \hyperref[def:functor]{関手} $F,\, G,\, H_1,\, H_2$ および\hyperref[def:nat]{自然変換} $\tau$ を
            \begin{center}
                % https://tikzcd.yichuanshen.de/#
                \begin{tikzcd}
                \Cat{C} \arrow[r, "F"] & \Cat{D} \arrow[r, "H_1"{name=U, above}, bend left, shift left] \arrow[r, "H_2"{name=D, below}, bend right, shift right] & \Cat{E} \arrow[r, "G"] & \Cat{F}
                \ar[Rightarrow, "\tau", from=U, to=D]
                \end{tikzcd}
            \end{center}
            のように与える．
            このとき，$\tau$ の\textbf{wiskering}を
            \begin{align}
                \bm{\tau \cdot F} &\coloneqq \Familyset[\big]{\tau_{F(m)} \colon H_1 \circ F(m) \lto H_2 \circ F(m)}{m \in \Obj{\Cat{M}}}, \\
                \bm{G \cdot \tau} &\coloneqq \Familyset[\big]{G(\tau_{m}) \colon G \circ H_1(m) \lto G \circ H_2(m)}{m \in \Obj{\Cat{M}}}
            \end{align}
            で定義する．$\tau \cdot F,\, G \cdot \tau$ は自然変換である．
        \end{mydef}
        ...
        \begin{center}
            % https://tikzcd.yichuanshen.de/#
            \begin{tikzcd}
            \Cat{C} \arrow[r,red, "F"] & \Cat{D} \arrow[r,red, "H_1"{name=U, above}, bend left, shift left] \arrow[r, red, "H_2"{name=D, below}, bend right, shift right] & \Cat{E} \arrow[r, "G"] & \Cat{F}
            \ar[Rightarrow,red, "\tau", from=U, to=D]
            \end{tikzcd}
        \end{center}
        の赤色の部分を，$G \cdot \tau$ は
                \begin{center}
            % https://tikzcd.yichuanshen.de/#
            \begin{tikzcd}
            \Cat{C} \arrow[r,"F"] & \Cat{D} \arrow[r,blue, "H_1"{name=U, above}, bend left, shift left] \arrow[r, blue, "H_2"{name=D, below}, bend right, shift right] & \Cat{E} \arrow[r, blue, "G"] & \Cat{F}
            \ar[Rightarrow,blue, "\tau", from=U, to=D]
            \end{tikzcd}
        \end{center}
        の青色の部分を表している．
        ...
        \item 補題\ref{lem:exact-Fun}-(1) より，$\forall F \in \Obj{\LModFun{\Cat{C}} (\Cat{M},\, \Cat{M})}$ に対して常にその左随伴 $F^* \dashv F$ が存在する．
        このとき\hyperref[def:adjoint]{随伴の定義}および補題\ref{lem:rigid-Hom}から，$\forall m,\, n \in \Obj{\LModCat{\Cat{M}}{\Cat{C}}}$ および $\forall x\in \Obj{\Cat{C}}$ に対して
        \begin{align}
            \Hom{\LModCat{\Cat{M}}{\Cat{C}}} \bigl(m,\, F^*(x \btr n)\bigr)
            &\cong \Hom{\LModCat{\Cat{M}}{\Cat{C}}} \bigl(F(m),\, x \btr n\bigr) \\
            &\cong \Hom{\LModCat{\Cat{M}}{\Cat{C}}} \bigl(x^* \btr F(m),\, n\bigr) \\
            &\cong \Hom{\LModCat{\Cat{M}}{\Cat{C}}} \bigl(F(x^* \btr m),\, n\bigr) &&\because F\; \text{is a \hyperref[def:module-functor]{module functor}} \\
            &\cong \Hom{\LModCat{\Cat{M}}{\Cat{C}}} \bigl(x^* \btr m,\, F^*(n)\bigr) \\
            &\cong \Hom{\LModCat{\Cat{M}}{\Cat{C}}} \bigl(m,\, x \btr F^*(n)\bigr)
        \end{align}
        が成り立つ．故に\hyperref[col:Yoneda]{米田の補題}から自然同型 $F^* (x \btr n) \cong x \btr F^*(n)$ が存在し，$F^* \in \Obj{\LModFun{\Cat{C}} (\Cat{M},\, \Cat{M})}$ であることが分かった．
        右随伴 $F \vdash \rdual{F}$ についても同様に $\rdual{F} \in \Obj{\LModFun{\Cat{C}} (\Cat{M},\, \Cat{M})}$ が分かる．
        % counit / unit of adjunction が取れる．

        　ここで，\hyperref[def:adjoint]{随伴} $F^* \dashv F$ の\hyperref[def:unit-counit-adjunction]{unit, counit}によりそれぞれ自然変換
        \begin{align}
            \eta^F &\in \Hom{\LModFun{\Cat{C}} (\Cat{M},\, \Cat{M})} (\Id_{\Cat{M}},\, F \otimes F^*), \\
            \epsilon^F &\in \Hom{\LModFun{\Cat{C}} (\Cat{M},\, \Cat{M})} (F^* \otimes F,\, \Id_{\Cat{M}})
        \end{align}
        が定まる．
        いま，3つ組 $(F^*,\, \eta^F,\, \epsilon^F)$ が $F$ の\hyperref[redef:dual]{左双対}になっていることを示そう．
        実際，\hyperref[prop:unit-counit]{\textsf{\textbf{(triangle identity)}}}より
        \begin{align}
            \Id_F 
            &= ( F \cdot \epsilon^F ) \circ (\eta^F \cdot F) \\
            &= (\Id_F \cdot \Id_{\Cat{M}}) \circ ( F \cdot \epsilon^F ) \circ (\eta^F \cdot F) \circ (\Id_{\Cat{M}} \cdot \Id_F) \\
            &= (\Id_F \otimes \epsilon^F) \circ (\eta^F \otimes \Id_F) , \\
            \Id_{F^*} 
            &= \bigl( \epsilon^F \cdot F^*\bigr) \circ (F^* \cdot \eta^F) \\
            &= \bigl( \epsilon^F \cdot F^*\bigr) \circ (\Id_{\Cat{M}} \cdot \Id_{F^*}) \circ (\Id_{F^*} \cdot \Id_{\Cat{M}}) \circ (F^* \cdot \eta^F) \\
            &= (\epsilon^F \otimes \Id_{F^*}) \circ (\Id_{F^*} \otimes \eta^F)
        \end{align}
        と計算できるので，\hyperref[redef:dual]{\textsf{\textbf{(zig-zag-identity)}}}が示された．
        同様に\hyperref[def:adjoint]{右随伴} $F \vdash \rdual{F}$ に関する\hyperref[def:unit-counit-adjunction]{unit, counit}をとることで，$F$ の\hyperref[redef:dual]{右双対}が与えられる．
        \item (2) および補題\ref{lem:exact-Fun}-(3) から従う．
        \item 定理\ref{thm:modulecat-complete-reducible}より従う．詳細は~\cite[LEMMA 7.12.6.]{etingof2015tensor} を参照．
    \end{enumerate}
\end{proof}

\begin{mydef}[label=def:module-End]{双対テンソル圏}
    \begin{itemize}
        \item \hyperref[def:finite-abcat]{有限}な\hyperref[def:tensorfusion-cat]{多重テンソル圏} $\Cat{C}_1,\, \Cat{C}_2$
        \item \hyperref[def:modulecat-exact]{完全}な\hyperref[def:modulecat]{左，右}，\hyperref[def:bimodulecat]{両側}加群圏 $\LModCat{\Cat{M}}{\Cat{C}_1},\, \RModCat{\Cat{M}}{\Cat{C}_2},\, \BiModCat{\Cat{M}}{\Cat{C}_1}{\Cat{C}_2}$
    \end{itemize}
    を与える．このとき命題\ref{prop:module-End}-(3) により得られる\hyperref[def:finite-abcat]{有限}な\hyperref[def:tensorfusion-cat]{多重テンソル圏}をそれぞれ
    \begin{align}
        \LModEnd{\Cat{C}_1} (\Cat{M})&\coloneqq \LModFun{\Cat{C}_1} (\Cat{M},\,\Cat{M}), \\
        \RModEnd{\Cat{C}_2} (\Cat{M})&\coloneqq \RModFun{\Cat{C}_2} (\Cat{M},\,\Cat{M}),\\ 
        \BiModEnd{\Cat{C}_1}{\Cat{C}_2} (\Cat{M}) &\coloneqq \BiModFun{\Cat{C}_1}{\Cat{C}_2} (\Cat{M},\, \Cat{M})
    \end{align}
    とおき，$\Cat{M}$ による $\Cat{C}_i$ の\textbf{双対テンソル圏} (dual tensor category to $\Cat{C}_i$ with respect to $\Cat{M}$) と呼ぶ．
\end{mydef}
...
\begin{mydef}[label=def:Drinfeld-center]{Drinfeld中心}
    ...
    \begin{itemize}
        ...
        ただし，$\gamma_Z$ は以下の条件を充たさねばならない：
        \begin{description}
            \item[\textbf{(Half-braiding)}]　
            
            $\forall x,\, y \in \Obj{\Cat{C}}$ に対して以下の図式を可換にする：
            \begin{center}
...
\begin{proof}
    ...
    \begin{enumerate}
        ...
        \end{align}
        さらに，$1^{\Dri{\Cat{C}}} \coloneqq (1,\, l^{-1} \circ r) \in \Obj{\Dri{\Cat{C}}}$ および
        \begin{align}
        ...
        i.e. 5つ組 $\bigl( \Dri{\Cat{C}},\, \otimes^{\Dri{\Cat{C}}},\, 1^{\Dri{\Cat{C}}},\, a^{\Dri{\Cat{C}}},\, l^{\Dri{\Cat{C}}},\, r^{\Dri{\Cat{C}}} \bigr)$ は\hyperref[redef:monoidal-category]{モノイダル圏}である．
        ...
        i.e. 6つ組 $\bigl( \Dri{\Cat{C}},\, \otimes^{\Dri{\Cat{C}}},\, 1^{\Dri{\Cat{C}}},\, a^{\Dri{\Cat{C}}},\, l^{\Dri{\Cat{C}}},\, r^{\Dri{\Cat{C}}},\, b^{\Dri{\Cat{C}}} \bigr)$ は\hyperref[redef:braided-monoidal]{組紐付きモノイダル圏}である．
...
\begin{mylem}[label=lem:rev-center]{}
    \hyperref[redef:braided-monoidal]{組紐付きモノイダル圏} $(\Cat{C},\, \otimes,\, 1,\, a,\, l,\, r,\, b)$ を与える．
    \begin{enumerate}
        \item 以下の関手は\hyperref[def:faithful]{忠実充満}な\hyperref[def:braided-monoidal-functor]{組紐付きモノイダル関手}である：
        \begin{align}
            G_+ \colon \Cat{C} &\lto \Dri{\Cat{C}}, \\
            x &\lmto (x,\, b_{(\, \mhyphen \,),\, x}), \\
            (x \xrightarrow{f} y) &\lmto \bigl( (x,\, b_{(\, \mhyphen \,),\, x}) \xrightarrow{f} (y,\, b_{(\, \mhyphen \,),\, y}) \bigr) 
        \end{align}
        \item 以下の関手は\hyperref[def:faithful]{忠実充満}な\hyperref[def:braided-monoidal-functor]{組紐付きモノイダル関手}である：
        \begin{align}
            G_- \colon \revMon{\Cat{C}} &\lto \Dri{\Cat{C}}, \\
            x &\lmto (x,\, b^{-1}_{x,\, \mhyphen}), \\
            (x \xrightarrow{f} y) &\lmto \bigl( (x,\, b^{-1}_{x,\, (\, \mhyphen \,)}) \xrightarrow{f} (y,\, b^{-1}_{y,\, (\, \mhyphen \,)}) \bigr) 
        \end{align}
    \end{enumerate}
\end{mylem}

\begin{proof}
    証明は全く同様なので (1) についてのみ示す．

    まず，$G_+ \colon \Cat{C} \lto \Dri{\Cat{C}}$ がwell-definedであることを示そう．
    実際\hyperref[redef:braided-monoidal]{\textsf{\textbf{(hexagon equiations)}}}より，$\forall x \in \Obj{\Cat{C}}$ に対して自然同型
    \begin{align}
        b_{(\, \mhyphen \,),\, x} \coloneqq \Familyset[\big]{b_{y,\, x} \colon y \otimes x \xrightarrow{\cong} x \otimes y }{y \in \Obj{\Cat{C}}}
    \end{align}
    は \hyperref[def:Drinfeld-center]{\textsf{\textbf{(Half-braiding)}}} を充たすので　$G_+(x) \in \Obj{\Dri{\Cat{C}}}$ である．
    さらに $b \colon \otimes \Longrightarrow \otimes \circ \mathrm{swap}$ は\hyperref[def:nat]{自然同型}だから，
    $\forall z \in \Obj{\Cat{C}}$ および $\forall f \in \Hom{\Cat{C}} (X,\, Y)$ に対して
    \begin{align}
        \label{eq:nat-braiding}
        (f \otimes \Id_z) \circ b_{z,\, x} = b_{z,\, y} \circ (\Id_z \otimes f)
    \end{align}
    が成り立つ．i.e. $G_+(f) \coloneqq f \in \Hom{\Dri{\Cat{C}}} \bigl( (x,\, b_{(\, \mhyphen \,),\, x}),\, (y,\, b_{(\, \mhyphen \,),\, y}) \bigr)$ である．

    次に，$G_+$ が\hyperref[def:braided-monoidal-functor]{組紐つきモノイダル関手}であることを示す．
    $G_+(1) = (1,\, b_{(\, \mhyphen \,),\, 1})$ であるが，補題\ref{lem:dual-braid}-(1) より $b_{((\, \mhyphen \,),\, 1)} = l^{-1} \circ r$ が成り立つ．よって命題\ref{prop:Drinfeld-center}-(1) の証明における $1_{\Dri{\Cat{C}}}$ の構成から $1_{\Dri{\Cat{C}}} = G_+(1_{\Cat{C}})$ が分かる．
    さらに，$\forall x,\, y \in \Obj{\Cat{C}}$ に対して $G_+(x \otimes y) = (x \otimes y,\, b_{(\, \mhyphen \,),\, x \otimes y})$ であるが，\hyperref[redef:braided-monoidal]{\textsf{\textbf{(hexiagon equiations)}}}および命題\ref{prop:Drinfeld-center}-(1) の証明における $\otimes^{\Dri{\Cat{C}}}$ の構成から
    \begin{align}
        b_{(\, \mhyphen \,),\, x \otimes y}
        &= a^{-1}_{x,\, y,\, (\, \mhyphen \,)} \circ (\Id_x \otimes b_{(\, \mhyphen \,),\, y}) \circ a_{x,\, (\, \mhyphen \,),\, y} \circ (b_{(\, \mhyphen \,),\, x} \otimes \Id_y) \circ a^{-1}_{(\, \mhyphen \,),\, x,\, y} \\
        &= b_{(\, \mhyphen \,),\, x} \otimes^{\Dri{\Cat{C}}} b_{(\, \mhyphen \,),\, y}
    \end{align}
    が言える．i.e. $G_+$ は\hyperref[redef:monidal-functor]{強いモノイダル関手}である．
    そのうえ，命題\ref{prop:Drinfeld-center}-(1) の証明における $b^{\Dri{\Cat{C}}}$ の構成から
    \begin{align}
        b^{\Dri{\Cat{C}}}_{(x,\, b_{(\, \mhyphen \,),\, x}),\, (y,\, b_{(\, \mhyphen \,),\, y})} = b_{x,\, y} = G_+(b_{x,\, y})
    \end{align}
    が言えるので，$G_+$ が強い\hyperref[def:braided-monoidal-functor]{組紐つきモノイダル関手}であることが分かった．

    最後に，$G_+$ が\hyperref[def:faithful]{忠実充満}であること，i.e. $\forall x,\, y \in \Obj{\Cat{C}}$ に対して写像
    \begin{align}
        \Hom{\Cat{C}} (x,\, y) &\lto \Hom{\Dri{\Cat{C}}} \bigl( (x,\, b_{(\, \mhyphen \,),\, x}),\, (y,\, b_{(\, \mhyphen \,),\, y}) \bigr), \\
        f &\lto G_+(f)
    \end{align}
    が全単射であることを示す．
    実際 $\forall z \in \Obj{\Cat{C}}$ および $\forall f \in \Hom{\Cat{C}} (X,\, Y)$ に対して \eqref{eq:nat-braiding} が成り立つことから，$\Hom{\Dri{\Cat{C}}} \bigl( (x,\, b_{(\, \mhyphen \,),\, x}),\, (y,\, b_{(\, \mhyphen \,),\, y}) \bigr) = \Hom{\Cat{C}} (x,\, y)$ が分かる．
\end{proof}

補題\ref{lem:rev-center}より，忠実充満な強い\hyperref[def:braided-monoidal-functor]{組紐つきモノイダル関手}
\begin{align}
    \label{def:decomposer}
    G \colon \Cat{C} \boxtimes \revMon{\Cat{C}} &\lto \Dri{\Cat{C}}, \\
    x \boxtimes y &\lmto G_+(x) \otimes^{\Dri{\Cat{C}}} G_-(y), \\
    f \boxtimes g &\lmto G_+(f) \otimes^{\Dri{\Cat{C}}} G_-(g)
\end{align}
が定まる．

\begin{mytheo}[label=thm:factorizable]{分解可能なテンソル圏}
    $\Cat{C}$ を\hyperref[redef:braided-monoidal]{組紐付き}\hyperref[def:tensorfusion-cat]{フュージョン圏}とする．
    このとき，\hyperref[def:faithful]{忠実充満}な\hyperref[def:braided-monoidal-functor]{組紐つきモノイダル関手} \eqref{def:decomposer} が\hyperref[def:braided-monoidal-functor]{組紐付きテンソル同値}であるための必要十分条件は，
    $\Cat{C}$ が\hyperref[def:nondegen-BFC]{非退化}であることである．
\end{mytheo}

\begin{proof}
    ~\cite[Proposition 3.7]{DGNO2009BFC}
\end{proof}

\subsection{左加群圏とDrinfeld中心}

\begin{mytheo}[label=thm:module-center]{}
    \begin{itemize}
        \item \hyperref[redef:braided-monoidal]{組紐付き}\hyperref[def:tensorfusion-cat]{フュージョン圏} $\Cat{C}$
        \item \hyperref[def:modulecat-indecomposable]{既約}な\hyperref[def:modulecat]{右 $\Cat{C}$-加群圏} $\RModCat{\Cat{M}}{\Cat{C}}$
    \end{itemize}
    を与える．このとき，\hyperref[def:braided-monoidal-functor]{組紐付きテンソル同値}
    \begin{align}
        \Dri{\Cat{C}} \cong \Dri{\RModEnd{\Cat{C}}(\RModCat{\Cat{M}}{\Cat{C}})}
    \end{align}
    がある．
\end{mytheo}

\begin{proof}
    ~\cite[COROLLARY 7.16.2.]{etingof2015tensor}
\end{proof}
...
$\Cat{C}$ を\hyperref[redef:monoidal-category]{モノイダル圏}とする．
\hyperref[def:Drinfeld-center]{Drinfeld中心の定義}から，常に忘却関手
\begin{align}
    \forget \colon \Dri{\Cat{C}} &\lto \Cat{C}, \\
    (Z,\, \gamma_Z) &\lmto Z
\end{align}
を考えることができる．

\begin{mydef}[label=def:central-functor,breakable]{中心的関手}
    \begin{itemize}
        \item \hyperref[def:finite-abcat]{有限}の\hyperref[redef:braided-monoidal]{組紐付き}\hyperref[def:tensorfusion-cat]{テンソル圏} $\Cat{C}$ 
        \item \hyperref[def:finite-abcat]{有限}の\hyperref[def:tensorfusion-cat]{テンソル圏} $\Cat{A}$
        \item \hyperref[def:tensor-functor]{テンソル関手} $F \colon \Cat{C} \lto \Cat{A}$
    \end{itemize}
    を与える．
    このとき，$F$ の\textbf{中心構造} (central structure) とは，
    ある\hyperref[def:braided-monoidal-functor]{組紐つき}\hyperref[def:tensor-functor]{テンソル関手} $\bar{F} \colon \Cat{C} \lto \Dri{\Cat{A}}$ であって
    $F = \forget \circ \bar{F}$ を充たすもののこと．
    \begin{center}
        % https://tikzcd.yichuanshen.de/#N4Igdg9gJgpgziAXAbVABwnAlgFyxMJZABgBoBGAXVJADcBDAGwFcYkQAdDgYXp2G4BfEINLpMufIRTkK1Ok1bsuvfgEFho8djwEis4vIYs2iThwAiAJyzAVfYBs3yYUAObwioAGZWIAWyQyEBwIJAAmGmMlMy4AI3orYAAxTTEQXwCImlCkWQUTZQ5vCCsPHBF0zMDEYNzEfMZ6OJhGAAUJXWkQGzcACwqoxVMQZJFKQSA
        \begin{tikzcd}
                                                    & \Dri{\Cat{A}} \arrow[d, "\forget"] \\
        \Cat{C} \arrow[ru, "\bar{F}"] \arrow[r, "F"'] & \Cat{A}                           
        \end{tikzcd}
    \end{center}
    \textbf{中心的関手} (central functor) とは，組み $(F,\, \bar{F})$ のこと．
    \tcblower

    同じことだが，$F$ の\textbf{中心構造}とは\hyperref[def:nat]{自然同型}
    \begin{align}
        \gamma = \Familyset[\big]{\gamma_{X,\, a} \colon a \otimes F(X) \xrightarrow{\cong} F(X) \otimes a}{a \in \Obj{\Cat{A}},\, X \in \Cat{C}}
    \end{align}
    であって以下の条件を充たすもののこと\footnote{この自然同型 $\gamma$ により，$\bar{F}(X) \coloneqq \bigl( F(X),\, \gamma_{X,\, (\, \mhyphen \,)} \bigr)  \in \Obj{\Dri{\Cat{A}}}$ として $\bar{F} \colon \Cat{C} \lto \Dri{\Cat{A}}$ が定まる}：
    \begin{description}
        \item[\textbf{(central-1)}]
        $\forall X \in \Cat{C}$ および $\forall a,\, b \in \Cat{A}$ に対して，以下の $\Cat{A}$ の図式を可換にする\footnote{この条件は\hyperref[def:Drinfeld-center]{\textsf{\textbf{(Half-braiding)}}}に対応する．}：
        \begin{center}
            % https://tikzcd.yichuanshen.de/#-oDgbqBrLoXCEUieKQuHQVDskuLJbNVY73fjXU9hQNyZSeDS6YzmcA0iiLaoKCogA
            \begin{tikzcd}
            (a \otimes b) \otimes F(X) \arrow[d, "{\gamma_{X,\,a\otimes b}}"'] \arrow[r, "{a^{\Cat{A}}_{a,b,F(X)}}"] & a \otimes \bigl(b \otimes F(X)\bigr) \arrow[r, "{\Id_a \otimes \gamma_{X,\, b}}"] & a \otimes \bigl(F(X) \otimes b\bigr) \arrow[d, "{(a^{\Cat{A}}_{a,F(X),b})^{-1}}"]  \\
            F(X) \otimes (a \otimes b)                                                                                & \bigl(F(X) \otimes a\bigr) \otimes b \arrow[l, "{a^{\Cat{A}}_{F(X),a,b}}"]        & \bigl(a \otimes F(X)\bigr) \otimes b \arrow[l, "{\gamma_{X,\, a} \otimes \Id_b}"']
            \end{tikzcd}
        \end{center}
        
        \item[\textbf{(central-2)}]
        $\forall X,\, Y \in \Cat{C}$ および $\forall a \in \Cat{A}$ に対して，以下の $\Cat{A}$ の図式を可換にする\footnote{この条件は $\bar{F}$ が\hyperref[def:tensor-functor]{テンソル関手}であるという条件に対応する．より正確には，条件\hyperref[def:tensor-functor]{\textsf{\textbf{(TF-2)}}}のうち $\bar{F}(X \otimes^{\Cat{C}} Y) = \bar{F}(X) \otimes^{\Dri{\Cat{A}}} \bar{F}(Y)$ の，half-braidingの部分の等号に対応する（命題\ref{prop:Drinfeld-center}-(1) の証明における $\otimes^{\Dri{\Cat{A}}}$ の構成も参照）．}：
        \begin{center}
            % https://tikzcd.yichuanshen.de/#N4Igdg9gJgpgziAXAbVABwnAlgFyxMJZABgBoBGAXVJADcBDAGwFcYkR6ACAHW4jwC28TgDEAFAA0efQcICaAShABfUuky58hFOVLFqdJq3Zde-LELjSARlgDmjMeIkLp5y6LGLetuwCclVXVsPAIiMgAmAwYWNkQQZzdZK0Uki2F6FTUQDBCtIl0AZmijOJAfe0dnVzNkz29uXwC0j0ygnI1Q7WQI0mKaGON46pbhCocnLxqZdKt6cYCs4M0wlF6ogdL2EdrZm0qxUxmPcQamwOzcle7e-U3Y9nHHI--OepAAdkLoPJlJpdOZ3FZ7Jh0gAklBY1NW+is0gAKydxA+4kg3hTmcuZTjskUqm0pkstlz6ULxA981L3v0QMhsOR6PbuvVn4ZyjKIA
            \begin{tikzcd}[column sep=large, row sep=large]
                                                                            & a \otimes \bigl(F(X) \otimes F(Y)\bigr) \arrow[ld, "="'] \arrow[r, "{(a^{\Cat{A}}_{a,F(X),F(Y)})^{-1}}"] & \bigl(a \otimes F(X)\bigr) \otimes F(Y) \arrow[d, "{\gamma_{X,\, a} \otimes \Id_{F(Y)}}"] \\
            a \otimes F(X \otimes Y) \arrow[d, "{\gamma_{X \otimes Y,\, a}}"'] &                                                                                                          & \bigl(F(X) \otimes a\bigr) \otimes F(Y) \arrow[d, "{a^{\Cat{A}}_{F(X),a,F(Y)}}"]          \\
            F(X \otimes Y) \otimes a                                           &                                                                                                          & F(X) \otimes \bigl(a \otimes F(Y)\bigr) \arrow[d, "{\Id_{F(X)} \otimes \gamma_{Y,\, a}}"] \\
                                                                            & \bigl(F(X) \otimes F(Y)\bigr) \otimes a \arrow[lu, "="]  & F(X) \otimes \bigl(F(Y) \otimes a\bigr)     \arrow[l, "{(a^{\Cat{A}}_{F(X),F(Y),a})^{-1}}"]                                             
            \end{tikzcd}
        \end{center}
        
        \item[\textbf{(central-3)}] 
        $\forall X,\, Y \in \Cat{C}$ に対して，以下の $\Cat{A}$ の図式を可換にする\footnote{この条件は\hyperref[def:tensor-functor]{テンソル関手} $\bar{F}$ が\hyperref[def:braided-monoidal-functor]{組紐付きモノイダル関手}であるという条件に対応する（命題\ref{prop:Drinfeld-center}-(1) の証明における $b^{\Dri{\Cat{A}}}$ の構成も参照）．}：
        \begin{center}
            % https://tikzcd.yichuanshen.de/#
            \begin{tikzcd}
            F(X) \otimes F(Y) \arrow[d, "="'] \arrow[r, "{\gamma_{F(Y),\, X}}"] & F(Y) \otimes F(X) \arrow[d, "="] \\
            F(X \otimes Y) \arrow[r, "{F(b^{\Cat{C}}_{X,\, Y})}"']              & F(Y \otimes X)                  
            \end{tikzcd}
        \end{center}
    \end{description}
\end{mydef}
...
圏論的トレースの理論は，~\cite{henriques2015categorifiedtracemoduletensor}によって大きく発展した．

\begin{mydef}[label=def:cat-trace]{圏論的トレース}
    \begin{itemize}
        \item \hyperref[def:tensorfusion-cat]{多重テンソル圏} $(\Cat{A},\, \otimes,\, 1,\, a,\, l,\, r,\, \lev,\, \lcoev,\, \rev,\, \rcoev)$
        \item 圏 $\Cat{C}$
    \end{itemize}
    を与える．\textbf{圏論的トレース} (categorified trace) は，以下のデータからなる：
    \begin{itemize}
        \item 関手 $\Tr \colon \Cat{A} \lto \Cat{C}$
        \item \textbf{traciator}と呼ばれる\hyperref[def:nat]{自然同型}
        \begin{align}
            \Familyset[\big]{\tau_{x,\, y} \colon \Tr(x \otimes y) \xrightarrow{\cong} \Tr(y \otimes x)}{x,\,y \in \Obj{\Cat{A}}}
        \end{align}
    \end{itemize}
    これらは以下の条件を充たさねばならない：
    \begin{description}
        ...
        $\forall x,\, y,\, z \in \Obj{\Cat{A}}$ について，$\Cat{C}$ における以下の図式を可換にする：
        \begin{center}
            % https://tikzcd.yichuanshen.de/#-P4A6jApD1cHTVqiOhwhHKUg6UwAPWAAFp7KYKpEqf9EAy6YgAMyTU4tdhQ2Hw679TlonaipAyoEgxAAFllEOZrOAOlI7J5-MFwu+vzFBp1SAArIamQroXCysiVoi0RisTj8dVRiBiUIyRTNYgXY7EGCmm7eCy4RyzeJLQKhWYKKYgA
            \begin{tikzcd}[row sep=large, column sep=large]
            \Tr\bigl(x \otimes (y \otimes z)\bigr) \arrow[d, "{\tau_{x,y\otimes z}}"'] \arrow[r, "{\Tr(a_{x,y,z}^{-1})}"] & \Tr\bigl((x \otimes y) \otimes z\bigr) \arrow[r, "{\tau_{x\otimes y,z}}"] & \Tr\bigl(z \otimes (x \otimes y)\bigr) \arrow[d, "{\Tr(a_{z,x,y}^{-1})}"] \\
            \Tr\bigl((y \otimes z) \otimes x\bigr)                                                                        & \Tr\bigl(y \otimes (z \otimes x)\bigr) \arrow[l, "{\Tr(a_{y,z,x}^{-1})}"] & \Tr\bigl((z \otimes x)\otimes y\bigr) \arrow[l, "{\tau_{z\otimes x,y}}"]
            \end{tikzcd}
        \end{center}
    \end{description}
\end{mydef}

圏論的トレース $\Tr \colon \Cat{A} \lto \Cat{C}$ をストリング図式で描くときは，$\Cat{A}$ のストリング図式が描かれている平面を円筒状に「丸める」ことで表現する~\cite[p.14]{henriques2015categorifiedtracemoduletensor}：
\begin{align}
    \Tr \left(\,
        \begin{tikzpicture}[baseline={([yshift=-.5ex]current bounding box.center)}]
            % plane
            ...
            % string
            \draw[->-=.5] (0,0) -- node[midway,right] {$x$} ++(0,1) node[bullet] (f) {};
            \draw[->-=.5] (f.center) -- node[midway,right] {$y$} ++(0,1);
        \end{tikzpicture}
     \,\right) 
     \quad \eqqcolon \quad
     \begin{tikzpicture}[baseline={([yshift=-.5ex]current bounding box.center)}]
    % cylinder
        ...
        \pgfmathsetmacro{\h}{2}
        \halfDottedEllipse{(0,0)}{\R}{\r}
        \Ellipse{(0,\h)}{\R}{\r}
        \draw[thick] (0,0) -- (0,\h);
        \draw[thick] ({2*\R},0) -- ({2*\R},\h);
        % string
        \draw[->-=.5] (\R,-\r) -- node[midway,right] {$x$} ++(0,1) node[bullet] (f) {};
        \draw[->-=.5] (f.center) -- node[midway,right] {$y$} ++(0,1);
        \node[left] at (f) {$f$};
    \end{tikzpicture}
    \quad \in \Hom{\Cat{C}} \bigl( \Tr(x),\, \Tr(y) \bigr)
\end{align}
特に，\hyperref[def:categorialTr]{traciator}は次のように図示する：
\begin{align}
    \tau_{x,\, y} 
    &\eqqcolon \quad
    \begin{tikzpicture}[baseline={([yshift=-.5ex]current bounding box.center)}]
        ...
        \draw[thick] ({2*\R},0) -- ({2*\R},\h);
        % string
        \draw[->-=.5] ({\R - .2},-\r)  node[above left] {$x$} .. controls +(0,1) and +(0,-1) .. ++(.4,\h);
        \draw[->-=.5] ({\R + .2},-\r)  node[above right] {$y$} .. controls +(90:.5) and +(-135:.2) .. ({2*\R},{.4*\h});
        \draw[->-=.5] (0,{.6*\h}) .. controls +(45:.2) and +(-90:.5) .. ({\R - .2},{\h - \r});
        \draw[dashed] ({2*\R},{.4*\h}) -- (0,{.6*\h});
    \end{tikzpicture},
    &\tau_{y,\, x}^{-1}
    &\eqqcolon \quad
    \begin{tikzpicture}[baseline={([yshift=-.5ex]current bounding box.center)}]
        ...
        \draw[thick] ({2*\R},0) -- ({2*\R},\h);
        % string
        \draw[->-=.5] ({\R + .2},-\r)  node[above right] {$y$} .. controls +(0,1) and +(0,-1) .. ++(-.4,\h);
        \draw[->-=.5] ({\R - .2},-\r)  node[above left] {$x$} .. controls +(90:.5) and +(-45:.2) .. (0,{.4*\h});
        \draw[->-=.5] ({2*\R},{.6*\h}) .. controls +(135:.2) and +(-90:.5) .. ({\R + .2},{\h - \r});
        \draw[dashed] (0,{.4*\h}) -- ({2*\R},{.6*\h});
    \end{tikzpicture}
\end{align}

\begin{myprop}[label=prop:associated-categorialTr]{中心的関手に同伴する圏論的トレース}
    \begin{itemize}
        \item \hyperref[def:finite-abcat]{有限}の\hyperref[def:pivotal]{旋回構造}付き\hyperref[redef:braided-monoidal]{組紐}付き\hyperref[def:tensorfusion-cat]{テンソル圏} $\Cat{C}$ 
        \item \hyperref[def:finite-abcat]{有限}の\hyperref[def:pivotal]{旋回構造}付き\hyperref[def:tensorfusion-cat]{テンソル圏} $\Cat{A}$
        \item \hyperref[def:central-functor]{中心的関手} $F \colon \Cat{C} \lto \Cat{A}$
    \end{itemize}
    を与える．このとき，$F$ の\hyperref[def:adjoint]{右随伴} 
    \begin{center}
        \begin{tikzcd}
            \Cat{C}\ar[r,bend left,"F",""{name=A, below}] & \Cat{A}\ar[l,bend left,"\Tr_{\Cat{C}}",""{name=B,above}] 
            \ar[from=A, to=B, symbol=\dashv]
            \end{tikzcd}
    \end{center}
    は\hyperref[def:categorialTr]{圏論的トレース}である~\cite[4.4]{henriques2015categorifiedtracemoduletensor}．
\end{myprop}
