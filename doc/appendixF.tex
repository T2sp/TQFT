...
\begin{mydef}[label=def:additive-cat]{加法圏・アーベル圏}
    ...
    \begin{description}
        ...
        \textbf{零対称}\footnote{始対象かつ終対象} (zero object) $\bm{0} \in \Obj{\Cat{C}}$ が存在し，$\forall X \in \Obj{\Cat{C}}$ に対して $\Hom{\Cat{C}}(\bm{0},\, X) = \Hom{\Cat{C}}(\bm{X},\, 0) = 0$ を充たす\footnote{最右辺は \textsf{\textbf{(add-1)}} の意味で零ベクトル空間．}．
