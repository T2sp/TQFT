    ...
    \begin{myprop}[label=reprop:unitary-Cstar]{ユニタリ構造と $C^*$ 構造}
        \hyperref[def:additivecat]{$\mathbb{C}$-線形なアーベル圏} $\Cat{C}$ が\hyperref[def:starcat]{$C^*$-圏}であることと，\hyperref[redef:unitary]{ユニタリ圏}であることは同値である．
    \end{myprop}
...
\begin{myprop}[label=prop:S-matrix]{$S$-行列の性質}
    ...
    \begin{enumerate}
        ...
        \end{align}
        は，\hyperref[eq:fusionring]{フュージョン環}から $\mathbb{K}$ への環準同型を定める．
    \end{enumerate}
\end{myprop}
...
\begin{proof}
    \begin{enumerate}
        ...
        \begin{align}
            \Tr (b_{y \otimes z} \circ c_{x,\, y \otimes z})
            &= \sum_{w \in \Simp(\Cat{C})} N^w_{yz} \Tr (b_{w,\, x} \circ c_{x,\, w}) \\
