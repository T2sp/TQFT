...
\begin{mydef}[label=def:tensor-functor]{テンソル関手・ファイバー関手}
    ...
    \begin{description}
        ...
        \item[\textbf{(TF-2)}] $F$ は\hyperref[redef:monidal-functor]{弱いモノイダル関手}
...
\begin{myexample}[label=def:CG]{圏 $\Cat{C}(G)$}
    $G$ を群とする．\hyperref[redef:monoidal-category]{厳密なモノイダル圏} $\Cat{C}(G)$ を，
    \begin{itemize}
        \item $\Obj{\Cat{C}(G)} \coloneqq G$
        \item $\forall g_1,\, g_2 \in G$ に対して
        \begin{align}
            \Hom{\Cat{C}(G)}(g_1,\, g_2) \coloneqq 
            \begin{cases}
                \LU(1), &g_1 = g_2 \\
                \emptyset, &g_1 \neq g_2
            \end{cases}
        \end{align}
        \item $g_1 \otimes g_2 \coloneqq g_1 g_2$ かつ，$g \xrightarrow{\forall \theta_1,\, \theta_2}  g$ に対して $\theta_1 \otimes \theta_2 \coloneqq \theta_1 \theta_2$
        \item $1 \coloneqq 1_G$
        \item $a_{g_1,g_2,g_3} \coloneqq 1,\; l_{g} \coloneqq 1,\, r_g \coloneqq 1$
    \end{itemize}
    で定義する．
    
    $\mathbb{K} = \mathbb{C}$ とする．
    % $\Cat{C}(G)$ は明らかに\hyperref[def:tensorfusion-cat]{フュージョン圏}である．
\end{myexample}

\begin{myexample}[label=def:CGa]{圏 $\Cat{C}(G;\, \alpha)$}
    $G$ を群とする．\exref{def:CG}の\hyperref[redef:monoidal-category]{associatorとunitors}を非自明にしてみよう．
    今 $\alpha \in \irm{Z^3}{Grp} \bigl(G;\, \LU(1)\bigr)$ を1つ固定する\footnote{i.e. 3-コサイクル}．
    
    \hyperref[redef:monoidal-category]{モノイダル圏} $\Cat{C}(G;\, \alpha)$ を，
    \begin{align}
        a_{g_1,g_2,g_3} &\coloneqq \alpha(g_1,\, g_2,\, g_3), \\
        l_{g} &\coloneqq \alpha(1,\, 1,\, g)^{-1}, \\
        r_{g} &\coloneqq \alpha(g,\, 1,\, g)
    \end{align}
    とおくことにより定義する\footnote{他のデータは\exref{def:CG}と全く同じである}．実際，\hyperref[redef:monoidal-category]{\textsf{\textbf{(pentagon identity)}}}はコサイクル条件および $\LU(1)$ の可換性により
    \begin{align}
    ...
    \end{align}
    が成り立ち，充たされている．
    % \underline{$G$ が有限群ならば} $\Cat{C}(G;\, \alpha)$ は\hyperref[def:tensorfusion-cat]{フュージョン圏}である．
\end{myexample}
...
\begin{mydef}[label=def:qtrace,breakable]{量子トレース}
    \begin{itemize}
        ...
        \item 対象 $V \in \Cat{C}$
        \item $f \in \Hom{\Cat{C}}(V,\, V^{**}),\; g \in \Hom{\Cat{C}}(V,\, \prescript{**}{}{V})$
    \end{itemize}
    を与える．
    \begin{itemize}
        \item $f$ の\textbf{左量子トレース} (left quantum trace) を以下で定義する：
        \begin{align}
            \lqTr(f) \colon 1 \xrightarrow{\lcoev_V} V \otimes V^* \xrightarrow{f \otimes \Id_V} V^{**} \otimes V^* \xrightarrow{\lev_{V^*}} 1
        \end{align}
        ...
        \begin{align}
            ...
            \begin{tikzpicture}[baseline={([yshift=-.5ex]current bounding box.center)}]
                ...
                \draw[->-=.5] (A1) -- node[midway, left]{$V$} (f);
                \draw[->-=.5] (f) -- node[midway, left]{$V^{**}$} (A2);
                \draw[->-=.5] (B2) -- node[midway, right]{$V^{*}$} (B1);
                \LCOEV{A1}{B1}
                \REV{A2}{B2}
            \end{tikzpicture}
            \in \Hom{\Cat{C}} (1,\, 1)
        ...
        \begin{align}
            \rqTr(g) \colon 1 \xrightarrow{\rcoev_{\rdual{V}}} \rdual{V} \otimes V \xrightarrow{\Id_{\rdual{V}} \otimes g} \rdual{V} \otimes \prescript{**}{}{V} \xrightarrow{\rev_{\prescript{**}{}{V}}} 1
        \end{align}
        ストリング図式で書くと次のようになる\footnote{$\rev_{\prescript{**}{}{V}} = \lev_{\rdual{V}}$ を使った．}：
        \begin{align}
            \lqTr(g) \coloneqq 
            \begin{tikzpicture}[baseline={([yshift=-.5ex]current bounding box.center)}]
                \coordinate (A1) at (0,-1);
                ...
                \coordinate (B1) at (1,-1);
                \coordinate (B2) at (1,1);
                \draw[->-=.5] (B1) -- node[midway, right]{$V$} (g);
                \draw[->-=.5] (g) -- node[midway, right]{$\prescript{**}{}{V}$} (B2);
                \draw[->-=.5] (A2) -- node[midway, left]{$\rdual{V}$} (A1);
                \RCOEV{A1}{B1}
                \LEV{A2}{B2}
            \end{tikzpicture}
            \in \Hom{\Cat{C}} (1,\, 1)
...
\begin{mydef}[label=def:qdim,breakable]{球状圏・量子次元}
    ...
    圏 $\Cat{C}$ が\textbf{球状} (spherical) であるとは，$\forall V \in \Obj{\Cat{C}}$ に対して
    \begin{align}
        \lqTr(p_V) =
        \begin{tikzpicture}[baseline={([yshift=-.5ex]current bounding box.center)}]
            \coordinate (A1) at (0,-1);
            \coordinate (A2) at (0,1);
            \node[squarednode] (f) at (0,0) {$p_V$};
            \coordinate (B1) at (1,-1);
            \coordinate (B2) at (1,1);
            \draw[->-=.5] (A1) -- node[midway, left]{$V$} (f);
            \draw[->-=.5] (f) -- node[midway, left]{$V^{**}$} (A2);
    ...
    球状圏 $\Cat{C}$ の対象 $V \in \Obj{\Cat{C}}$ の\textbf{量子次元} (quantum dimension) を以下で定義する：
    \begin{align}
        \dim_p (V) \coloneqq \lqTr(p_V) \coloneqq 
        \begin{tikzpicture}[baseline={([yshift=-.5ex]current bounding box.center)}]
            \coordinate (A1) at (0,-1);
            \coordinate (A2) at (0,1);
            \node[squarednode] (f) at (0,0) {$a_V$};
            \coordinate (B1) at (1,-1);
            \coordinate (B2) at (1,1);
            \draw[->-=.5] (A1) -- node[midway, left]{$V$} (f);
            \draw[->-=.5] (f) -- node[midway, left]{$V^{**}$} (A2);
...
\begin{mydef}[label=def:modulecat]{左加群圏}
    ...
    \begin{itemize}
        \item 圏 $\Cat{M}$
        \item 関手 $\btr \colon \Cat{C} \times \Cat{M} \lto \Cat{M}$
        \item \hyperref[def:nat]{自然同型} $\Familyset[\big]{\alpha_{x,\, y,\, m} \colon (x \otimes y) \btr m \lto x \btr (y \btr m)}{x,y \in \Obj{\Cat{C}},\, m \in \Obj{\Cat{M}}}$
    \end{itemize}
    これらは以下の条件を充たす：
    \begin{description}
    ...
    \textbf{右 $\Cat{C}$-加群圏} $\Cat{M}$ も同様に定義される．
\end{mydef}
...
\begin{mydef}[label=def:bimodulecat,breakable]{両側加群圏}
    ...
    \begin{itemize}
        \item 圏 $\Cat{M}$
        \item 関手 $\btr \colon \Cat{C}_1 \times \Cat{M} \lto \Cat{M}$
        \item 関手 $\btl \colon \Cat{M} \times \Cat{C}_2 \lto \Cat{M}$
        \item \hyperref[def:nat]{自然同型} $\Familyset[\big]{\alpha_{x_1,\, y_1,\, m} \colon (x_1 \otimes y_1) \btr m \lto x_1 \btr (y_1 \btr m)}{x_1,y_1\in \Obj{\Cat{C}_1},\, m \in \Obj{\Cat{M}}}$
        \item \hyperref[def:nat]{自然同型} $\Familyset[\big]{\beta_{m,\, x_2,\, y_2} \colon m \btl (x_2 \otimes y_2) \lto  (m \btl x_2) \btl y_2}{x_2,y_2 \in \Obj{\Cat{C}_2},\, m \in \Obj{\Cat{M}}}$
        \item \hyperref[def:nat]{自然同型} $\Familyset[\big]{b_{x_1,\,m,\, x_2} \colon (x_1 \btr m) \btl x_2 \lto  x_1 \btr (m \btl x_2)}{x_1 \in \Obj{\Cat{C}_1},\, x_2 \in \Obj{\Cat{C}_2} m \in \Obj{\Cat{M}}}$
    \end{itemize}
    これらは以下の条件を充たす：
    \begin{description}
        \item[\textbf{(Bimod-1)}] 関手 $\Cat{M} \lto \Cat{M},\; m \lmto 1 \btr m$ および $\Cat{M} \lto \Cat{M},\; m \lmto m \btl 1$ は圏の自己同型である
        \item[\textbf{(Bimod-2)}] $\forall x,\,y,\, z \in \Obj{\Cat{C}_1},\; \forall m \in \Obj{\Cat{M}}$ に対して以下の図式を可換にする：
        \begin{center}
            \begin{tikzcd}[row sep=large, column sep=large]
                 & &\bigl( (x \otimes y) \otimes z \bigr) \btr m \ar[ddl, "a_{x,y,z}\btr \Id_m"']\ar[dr, "\alpha_{x\otimes y,z,m}"] & \\
                 & & &(x\otimes y) \btr (z \btr m) \ar[dd, "\alpha_{x,y,z\btr m}"] \\
            ...
            \end{tikzcd}
        \end{center}
        \item[\textbf{(Bimod-3)}] $\forall x,\,y,\, z \in \Obj{\Cat{C}_2},\; \forall m \in \Obj{\Cat{M}}$ に対して以下の図式を可換にする：
        \begin{center}
            \begin{tikzcd}[row sep=large, column sep=large]
                 & &m \btl \bigl( x \otimes (y \otimes z) \bigr) \ar[ddl, "\Id_m \btl a_{x,y,z}"']\ar[dr, "\beta_{m, x,y\otimes z}"] & \\
                 & & &(m \btl x) \btl (y\otimes z) \ar[dd, "\beta_{m\btl x,y,z}"] \\
                 &m \btl \bigl( (x \otimes y) \otimes z \bigr) \ar[ddr, "\beta_{m,x\otimes y,z}"'] & & \\
                 & & &\bigl((m \btl x) \btl y \bigr) \btl z \\
                 & &\bigl(m \btl (x \otimes y) \bigr) \btl z \ar[ur, "\Id_x \btl \beta_{m,x,y}"'] &
            \end{tikzcd}
        \end{center} 
        \item[\textbf{(Bimod-4)}] $\forall x_i,\, y_i \in \Obj{\Cat{C}_i},\; \forall m \in \Obj{\Cat{M}}$ に対して以下の図式を可換にする：
        \begin{center}
            \begin{tikzcd}[row sep=large, column sep=large]
