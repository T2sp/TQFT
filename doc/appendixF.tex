...
圏論的トレース $\Tr \colon \Cat{A} \to \Cat{C}$ をストリング図式で描くときは，$\Cat{A}$ のストリング図式が描かれている平面を円筒状に「丸める」ことで表現する~\cite[p.14]{henriques2015categorifiedtracemoduletensor}：
\begin{align}
    \Tr \left(\,
        \begin{tikzpicture}[baseline={([yshift=-.5ex]current bounding box.center)}]
            % plane
    ...
    % cylinder
        ...
        \node[left] at (f) {$f$};
    \end{tikzpicture}
    \in \Hom{\Cat{A}} \bigl( \Tr(x),\, \Tr(y) \bigr)
\end{align}
