...
時計方向の過程はその逆射で与えられる．組紐を全ての $x,\, y$ に関して集めたものは\hyperref[def:nat]{自然同型} $b \colon \otimes \Longrightarrow \otimes \circ \tau$ を成す．
...
\begin{figure}[H]
    ...
    \begin{subfigure}{0.4\columnwidth}
        ...
        \begin{tikzpicture}
            ...
            +(0,4) coordinate (ux)
            +(2,4) coordinate (uy)
            +(3,1) coordinate (ctrl_1)
            +(3,3) coordinate (ctrl_2)
            ;
            \begin{knot}[
            ...
            \draw[name path=y, ->-=.5] (dy.center) -- (uy);
            \draw[name path=x, ->-=.5] (dx.center) .. controls (ctrl_1) and (ctrl_2) .. (ux);
        \end{tikzpicture}
