...
\begin{marker}
    gapless quantum liquidの厳密な定義は，筆者の知る限りではこれといったものがない．
\end{marker}
...
\begin{mydefph}[label=def:quantum-phase,breakable]{bosonicかつgappedなトポロジカル秩序}
    熱力学極限化にある2つの\hyperref[def:gappedQL]{gappedな量子液体} $\bigl( \{\Lambda_i\}_i,\, \{\hat{H}^{(0)}_{\Lambda_i}\}_i \bigr),\; \bigl( \{\Lambda_i\}_i,\, \{\hat{H}^{(1)}_{\Lambda_i}\}_i \bigr)$ を与える．
    \hyperref[def:bosonic-lattice-model]{熱力学極限}をとった\hyperref[def:gappedQL]{gappedな量子液体}全体がなす集合を $\mathrm{gQL}_{(D)}$ とおく．$\mathrm{gQL}_{(D)}$ には適切な位相を入れて位相空間にする
    \footnote{この位相空間を可逆相の場合に厳密に定義した仕事として，~\cite{Kubota2025}が挙げられる．}．

    　このとき，$\hat{H}^{(0)}_{\Lambda_\infty}$ と $\hat{H}^{(1)}_{\Lambda_\infty}$ が同じ\textbf{gappedなトポロジカル秩序} (gapped topological order) にあるとは，連続曲線 $\hat{H} \colon [0,\, 1] \lto \mathrm{gQL}_{D}$ が存在して
...
\subsection{Remote-detectability}
...
\begin{mydefph}[label=def:anomalousQP]{Remote detectability}
    空間 $D$ 次元の\hyperref[def:quantum-phase]{量子相}が\textbf{遠隔検出可能} (remote detectable) である\footnote{文献によっては\textbf{アノマリーを持たない} (anomaly free) と呼ばれることもある．}
    とは，それが\hyperref[def:bosonic-lattice-model]{局所的な格子模型}として実現できることを言う．
\end{mydefph}

\begin{myconjph}[label=conj:gravitational-anomaly]{}
    空間次元 $D$ の任意の\hyperref[def:quantum-phase]{gappedな量子相} $\mathsf{A}_D$ に対して，
    ある空間次元 $D+1$ の\hyperref[def:anomalousQP]{遠隔検出可能}なgappedな量子相 $\Bulk{\mathsf{A}_D}$ が一意的に存在して，$\Bulk{\mathsf{A}_D}$ の境界として $\mathsf{A}_D$ が実現される．
    ~\cite[Lemma 2, p.19]{KongWen2014braidedfusioncategoriesgravitational}．
\end{myconjph}

% 予想\ref{conj:gravitational-anomaly}は，$\mathsf{A}_D$ が\hyperref[def:quantum-phase]{トポロジカル秩序}の場合には物理的な証明がある~\cite[Lemma 2, p.19]{KongWen2014braidedfusioncategoriesgravitational}．
...
\begin{mytheo}[label=thm:gapped-decay]{gappedな格子模型における相関関数の振る舞い}
    $D$ 次元格子 $\Lambda$ 及びその上の\hyperref[def:gapped]{gappedな量子系} $\hat{H}_\Lambda$ を考える．
...
従って，局所演算子が合成に関して成す結合代数 (\textbf{local operator algebra}) を $\irm{\mathcal{A}}{loc}$ と書くと，\hyperref[def:TD]{トポロジカル欠陥}を次のように特徴付けることもできる：
...
\begin{myprop}[label=prop:TD]{トポロジカル欠陥の特徴付け}
    \hyperref[def:TD]{トポロジカル欠陥}とは，$\irm{\mathcal{A}}{loc}$-加群のこと．
\end{myprop}
...
~\cite{KongZhang2022TO}に倣い，$\mathsf{C}_D$ 内部の全ての\hyperref[def:TD]{トポロジカル欠陥}（i.e. トポロジカル秩序）の集まりを\textbf{topological skelton}と呼び，$\sk (\mathsf{C}_D)$ と書くことにする．
次に考えるべきなのは，topologial skeltonのどの構造が，親となるトポロジカル秩序 $\mathsf{C}_D$ を完全に特徴付けるのか，と言う問題である．
2026年現在では次のように予想されている~\cite[Conjecture 2, p.11]{KongWen2014braidedfusioncategoriesgravitational}：
...
$x,\, y$ がそれぞれ互いに異なる点 $\xi,\, \eta \in \mathbb{R}^D$ に局在しているとしよう．このとき，代表元の素朴なテンソル積は，トポロジカル欠陥のレベルでのテンソル積 $x \otimes_{(\xi,\, \eta)} y$ を定める．
i.e. $x,\, y \in \Obj{\sk_0(\mathsf{C}_D)}$ のテンソル積の定義には，集合 $\bigl\{\, (\xi,\, \eta) \in \mathbb{R}^D \bigm| \xi \neq \eta \,\bigr\}$ だけの不定性が存在し得る．
定義\ref{def:TD}直下の注の方法で $\sk_0(\mathsf{C}_D)$ を，点 $\xi \in \mathbb{R}^D$ におけるトポロジカルな境界条件が成す圏 $\sk_0(\mathsf{C}_D)_\xi$ と見做すと，$\otimes_{(\xi,\, \eta)}$ は関手
\begin{align}
    \otimes_{(\xi,\, \eta)} \colon \sk_0(\mathsf{C}_D)_\xi \times \sk_0(\mathsf{C}_D)_\eta \lto \sk_0(\mathsf{C}_D)
\end{align}
を定める．

ところで，$(\xi,\, \eta) \in \Conf_2 (\mathbb{R}^D)$ を\footnote{位相空間 $X$ の\textbf{配置の空間} (configuration space) を $\Conf(X) \coloneqq \bigl\{\, (\xi,\, \eta) \in X^{\times 2} \bigm| \xi \neq \eta \,\bigr\}$ と定義した．}連続曲線 $\gamma \colon [0,\, 1] \lto \Conf (\mathbb{R}^D)$ に沿って $(\xi',\, \eta')$ まで断熱的に\footnote{i.e. 途中経過のトラップハミルトニアンの\hyperref[def:gapped]{ギャップ}を閉じずに}移動することで，
...
このようなときには，テンソル積 $\otimes_{(\xi,\, \eta)}$ は高次のホモトピーに依存すると考えられ，その記述には\hyperref[def:infty-operad]{$\infty$-オペラッド}が適していると考えらえる．
% $\sk_0(\mathsf{C}_D)$ の上に\textbf{$A_\infty$-圏}の構造を与える．
...
非自明な1つの\hyperref[def:TD]{トポロジカル欠陥}を，局所演算子によって生成・消滅させることはできない．
...
トーリックコードの $e$-エニオン（resp. $m$-エニオン）の場合は，\exref{ex:toric-emf}で見たように $\hat{Z}$ （resp. $\hat{X}$）を辺に作用させることで生成・消滅が記述できる．
\begin{figure}[H]
    \centering
...
トポロジカル欠陥の世界線は太さを持たない紐のように見えるが，実際には\hyperref[def:framing]{フレーミング}を持っていると見做すべきである．
この考え方はChern-Simons理論におけるWilson lineの期待値の計算において発散を除去する目的でWittenが導入し~\cite{Witten1989quantum}，後にReshetikhinとTuraevによるリボングラフTQFTの構成に繋がった~\cite{RT1991invariant}．
このReshetikhin-TuraevによるリボングラフTQFTの構成を後の章でレビューする．

トポロジカル欠陥 $x \in \Obj{\sk_0(\mathsf{C}_2)}$ の\textbf{リボン構造}は\footnote{\textbf{トポロジカルスピン} (topological spin) と呼ばれることもある．}，図\ref{fig:topological-spin}に示す世界線に対応する同型射であり，具体的には
\begin{align}
    \theta_x 
    &\coloneqq (x \xrightarrow{\lev_x{}^\dagger \otimes \Id_x} x^* \otimes x \otimes x \xrightarrow{\Id_{x^*} \otimes b_{x,\, x}} x^{*} \otimes x \otimes x \xrightarrow{\lev_x \otimes \Id_x}) \\
    &= (x \xrightarrow{\Id_x \otimes \lcoev_x} x \otimes x \otimes x^* \xrightarrow{b_{x,\, x} \otimes \Id_{x^*}} x \otimes x \otimes x^* \xrightarrow{\Id_x \otimes \lcoev_x{}^\dagger} x)
\end{align}
と構成される．一般のリボン構造の定義は付録Fの定義\ref{def:ribbon}も併せて参照されたい．
図\ref{fig:topological-spin}において，フレーミングは上から見た時に反時計回りに $2\pi$ 回転している．
後に\exref{ex:SO2alg}において，この見方が $\LSO(2)$ 構造付き\hyperref[def:DiskB-alg]{disk algebra}の構造射として $\theta$ を定めることを見る．
...
\begin{figure}[H]
    \centering
        ...
        \path[
            postaction=decorate,
            decoration={markings,
                mark=between positions 0.05 and 0.95 step 0.05 with {
                    \draw[-Stealth,red] (0,0) -- (0,-.3);
                }
            }
        ] 
            (0,0) -- (0,1) .. controls +(90:1) and +(0:.5) .. (-1,3) .. controls +(180:1) and +(180:1) .. (-1,1) .. controls +(0:.5) and +(-90:1) .. (0,3) -- (0,4);
        ;
        % knot
        \begin{knot}[
            consider self intersections,
        ]
            \strand[blue,very thick] (0,0) -- (0,1) .. controls +(90:1) and +(0:.5) .. (-1,3) .. controls +(180:1) and +(180:1) .. (-1,1) .. controls +(0:.5) and +(-90:1) .. (0,3) -- (0,4);
        \end{knot}
        % \draw[blue,very thick] (0,0) .. controls +(100:4) and +(-100:4) .. (0,4);
        \draw[dotted,blue,very thick] (0,0) -- ++(0,-.5);
        \draw[dotted,blue,very thick] (0,4) -- ++(0,.5);
    \end{tikzpicture}
    \caption{
        トポロジカル欠陥のリボン構造．赤い矢印は世界線のフレーミングを表す．
    }
    \label{fig:topological-spin}
\end{figure}%

\subsection{UMTC}

% トポロジカル秩序がと $\mathsf{C}_D$ の\hyperref[redef:braided-monoidal]{組紐構造}の間には重要な関係があると考えられている~\cite{KongWen2014braidedfusioncategoriesgravitational}：

\begin{myconjph}[label=conj:remote-detectable]{remote-detectable}
    トポロジカル秩序が\hyperref[def:anomalousQP]{遠隔検出可能である}ことと，
    余次元 $2$ 以上のトポロジカル欠陥が\hyperref[redef:braided-monoidal]{braiding}によって検出できることは等価である．
    ~\cite{KongWen2014braidedfusioncategoriesgravitational}
\end{myconjph}
...
\begin{mydef}[label=def:MTC]{MTC}
    \textbf{モジュラーテンソル圏} (modular tensor category; \textbf{MTC}) とは，\hyperref[def:nondegen-BFC]{非退化}な\hyperref[def:premodular-cat]{前モジュラー圏}のこと．
\end{mydef}

\begin{mypropph}[label=prop:sk0-braided]{$\sk_0(\mathsf{C}_2)$ はMTC}
    空間多様体 $\mathbb{R}^2$ 上に置かれた\hyperref[def:stableTO]{安定}かつ\hyperref[def:anomalousQP]{遠隔検出可能}な2+1次元のトポロジカル秩序 $\mathsf{C}_2$ が持つ点状のトポロジカル欠陥全体
...
MTCのよく使う性質を命題\ref{preprop:MTC}にまとめた．
